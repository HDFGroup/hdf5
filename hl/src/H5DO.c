...

/*-------------------------------------------------------------------------
 * Function:    H5DOsequence()
 *
 * Purpose:     To read sequences of elements from a dataset.
 *
 *      axis:       the dataset dimension (zero-based) for the sequential read
 *      extension:  the # of elements to read for the axis-th dimension
 *      memtype:    the datatype
 *      buf:        buffer for data from the read
 *
 * Return:      Non-negative on success/Negative on failure
 *
 * Programmer:  Vailin Choi; Jan 2014
             ...
             size_t sequence, hid_t memtype, void *buf)
{
    ...
    if((ndims = H5Sget_simple_extent_ndims(space_id)) < 0) {
        printf("Error:unable to get dataspace dimension:  %s: %d\n", __func__, __LINE__);
        //HGOTO_ERROR(H5E_INTERNAL, H5E_CANTGET, FAIL, "unable to get dataspace dimension");
    }
    ...
    /* get the dimensions sizes of the dataspace */
    if(H5Sget_simple_extent_dims(space_id, size, NULL) < 0) {
        printf("Error: unable to get dataspace dimension sizes: %s: %d\n", __func__, __LINE__);
        //HGOTO_ERROR(H5E_INTERNAL, H5E_CANTGET, FAIL, "unable to get dataspace dimension sizes");
    }
    ...
    if(space_id != FAIL && H5Sclose(space_id) < 0) {
        ...
        //HDONE_ERROR(H5E_DATASPACE, H5E_CANTDEC, FAIL, "unable to close dataspace");//HDONE_ERROR->HGOTO_ERROR
    }
