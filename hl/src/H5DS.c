...
static herr_t H5DS_is_reserved_ff(hid_t did, hid_t rc_id, hid_t estack_id);
			...
			const char *dimname, uint64_t version, hid_t rc_id, hid_t tr_id, hid_t estack_id)
{
    ...
    */
    ...
    //    return FAIL;
    printf("h5aexists \n");
    if ((H5Aexists_ff(dsid, DIMENSION_LIST, &exists1, rc_id, estack_id)) < 0)
       return FAIL;
    ...
    */
    ...
    ret = H5TRstart( tr_id, H5P_DEFAULT, H5_EVENT_STACK_NULL );

    printf("H5LT_find_ff \n");

      /* verify if the attribute already exists */
    has_attr = H5LT_find_attribute_ff(dsid, "CLASS", rc_id, estack_id);  

    printf("H5LT_set_attribute_string_ff \n");
    if (H5LT_set_attribute_string_ff(dsid,"CLASS",DIMENSION_SCALE_CLASS, rc_id, tr_id, estack_id, has_attr) < 0)
        return FAIL;

    printf("H5LT_set_attribute_string_ff2 \n");
    if (dimname!=NULL)
    {
      if (H5LT_set_attribute_string_ff(dsid,"NAME",dimname, rc_id, tr_id, estack_id, has_attr) < 0)
            return FAIL;
    }

    ret = H5TRfinish( tr_id, H5P_DEFAULT, &rc_id, H5_EVENT_STACK_NULL );
...
herr_t H5DSattach_scale_ff(hid_t did,hid_t dsid,
			   unsigned int idx, hid_t rc_id, hid_t tr_id,  hid_t estack_id)
{
    ...
    */
    ...
    if ((is_scale = H5DSis_scale_ff(did, rc_id, estack_id)) < 0)
        return FAIL;
    ...
    /* get info for the dataset in the parameter list */
    if(H5Oget_info_ff(did, &oi1, rc_id, estack_id) < 0)
        return FAIL;

    /* get info for the scale in the parameter list */
    if(H5Oget_info_ff(dsid, &oi2, rc_id, estack_id) < 0)
        return FAIL;
    ...
    /* the DS dataset cannot have dimension scales */
    if (H5LT_find_attribute_ff(dsid, DIMENSION_LIST, rc_id, estack_id)==1)
        return FAIL;

    /* check if the dataset is a "reserved" dataset (image, table) */
    if (H5DS_is_reserved_ff(did, rc_id, estack_id)==1)
        return FAIL;
    ...
    */
    ...
    /* try to find the attribute "DIMENSION_LIST" on the >>data<< dataset */
    if ((has_dimlist = H5LT_find_attribute_ff(did,DIMENSION_LIST, rc_id, estack_id)) < 0)
        return FAIL;
    ...
    */
    if (has_dimlist == 0)
    {
      ...
      dims[0] = (hsize_t)rank;
        ...
        /* create the attribute */
        if((aid = H5Acreate_ff(did, DIMENSION_LIST, tid, sid, H5P_DEFAULT, H5P_DEFAULT, tr_id, estack_id)) < 0)
            goto out;
    ...
    */
    ...
    {
        if((aid = H5Aopen_ff(did, DIMENSION_LIST, H5P_DEFAULT, rc_id, estack_id)) < 0)
            goto out;
        ...
        /* read */
        if(H5Aread_ff(aid, tid, buf, rc_id, estack_id) < 0)
            goto out;
        ...
        for(i = 0; i < (int)buf[idx].len; i++) {
            ...
            /* get info for DS in the parameter list */
            if(H5Oget_info_ff(dsid, &oi1, rc_id, estack_id) < 0)
                goto out;

            /* get info for this DS */
            if(H5Oget_info_ff(dsid_j, &oi2, rc_id, estack_id) < 0)
                goto out;
    ...
    */
    ...
    /* try to find the attribute "REFERENCE_LIST" on the >>DS<< dataset */
    if((has_reflist = H5LT_find_attribute_ff(dsid, REFERENCE_LIST, rc_id, estack_id)) < 0)
        goto out;
    ...
    */
    if(has_reflist == 0) {
        ...
        /* create the attribute */
        if((aid = H5Acreate_ff(dsid, REFERENCE_LIST, tid, sid, H5P_DEFAULT, H5P_DEFAULT, tr_id, estack_id)) < 0)
            goto out;
        ...
        /* write the attribute with the reference */
        if(H5Awrite_ff(aid, tid, &dsl, tr_id, estack_id) < 0)
            goto out;
    ...
    */
        ...
        if((aid = H5Aopen_ff(dsid, REFERENCE_LIST, H5P_DEFAULT, rc_id, estack_id)) < 0)
            goto out;
        ...
        if(H5Aread_ff(aid, ntid, dsbuf, rc_id, estack_id) < 0)
            goto out;
        ...
        */
        ...
        /* the attribute must be deleted, in order to the new one can reflect the changes*/
        if(H5Adelete_ff(dsid, REFERENCE_LIST, tr_id, estack_id) < 0)
            goto out;
        ...
        /* write the attribute with the new references */
        if(H5Awrite_ff(aid, ntid, dsbuf, tr_id, estack_id) < 0)
            goto out;
    ...
    */
    ...
    if((is_ds=H5DSis_scale_ff(dsid, rc_id, estack_id)) < 0)
        return FAIL;

    printf("SHOULD FAIL HERE H5DS.c not set\n");
    if(is_ds == 0) {
        if (H5LT_set_attribute_string_ff(dsid,"CLASS",DIMENSION_SCALE_CLASS, rc_id, tr_id, estack_id, has_attr) < 0)
            return FAIL;
    }
...
htri_t H5DSis_scale_ff(hid_t did, hid_t rc_id, hid_t estack_id)
{
    hid_t      tid = -1;   /* attribute type ID */
    hid_t      aid = -1;   /* attribute ID */
    herr_t     has_class;  /* has the "CLASS" attribute */
    ...
    */
    ...
    if ((H5Aexists_ff(did, "CLASS", &exists1, rc_id, estack_id)) < 0)
       return FAIL;

    has_class = exists1;

    //  if((has_class = H5LT_find_attribute_ff(did, "CLASS")) < 0)
    //    return FAIL;
    ...
    {
      if((aid = H5Aopen_ff(did, "CLASS", H5P_DEFAULT, rc_id, estack_id)) < 0)
            goto out;
...
static
herr_t H5DS_is_reserved_ff(hid_t did, hid_t rc_id, hid_t estack_id)
{
    int    has_class;
    hid_t  tid = -1;
    hid_t  aid = -1;
    ...
    /* try to find the attribute "CLASS" on the dataset */
    if((has_class = H5LT_find_attribute_ff(did, "CLASS", rc_id, estack_id)) < 0)
        return -1;
