...
### Fixed the page buffer corrupting its page counts when removing an entry

`H5PB_remove_entry()` decremented `meta_count` for every entry it removed, but `H5PB__insert_entry()` charges an entry to `raw_count` or `meta_count` according to the entry's own type. A page entry's type is stamped when the entry is created and is never revisited, so it records whichever access first brought the page into the buffer rather than whatever the page holds later, and it is not bounded by the allocation type the caller is operating on. A page allocated for the global heap, for instance, is stamped `H5F_MEM_PAGE_GHEAP` and charged to `raw_count`, because `H5MF__alloc_pagefs()` passes the allocation type to `H5PB_add_new_page()` and `H5PB_write()` reuses that entry without changing its type. Removing such an entry left `raw_count` too high and decremented `meta_count` for a page it had never counted. Both counts govern the minimum metadata and minimum raw data page protection, and because they are unsigned, `meta_count` could wrap and leave those reservations wrong for the remaining life of the file. The counts are now decremented according to the same classification that incremented them, every decrement asserts that the count it is releasing is non-zero, and `H5MF__sect_small_merge()` no longer calls `H5PB_remove_entry()` for raw allocation types, the global heap included.
