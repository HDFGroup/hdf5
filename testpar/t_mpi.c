        ...
        */
        ...
        for (n = 2; n <= 4; n += 2) {
            ...
            for (i = ntimes - 2; i <= ntimes; i++) {
                ...
                /* set data to some trivial pattern for easy verification */
                for (j = 0; j < MB; j++)
                    *(buf + j) = (char)(i * mpi_size + mpi_rank);
                if (VERBOSE_MED)
                    HDfprintf(stdout,
        ...
        */
        ...
        for (n = 2; n <= 4; n += 2) {
            ...
            for (i = ntimes - 2; i <= ntimes; i++) {
                ...
                expected = (char)(i * mpi_size + (mpi_size - mpi_rank - 1));
                vrfyerrs = 0;
                for (j = 0; j < MB; j++) {
                    if ((*(buf + j) != expected)
                            && (vrfyerrs++ < MAX_ERR_REPORT || VERBOSE_MED)) {
...
static int test_mpio_1wMr(char *filename, int special_request) {
    ...
    * ==================================================*/
    irank = 0;
    for (i = 0; i < DIMSIZE; i++)
        writedata[i] = (unsigned char)(irank * DIMSIZE + i);
    ...
    * ==================================================*/
    ...
    for (i = 0; i < DIMSIZE; i++) {
        expect_val = (unsigned char)(irank * DIMSIZE + i);
