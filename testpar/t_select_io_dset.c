...
check_io_mode(hid_t dxpl, unsigned chunked)
{
    ...
    if (chunked) {
        if (actual_io_mode != H5D_MPIO_CHUNK_COLLECTIVE) {
            nerrors++;
            if (MAINPROCESS)
                HDprintf("\n     Failed: Incorrect I/O mode (expected chunked, returned %u)", (unsigned)actual_io_mode);
        }
    }
    else if (actual_io_mode != H5D_MPIO_CONTIGUOUS_COLLECTIVE) {
        nerrors++;
        if (MAINPROCESS)
            HDprintf("\n     Failed: Incorrect I/O mode (expected contiguous, returned %u)", (unsigned)actual_io_mode);
    }
            ...
            for (test_select_config = (int)TEST_NO_TYPE_CONV; test_select_config < (int)TEST_SELECT_NTESTS;
                ...
                switch (test_select_config) {
                    ...
                    case TEST_MULTI_CONV_SEL_EMPTY: /* case 10 */
                        if (MAINPROCESS)
                            TESTING_2("multi-datasets: type conversion+empty selections");
