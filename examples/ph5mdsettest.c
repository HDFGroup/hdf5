...
typedef enum mpio_collective_mode_t { MPIO_COLLECTIVE_IO = 0, MPIO_INDIVIDUAL_IO } mpio_collective_mode_t;
...
typedef enum multi_mode_t { MULTI_DSET = 0, SINGLE_DSET } multi_mode_t;
...
int  dataset_vrfy(hsize_t start[], hsize_t count[], hsize_t stride[], DTYPE_INT *dataset, DTYPE_INT *original,
                  int dnum);
...
static float
timeval2float(struct timeval *tv)
{
    float ret_val = 0;
...
static void
float2timeval(float value, struct timeval *tv)
{
    int status    = 0;
    int sec       = (int)value;
    (*tv).tv_sec  = sec;
...
void
dataset_fill2_1d(hsize_t start[], hsize_t stride[], hsize_t count[], hsize_t block[], DTYPE_INT *dataset,
                 int add)
{
...
void
dataset_fill2_2d(hsize_t start[], hsize_t stride[], hsize_t count[], hsize_t block[], DTYPE_INT *dataset,
                 int add)
{
    DTYPE_INT *dataptr = NULL;
    hsize_t    i, j;
...
void
dataset_fill3_2d(hsize_t start[], hsize_t stride[], hsize_t count[], hsize_t block[], DTYPE_INT *dataset,
                 int add, int mode)
{
    DTYPE_INT *dataptr = NULL;
    hsize_t    i, j;
 ...
 */
void
dataset_print(hsize_t start[], hsize_t count[], hsize_t stride[], DTYPE_INT *dataset)
{
    DTYPE_INT *dataptr = dataset;
    hsize_t    i, j;
...
void
dataset_print_1d(hsize_t start[], hsize_t count[], hsize_t stride[], DTYPE_INT *dataset)
{
    DTYPE_INT *dataptr = dataset;
    hsize_t    i;
 ...
 */
void
dset_select_display(hsize_t start[], hsize_t count[], hsize_t stride[], DTYPE_INT *dataset, int dnum)
{
    DTYPE_INT *dataptr = dataset;
    hsize_t    i, j;
 ...
 */
void
dset_display(hsize_t dim1, hsize_t dim2, DTYPE_INT *dataset, int dnum)
{
    DTYPE_INT *dataptr = dataset;
    hsize_t    i, j;
 ...
 */
void
data_array2d_init(hsize_t dim1, hsize_t dim2, DTYPE_INT *dataset, int val)
{
    DTYPE_INT *dataptr = dataset;
    hsize_t    i, j;
...
void
print_dsets_2d(hsize_t dim_col, hsize_t dim_row, DTYPE_INT *wbuf, DTYPE_INT *rbuf)
{
    int        i = 0, j = 0;
    int        nerr = 0;
    DTYPE_INT *dataptr1;
 ...
 */
int
dataset_vrfy(hsize_t start[], hsize_t count[], hsize_t stride[], DTYPE_INT *dataset, DTYPE_INT *original,
             int dnum)
{
...
int
dataset_vrfy2(hsize_t start[], hsize_t stride[], hsize_t count[], hsize_t block[], DTYPE_INT *original,
              DTYPE_INT *dataset, int dnum)
{
#define MAX_ERR_REPORT 10 /* Maximum number of errors reported */
...
int
diff_datasets(hsize_t dim1, hsize_t dim2, DTYPE_INT *original, DTYPE_INT *dataset, int dnum)
{
...
int
diff_dset_points(int pnt_num, hsize_t pcoords[][2], DTYPE_INT *wbuf, DTYPE_INT *rbuf)
{
    int        i = 0, j = 0;
    int        nerr = 0;
    DTYPE_INT *dataptr1;
 ...
 */
void
phdf5Write_mdset_All(char *filename, phdf5_mode_t pmode, mpio_collective_mode_t mpio_opt, sel_mode_t sel_mode,
                     multi_mode_t multi_mode)
{
    int            i;
    int            Count;
    ...
    DTYPE_INT data_array1[SPACE1_DIM1][SPACE1_DIM2] = {{
                                                           0,
                                                       },
                                                       {
                                                           0,
                                                       }}; /* data buffer */
    DTYPE_INT data_array2[SPACE1_DIM1][SPACE1_DIM2] = {{
                                                           0,
                                                       },
                                                       {
                                                           0,
                                                       }}; /* data buffer */
    DTYPE_INT data_array3[SPACE1_DIM1][SPACE1_DIM2] = {{
                                                           0,
                                                       },
                                                       {
                                                           0,
                                                       }}; /* data buffer */
    DTYPE_INT data_array4[SPACE1_DIM1][SPACE1_DIM2] = {{
                                                           0,
                                                       },
                                                       {
                                                           0,
    ...
    if (sel_mode == SEL_HYPER_1BLOCK) {
        ...
        pcoord[0][0] = (SPACE1_DIM1 / mpi_size) * mpi_rank;
        pcoord[0][1] = (SPACE1_DIM2 / mpi_size) * mpi_rank;
        pcoord[1][0] = (SPACE1_DIM1 / mpi_size) * mpi_rank;
        pcoord[1][1] = (SPACE1_DIM2 / mpi_size) * mpi_rank + 1;
    ...
    if (mpi_rank == 0) {
      ...
      // init H5D_rw_multi_t for write DSET1 (CHUNKED)
        ...
        if (mem_dataspace1) {
            ret = H5Sclose(mem_dataspace1);
            assert(ret != FAIL);
        }
        if (file_dataspace1) {
            ret = H5Sclose(file_dataspace1);
            assert(ret != FAIL);
        }
        if (dataset1) {
      ...
      // init H5D_rw_multi_t for write DSET2 (CHUNKED)
        ...
        if (mem_dataspace2) {
            ret = H5Sclose(mem_dataspace2);
            assert(ret != FAIL);
        }
        if (file_dataspace2) {
            ret = H5Sclose(file_dataspace2);
            assert(ret != FAIL);
        }
        if (dataset2) {
      ...
      // init H5D_rw_multi_t for write DSET3 (CONTIG)
        ...
        if (mem_dataspace3) {
            ret = H5Sclose(mem_dataspace3);
            assert(ret != FAIL);
        }
        if (file_dataspace3) {
            ret = H5Sclose(file_dataspace3);
            assert(ret != FAIL);
        }
        if (dataset3) {
      ...
      // init H5D_rw_multi_t for write DSET4 (CONTIG)
        ...
        if (mem_dataspace4) {
            ret = H5Sclose(mem_dataspace4);
            assert(ret != FAIL);
        }
        if (file_dataspace4) {
            ret = H5Sclose(file_dataspace4);
            assert(ret != FAIL);
        }
        if (dataset4) {
    ...
    if (multi_mode == MULTI_DSET) {
        ...
        for (i = 0; i < Count; i++)
            ret = H5Dwrite(dset_info[i].dset_id, dset_info[i].mem_type_id, dset_info[i].mem_space_id,
                           dset_info[i].dset_space_id, xfer_plist, dset_info[i].u.wbuf);
    }
 ...
 */
void
phdf5Read_mdset_All(char *filename, phdf5_mode_t pmode, mpio_collective_mode_t mpio_opt, sel_mode_t sel_mode,
                    multi_mode_t multi_mode)
{
    int            i;
    int            Count;
    ...
    /* Write Buffer */
    DTYPE_INT data_array1_w[SPACE1_DIM1][SPACE1_DIM2] = {{
                                                             0,
                                                         },
                                                         {
                                                             0,
                                                         }}; /* data buffer */
    DTYPE_INT data_array2_w[SPACE1_DIM1][SPACE1_DIM2] = {{
                                                             0,
                                                         },
                                                         {
                                                             0,
                                                         }}; /* data buffer */
    DTYPE_INT data_array3_w[SPACE1_DIM1][SPACE1_DIM2] = {{
                                                             0,
                                                         },
                                                         {
                                                             0,
                                                         }}; /* data buffer */
    DTYPE_INT data_array4_w[SPACE1_DIM1][SPACE1_DIM2] = {{
                                                             0,
                                                         },
                                                         {
                                                             0,
    ...
    /* Read Buffer */
    DTYPE_INT data_array1_r[SPACE1_DIM1][SPACE1_DIM2] = {{
                                                             0,
                                                         },
                                                         {
                                                             0,
                                                         }}; /* data buffer */
    DTYPE_INT data_array2_r[SPACE1_DIM1][SPACE1_DIM2] = {{
                                                             0,
                                                         },
                                                         {
                                                             0,
                                                         }}; /* data buffer */
    DTYPE_INT data_array3_r[SPACE1_DIM1][SPACE1_DIM2] = {{
                                                             0,
                                                         },
                                                         {
                                                             0,
                                                         }}; /* data buffer */
    DTYPE_INT data_array4_r[SPACE1_DIM1][SPACE1_DIM2] = {{
                                                             0,
                                                         },
                                                         {
                                                             0,
    ...
    if (multi_mode == MULTI_DSET) {
        ...
        // Read
        for (i = 0; i < Count; i++) {
            ret = H5Dread(dset_info[i].dset_id, dset_info[i].mem_type_id, dset_info[i].mem_space_id,
                          dset_info[i].dset_space_id, xfer_plist, dset_info[i].u.rbuf) < 0;
        }
    }
    ...
    if (sel_mode == SEL_HYPER_1BLOCK || sel_mode == SEL_POINTS || sel_mode == SEL_1POINT) {
        if (Count >= 1) // relate to TEST_MDSET_NO_LAST_DSET_2ND_PROC
            ret = dataset_vrfy2(start1, stride1, count1, block1, (DTYPE_INT *)dset_info[0].u.wbuf,
                                (DTYPE_INT *)dset_info[0].u.rbuf, 1);
        if (Count >= 2) // relate to TEST_MDSET_NO_LAST_DSET_2ND_PROC
            ret = dataset_vrfy2(start2, stride2, count2, block2, (DTYPE_INT *)dset_info[1].u.wbuf,
                                (DTYPE_INT *)dset_info[1].u.rbuf, 2);
        if (Count >= 3) // relate to TEST_MDSET_NO_LAST_DSET_2ND_PROC
            ret = dataset_vrfy2(start3, stride3, count3, block3, (DTYPE_INT *)dset_info[2].u.wbuf,
                                (DTYPE_INT *)dset_info[2].u.rbuf, 3);
        if (Count >= 4) // relate to TEST_MDSET_NO_LAST_DSET_2ND_PROC
            ret = dataset_vrfy2(start4, stride4, count4, block4, (DTYPE_INT *)dset_info[3].u.wbuf,
                                (DTYPE_INT *)dset_info[3].u.rbuf, 4);
    }
    else if (sel_mode == SEL_NONE) {
        ...
        data_array2d_init(SPACE1_DIM1, SPACE1_DIM2, &data_array4_w[0][0], INIT_VAL);
        if (Count >= 1)
            ret = diff_datasets(SPACE1_DIM1, SPACE1_DIM2, (DTYPE_INT *)dset_info[0].u.wbuf,
                                (DTYPE_INT *)dset_info[0].u.rbuf, 1);
        if (Count >= 2)
            ret = diff_datasets(SPACE1_DIM1, SPACE1_DIM2, (DTYPE_INT *)dset_info[1].u.wbuf,
                                (DTYPE_INT *)dset_info[1].u.rbuf, 2);
        if (Count >= 3)
            ret = diff_datasets(SPACE1_DIM1, SPACE1_DIM2, (DTYPE_INT *)dset_info[2].u.wbuf,
                                (DTYPE_INT *)dset_info[2].u.rbuf, 3);
        if (Count > 4)
            ret = diff_datasets(SPACE1_DIM1, SPACE1_DIM2, (DTYPE_INT *)dset_info[3].u.wbuf,
                                (DTYPE_INT *)dset_info[3].u.rbuf, 4);
    }
...
void
phdf5Write_mdset_Chunk(char *filename, phdf5_mode_t pmode, mpio_collective_mode_t mpio_opt,
                       sel_mode_t sel_mode, multi_mode_t multi_mode)
{
    int            i;
    int            Count = 0;
    hid_t          fid;                                            /* HDF5 file IDs */
 ...
 */
    ...
    if (multi_mode == MULTI_DSET) {
        ...
        for (i = 0; i < Count; i++) {
            ret = H5Dwrite(dset_info[i].dset_id, dset_info[i].mem_type_id, dset_info[i].mem_space_id,
                           dset_info[i].dset_space_id, xfer_plist, dset_info[i].u.wbuf) < 0;
        }
    }
...
/* This test with one or two CHUNKED dset */
void
phdf5Read_mdset_Chunk(char *filename, phdf5_mode_t pmode, mpio_collective_mode_t mpio_opt,
                      sel_mode_t sel_mode, multi_mode_t multi_mode)
{
    int            i;
    int            Count = 0;
...
void
phdf5Write_mdset_Contig(char *filename, phdf5_mode_t pmode, mpio_collective_mode_t mpio_opt,
                        sel_mode_t sel_mode, multi_mode_t multi_mode)
{
    int            i;
    int            Count;
...
/* This test with one or two CONTIG dset */
void
phdf5Read_mdset_Contig(char *filename, phdf5_mode_t pmode, mpio_collective_mode_t mpio_opt,
                       sel_mode_t sel_mode, multi_mode_t multi_mode)
{
    int            i;
    int            Count;
 ...
 */
    ...
    if (sel_mode == SEL_HYPER_1BLOCK || sel_mode == SEL_POINTS || sel_mode == SEL_1POINT) {
        if (Count >= 1) // relate to TEST_MDSET_NO_LAST_DSET_2ND_PROC
            ret = dataset_vrfy2(start3, stride3, count3, block3, (DTYPE_INT *)dset_info[0].u.wbuf,
                                (DTYPE_INT *)dset_info[0].u.rbuf, 1);
        if (Count > 1) // relate to TEST_MDSET_NO_LAST_DSET_2ND_PROC
            ret = dataset_vrfy2(start4, stride4, count4, block4, (DTYPE_INT *)dset_info[1].u.wbuf,
                                (DTYPE_INT *)dset_info[1].u.rbuf, 2);
    }
    else if (sel_mode == SEL_NONE) {
        // init the wbuf, as rbuf shouldn't read any
 ...
 */
void
phdf5Write_mdset_many(char *filename, size_t ndsets, hsize_t dim0, hsize_t chunks, phdf5_mode_t pmode,
                      multi_mode_t multi_mode, mpio_collective_mode_t mpio_opt, sel_mode_t sel_mode)
{
    int             i, j;
    int             Count;
    hid_t           fid;                      /* HDF5 file IDs */
    ...
    if (multi_mode == SINGLE_DSET) {
        ...
        for (j = 0; j < Count; j++) {
            ...
            ret = H5Dwrite(dset_info[j].dset_id, dset_info[j].mem_type_id, dset_info[j].mem_space_id,
                           dset_info[j].dset_space_id, xfer_plist, dset_info[j].u.wbuf);
            assert(ret != FAIL);
            sync();
 ...
 */
void
phdf5Read_mdset_many(char *filename, size_t ndsets, hsize_t dim0, hsize_t chunks, phdf5_mode_t pmode,
                     multi_mode_t multi_mode, mpio_collective_mode_t mpio_opt, sel_mode_t sel_mode)
{
    int             i, j;
    int             Count;
 ...
 */
    ...
    if (multi_mode == SINGLE_DSET) {
        for (j = 0; j < Count; j++) {
            ...
            gettimeofday(&tv1, NULL);
            /* Read Now */
            ret = H5Dread(dset_info[j].dset_id, dset_info[j].mem_type_id, dset_info[j].mem_space_id,
                          dset_info[j].dset_space_id, xfer_plist, dset_info[j].u.rbuf);
            assert(ret != FAIL);
            sync();
            gettimeofday(&tv2, NULL);
            timersub(&tv2, &tv1, &tv_sub);
    ...
    if (doread) {
        if (doCOLL) {
            ...
            if (TEST_TYPE) {
                 ...
                 */
                phdf5Read_mdset_All(testfiles[0], PHDF5_PARALLEL, MPIO_COLLECTIVE_IO, SEL_HYPER_1BLOCK,
                                    MULTI_DSET);
                phdf5Read_mdset_All(testfiles[1], PHDF5_PARALLEL, MPIO_COLLECTIVE_IO, SEL_HYPER_BLOCKS,
                                    MULTI_DSET);
                ...
                // phdf5Read_mdset_Chunk(testfiles[5], PHDF5_PARALLEL, MPIO_COLLECTIVE_IO, SEL_HYPER_1BLOCK,
                // MULTI_DSET); phdf5Read_mdset_Chunk(testfiles[6], PHDF5_PARALLEL, MPIO_COLLECTIVE_IO,
                // SEL_HYPER_BLOCKS, MULTI_DSET); phdf5Read_mdset_Chunk(testfiles[7], PHDF5_PARALLEL,
                // MPIO_COLLECTIVE_IO, SEL_NONE, MULTI_DSET); phdf5Read_mdset_Chunk(testfiles[8],
                // PHDF5_PARALLEL, MPIO_COLLECTIVE_IO, SEL_POINTS, MULTI_DSET);
                // phdf5Read_mdset_Chunk(testfiles[9], PHDF5_PARALLEL, MPIO_COLLECTIVE_IO, SEL_1POINT,
                // MULTI_DSET);

                // phdf5Read_mdset_Contig(testfiles[10], PHDF5_PARALLEL, MPIO_COLLECTIVE_IO, SEL_HYPER_1BLOCK,
                // MULTI_DSET); phdf5Read_mdset_Contig(testfiles[11], PHDF5_PARALLEL, MPIO_COLLECTIVE_IO,
                // SEL_HYPER_BLOCKS, MULTI_DSET); phdf5Read_mdset_Contig(testfiles[12], PHDF5_PARALLEL,
                // MPIO_COLLECTIVE_IO, SEL_NONE, MULTI_DSET); phdf5Read_mdset_Contig(testfiles[13],
                // PHDF5_PARALLEL, MPIO_COLLECTIVE_IO, SEL_POINTS, MULTI_DSET);
                // phdf5Read_mdset_Contig(testfiles[14], PHDF5_PARALLEL, MPIO_COLLECTIVE_IO, SEL_1POINT,
                // MULTI_DSET);
            }
            else {
                /*
                 ...
                 */
                phdf5Read_mdset_many(fname_g, ndset_g, dim0_g, chunks_g, PHDF5_PARALLEL, multi_mode_g,
                                     MPIO_COLLECTIVE_IO, SEL_HYPER_1BLOCK);
                // phdf5Read_mdset_many(fname_g, ndset_g, dim0_g, chunks_g, PHDF5_PARALLEL, multi_mode_g,
                // MPIO_COLLECTIVE_IO, SEL_HYPER_BLOCKS);
            }
        }
        if (doIND) {
            ...
            if (TEST_TYPE) {
                 ...
                 * This section is for multi-dset feature tests
                 */
                phdf5Read_mdset_All(testfiles[0], PHDF5_PARALLEL, MPIO_INDIVIDUAL_IO, SEL_HYPER_1BLOCK,
                                    MULTI_DSET);
                phdf5Read_mdset_All(testfiles[1], PHDF5_PARALLEL, MPIO_INDIVIDUAL_IO, SEL_HYPER_BLOCKS,
                                    MULTI_DSET);
                ...
                // phdf5Read_mdset_All(testfiles[4], PHDF5_PARALLEL, MPIO_INDIVIDUAL_IO, SEL_1POINT,
                // MULTI_DSET);

                // phdf5Read_mdset_Chunk(testfiles[5], PHDF5_PARALLEL, MPIO_INDIVIDUAL_IO, SEL_HYPER_1BLOCK,
                // MULTI_DSET); phdf5Read_mdset_Chunk(testfiles[6], PHDF5_PARALLEL, MPIO_INDIVIDUAL_IO,
                // SEL_HYPER_BLOCKS, MULTI_DSET); phdf5Read_mdset_Chunk(testfiles[7], PHDF5_PARALLEL,
                // MPIO_INDIVIDUAL_IO, SEL_NONE, MULTI_DSET); phdf5Read_mdset_Chunk(testfiles[8],
                // PHDF5_PARALLEL, MPIO_INDIVIDUAL_IO, SEL_POINTS, MULTI_DSET);
                // phdf5Read_mdset_Chunk(testfiles[9], PHDF5_PARALLEL, MPIO_INDIVIDUAL_IO, SEL_1POINT,
                // MULTI_DSET);

                // phdf5Read_mdset_Contig(testfiles[10], PHDF5_PARALLEL, MPIO_INDIVIDUAL_IO, SEL_HYPER_1BLOCK,
                // MULTI_DSET); phdf5Read_mdset_Contig(testfiles[11], PHDF5_PARALLEL, MPIO_INDIVIDUAL_IO,
                // SEL_HYPER_BLOCKS, MULTI_DSET); phdf5Read_mdset_Contig(testfiles[12], PHDF5_PARALLEL,
                // MPIO_INDIVIDUAL_IO, SEL_NONE, MULTI_DSET); phdf5Read_mdset_Contig(testfiles[13],
                // PHDF5_PARALLEL, MPIO_INDIVIDUAL_IO, SEL_POINTS, MULTI_DSET);
                // phdf5Read_mdset_Contig(testfiles[14], PHDF5_PARALLEL, MPIO_INDIVIDUAL_IO, SEL_1POINT,
                // MULTI_DSET);
            }
            else {
                /*
                 ...
                 */
                phdf5Read_mdset_many(fname_g, ndset_g, dim0_g, chunks_g, PHDF5_PARALLEL, multi_mode_g,
                                     MPIO_INDIVIDUAL_IO, SEL_HYPER_1BLOCK);
                // phdf5Read_mdset_many(fname_g, ndset_g, dim0_g, chunks_g, PHDF5_PARALLEL, multi_mode_g,
                // MPIO_INDIVIDUAL_IO, SEL_HYPER_BLOCKS);
            }
        }
        if (doSERIAL) {
            ...
            if (TEST_TYPE) {
                 ...
                 * This section is for multi-dset feature tests
                 */
                phdf5Read_mdset_All(testfiles[0], PHDF5_SERIAL, 0, SEL_HYPER_1BLOCK,
                                    SINGLE_DSET /*MULTI_DSET*/);
                phdf5Read_mdset_All(testfiles[1], PHDF5_SERIAL, 0, SEL_HYPER_BLOCKS,
                                    SINGLE_DSET /*MULTI_DSET*/);
                ...
                // phdf5Read_mdset_Chunk(testfiles[5], PHDF5_SERIAL, 0, SEL_HYPER_1BLOCK, SINGLE_DSET
                // /*MULTI_DSET*/); phdf5Read_mdset_Chunk(testfiles[6], PHDF5_SERIAL, 0, SEL_HYPER_BLOCKS,
                // SINGLE_DSET /*MULTI_DSET*/); phdf5Read_mdset_Chunk(testfiles[7], PHDF5_SERIAL, 0, SEL_NONE,
                // SINGLE_DSET /*MULTI_DSET*/); phdf5Read_mdset_Chunk(testfiles[8], PHDF5_SERIAL, 0,
                // SEL_POINTS, SINGLE_DSET /*MULTI_DSET*/); phdf5Read_mdset_Chunk(testfiles[9], PHDF5_SERIAL,
                // 0, SEL_1POINT, SINGLE_DSET /*MULTI_DSET*/);

                // phdf5Read_mdset_Contig(testfiles[10], PHDF5_SERIAL, 0, SEL_HYPER_1BLOCK, SINGLE_DSET
                // /*MULTI_DSET*/); phdf5Read_mdset_Contig(testfiles[11], PHDF5_SERIAL, 0, SEL_HYPER_BLOCKS,
                // SINGLE_DSET /*MULTI_DSET*/); phdf5Read_mdset_Contig(testfiles[12], PHDF5_SERIAL, 0,
                // SEL_NONE, SINGLE_DSET /*MULTI_DSET*/); phdf5Read_mdset_Contig(testfiles[13], PHDF5_SERIAL,
                // 0, SEL_POINTS, SINGLE_DSET /*MULTI_DSET*/); phdf5Read_mdset_Contig(testfiles[14],
                // PHDF5_SERIAL, 0, SEL_1POINT, SINGLE_DSET /*MULTI_DSET*/);
            }
            else {
                /*
                 * This is for performance test
                 * Generating many dsets with many processes
                 */
                phdf5Read_mdset_many(fname_g, ndset_g, dim0_g, chunks_g, PHDF5_SERIAL, multi_mode_g, 0,
                                     SEL_HYPER_1BLOCK);
                // phdf5Read_mdset_many(fname_g, ndset_g, dim0_g, chunks_g, PHDF5_SERIAL, multi_mode_g, 0,
                // SEL_HYPER_BLOCKS);
            }
    ...
    if (dowrite) {
        if (doCOLL) {
            ...
            if (TEST_TYPE) {
                 ...
                 * This section is for multi-dset feature tests
                 */
                phdf5Write_mdset_All(testfiles[0], PHDF5_PARALLEL, MPIO_COLLECTIVE_IO, SEL_HYPER_1BLOCK,
                                     MULTI_DSET);
                phdf5Write_mdset_All(testfiles[1], PHDF5_PARALLEL, MPIO_COLLECTIVE_IO, SEL_HYPER_BLOCKS,
                                     MULTI_DSET);
                phdf5Write_mdset_All(testfiles[2], PHDF5_PARALLEL, MPIO_COLLECTIVE_IO, SEL_NONE, MULTI_DSET);
                phdf5Write_mdset_All(testfiles[3], PHDF5_PARALLEL, MPIO_COLLECTIVE_IO, SEL_POINTS,
                                     MULTI_DSET);
                phdf5Write_mdset_All(testfiles[4], PHDF5_PARALLEL, MPIO_COLLECTIVE_IO, SEL_1POINT,
                                     MULTI_DSET);

                // phdf5Write_mdset_Chunk(testfiles[5], PHDF5_PARALLEL, MPIO_COLLECTIVE_IO, SEL_HYPER_1BLOCK,
                // MULTI_DSET); phdf5Write_mdset_Chunk(testfiles[6], PHDF5_PARALLEL, MPIO_COLLECTIVE_IO,
                // SEL_HYPER_BLOCKS, MULTI_DSET); phdf5Write_mdset_Chunk(testfiles[7], PHDF5_PARALLEL,
                // MPIO_COLLECTIVE_IO, SEL_NONE, MULTI_DSET); phdf5Write_mdset_Chunk(testfiles[8],
                // PHDF5_PARALLEL, MPIO_COLLECTIVE_IO, SEL_POINTS, MULTI_DSET);
                // phdf5Write_mdset_Chunk(testfiles[9], PHDF5_PARALLEL, MPIO_COLLECTIVE_IO, SEL_1POINT,
                // MULTI_DSET);

                // phdf5Write_mdset_Contig(testfiles[10], PHDF5_PARALLEL, MPIO_COLLECTIVE_IO,
                // SEL_HYPER_1BLOCK, MULTI_DSET); phdf5Write_mdset_Contig(testfiles[11], PHDF5_PARALLEL,
                // MPIO_COLLECTIVE_IO, SEL_HYPER_BLOCKS, MULTI_DSET); phdf5Write_mdset_Contig(testfiles[12],
                // PHDF5_PARALLEL, MPIO_COLLECTIVE_IO, SEL_NONE, MULTI_DSET);
                // phdf5Write_mdset_Contig(testfiles[13], PHDF5_PARALLEL, MPIO_COLLECTIVE_IO, SEL_POINTS,
                // MULTI_DSET); phdf5Write_mdset_Contig(testfiles[14], PHDF5_PARALLEL, MPIO_COLLECTIVE_IO,
                // SEL_1POINT, MULTI_DSET);
            }
            else {
                /*
                 * This is for performance test
                 * Generating many dsets with many processes
                 */
                phdf5Write_mdset_many(fname_g, ndset_g, dim0_g, chunks_g, PHDF5_PARALLEL, multi_mode_g,
                                      MPIO_COLLECTIVE_IO, SEL_HYPER_1BLOCK);
                // phdf5Write_mdset_many(fname_g, ndset_g, dim0_g, chunks_g, PHDF5_PARALLEL, multi_mode_g,
                // MPIO_COLLECTIVE_IO, SEL_HYPER_BLOCKS);
            }
        ...
        if (doIND) {
            ...
            if (TEST_TYPE) {
                 ...
                 * This section is for multi-dset feature tests
                 */
                phdf5Write_mdset_All(testfiles[0], PHDF5_PARALLEL, MPIO_INDIVIDUAL_IO, SEL_HYPER_1BLOCK,
                                     MULTI_DSET);
                phdf5Write_mdset_All(testfiles[1], PHDF5_PARALLEL, MPIO_INDIVIDUAL_IO, SEL_HYPER_BLOCKS,
                                     MULTI_DSET);
                phdf5Write_mdset_All(testfiles[2], PHDF5_PARALLEL, MPIO_INDIVIDUAL_IO, SEL_NONE, MULTI_DSET);
                phdf5Write_mdset_All(testfiles[3], PHDF5_PARALLEL, MPIO_INDIVIDUAL_IO, SEL_POINTS,
                                     MULTI_DSET);
                phdf5Write_mdset_All(testfiles[4], PHDF5_PARALLEL, MPIO_INDIVIDUAL_IO, SEL_1POINT,
                                     MULTI_DSET);

                // phdf5Write_mdset_Chunk(testfiles[5], PHDF5_PARALLEL, MPIO_INDIVIDUAL_IO, SEL_HYPER_1BLOCK,
                // MULTI_DSET); phdf5Write_mdset_Chunk(testfiles[6], PHDF5_PARALLEL, MPIO_INDIVIDUAL_IO,
                // SEL_HYPER_BLOCKS, MULTI_DSET); phdf5Write_mdset_Chunk(testfiles[7], PHDF5_PARALLEL,
                // MPIO_INDIVIDUAL_IO, SEL_NONE, MULTI_DSET); phdf5Write_mdset_Chunk(testfiles[8],
                // PHDF5_PARALLEL, MPIO_INDIVIDUAL_IO, SEL_POINTS, MULTI_DSET);
                // phdf5Write_mdset_Chunk(testfiles[9], PHDF5_PARALLEL, MPIO_INDIVIDUAL_IO, SEL_1POINT,
                // MULTI_DSET);

                // phdf5Write_mdset_Contig(testfiles[10], PHDF5_PARALLEL, MPIO_INDIVIDUAL_IO,
                // SEL_HYPER_1BLOCK, MULTI_DSET); phdf5Write_mdset_Contig(testfiles[11], PHDF5_PARALLEL,
                // MPIO_INDIVIDUAL_IO, SEL_HYPER_BLOCKS, MULTI_DSET); phdf5Write_mdset_Contig(testfiles[12],
                // PHDF5_PARALLEL, MPIO_INDIVIDUAL_IO, SEL_NONE, MULTI_DSET);
                // phdf5Write_mdset_Contig(testfiles[13], PHDF5_PARALLEL, MPIO_INDIVIDUAL_IO, SEL_POINTS,
                // MULTI_DSET); phdf5Write_mdset_Contig(testfiles[14], PHDF5_PARALLEL, MPIO_INDIVIDUAL_IO,
                // SEL_1POINT, MULTI_DSET);
            }
            else {
                /*
        ...
        if (doSERIAL) {
            ...
            if (TEST_TYPE) {
                 ...
                 * Generating many dsets with many processes
                 */
                phdf5Write_mdset_many(fname_g, ndset_g, dim0_g, chunks_g, PHDF5_SERIAL, multi_mode_g, 0,
                                      SEL_HYPER_1BLOCK);
                // phdf5Write_mdset_many(fname_g, ndset_g, dim0_g, chunks_g, PHDF5_SERIAL, multi_mode_g, 0,
                // SEL_HYPER_BLOCKS);
            }
