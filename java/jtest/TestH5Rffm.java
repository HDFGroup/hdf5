...
public class TestH5Rffm {
    ...
    public void createH5file()
    {
        ...
        try (Arena arena = Arena.ofConfined()) {
            ...
            H5dsid = hdf5_h_1.H5Screate_simple(2, dimsSegment, MemorySegment.NULL);
            ...
            H5did = hdf5_h_1.H5Dcreate2(H5fid, dsetname, hdf5_h_1.H5T_STD_I32BE_g(), H5dsid,
            ...
            for (int i = 0; i < DIM_X; i++)
                for (int j = 0; j < DIM_Y; j++)
                    dataBuffer.setAtIndex(java.lang.foreign.ValueLayout.JAVA_INT, i * DIM_Y + j,
                                          data[i][j]);

            int result = hdf5_h_1.H5Dwrite(H5did, hdf5_h_1.H5T_NATIVE_INT_g(), hdf5_h.H5S_ALL(),
                                           hdf5_h.H5S_ALL(), hdf5_h.H5P_DEFAULT(), dataBuffer);
            assertTrue("H5Dwrite failed", isSuccess(result));

            // Flush file
            result = hdf5_h_1.H5Fflush(H5fid, hdf5_h.H5F_SCOPE_LOCAL());
    ...
    public void testH5Rcreate_destroy_object()
    {
        try (Arena arena = Arena.ofConfined()) {
            ...
            // Destroy reference
            result = hdf5_h_1.H5Rdestroy(ref_ptr);
    ...
    public void testH5Ropen_object()
    {
        try (Arena arena = Arena.ofConfined()) {
            ...
            long opened_did = hdf5_h_1.H5Ropen_object(ref_ptr, hdf5_h.H5P_DEFAULT(), hdf5_h.H5P_DEFAULT());
            assertTrue("H5Ropen_object failed", isValidId(opened_did));

            // Verify it's a dataset
            int obj_type = hdf5_h_1.H5Iget_type(opened_did);
            assertEquals("Should be dataset type", hdf5_h.H5I_DATASET(), obj_type);

            // Close opened object
            hdf5_h_1.H5Dclose(opened_did);
    ...
    public void testH5Rget_obj_name()
    {
        try (Arena arena = Arena.ofConfined()) {
            ...
            long actual_size =
                hdf5_h_1.H5Rget_obj_name(ref_ptr, hdf5_h.H5P_DEFAULT(), nameBuffer, name_size + 1);
            assertTrue("H5Rget_obj_name failed", actual_size > 0);

            String retrieved_name = nameBuffer.getString(0);
            assertEquals("Object name should be /dset", "/dset", retrieved_name);
    ...
    public void testH5Rcreate_region()
    {
        try (Arena arena = Arena.ofConfined()) {
            ...
            long region_sid = hdf5_h_1.H5Scopy(H5dsid);
            ...
            int result = hdf5_h_1.H5Sselect_hyperslab(region_sid, hdf5_h.H5S_SELECT_SET(), starts,
            ...
            result = hdf5_h_1.H5Rcreate_region(H5fid, dsetname, region_sid, hdf5_h.H5P_DEFAULT(), ref_ptr);
    ...
    public void testH5Rcreate_open_attr()
    {
        try (Arena arena = Arena.ofConfined()) {
            ...
            long aid =
                hdf5_h_1.H5Acreate2(H5did, attrname, hdf5_h_1.H5T_NATIVE_INT_g(), attr_sid,
                                    hdf5_h.H5P_DEFAULT(), hdf5_h.H5P_DEFAULT());
            ...
            setInt(attrData, 42);
            int result = hdf5_h_1.H5Awrite(aid, hdf5_h_1.H5T_NATIVE_INT_g(), attrData);
            ...
            result =
                hdf5_h_1.H5Rcreate_attr(H5fid, dsetname, attrname, hdf5_h.H5P_DEFAULT(), ref_ptr);
            ...
            long opened_aid = hdf5_h_1.H5Ropen_attr(ref_ptr, hdf5_h.H5P_DEFAULT(), hdf5_h.H5P_DEFAULT());
            assertTrue("H5Ropen_attr failed", isValidId(opened_aid));

            // Read attribute value back
            MemorySegment readData = allocateInt(arena);
            result                 = hdf5_h_1.H5Aread(opened_aid, hdf5_h_1.H5T_NATIVE_INT_g(), readData);
    ...
    public void testH5Rcopy_equal()
    {
        try (Arena arena = Arena.ofConfined()) {
            ...
            int result = hdf5_h_1.H5Rcreate_object(H5fid, dsetname, hdf5_h.H5P_DEFAULT(), ref1);
            assertTrue("H5Rcreate_object failed", isSuccess(result));

            // Copy reference
            MemorySegment ref2 = arena.allocate(hdf5_h.H5R_REF_BUF_SIZE());
            result             = hdf5_h_1.H5Rcopy(ref1, ref2);
            assertTrue("H5Rcopy failed", isSuccess(result));

            // Test equality
            result = hdf5_h_1.H5Requal(ref1, ref2);
            ...
            MemorySegment groupname = stringToSegment(arena, "Group1");
            result = hdf5_h_1.H5Rcreate_object(H5fid, groupname, hdf5_h.H5P_DEFAULT(), ref3);
            assertTrue("H5Rcreate_object Group1 failed", isSuccess(result));

            // Test inequality
            result = hdf5_h_1.H5Requal(ref1, ref3);
            assertEquals("References should not be equal", 0, result);

            // Clean up
            hdf5_h_1.H5Rdestroy(ref1);
            hdf5_h_1.H5Rdestroy(ref2);
            hdf5_h_1.H5Rdestroy(ref3);
        }
    }
    ...
    public void testH5Rget_file_name()
    {
        try (Arena arena = Arena.ofConfined()) {
            ...
            // Get file name size
            long name_size = hdf5_h_1.H5Rget_file_name(ref_ptr, MemorySegment.NULL, 0);
            assertTrue("H5Rget_file_name size query failed", name_size > 0);

            // Get file name
            MemorySegment nameBuffer = arena.allocate(name_size + 1);
            long actual_size         = hdf5_h_1.H5Rget_file_name(ref_ptr, nameBuffer, name_size + 1);
    ...
    public void testH5R_complete_workflow()
    {
        try (Arena arena = Arena.ofConfined()) {
            ...
            MemorySegment dsetname = stringToSegment(arena, "dset");
            int result = hdf5_h_1.H5Rcreate_object(H5fid, dsetname, hdf5_h.H5P_DEFAULT(), obj_ref);
            assertTrue("Create object reference failed", isSuccess(result));

            // 2. Verify type
            int ref_type = hdf5_h_1.H5Rget_type(obj_ref);
            ...
            result                     = hdf5_h_1.H5Rcopy(obj_ref, obj_ref_copy);
            assertTrue("Copy reference failed", isSuccess(result));

            // 4. Verify equality
            result = hdf5_h_1.H5Requal(obj_ref, obj_ref_copy);
            assertTrue("References should be equal", result > 0);

            // 5. Open via reference
            long opened_did = hdf5_h_1.H5Ropen_object(obj_ref, hdf5_h.H5P_DEFAULT(), hdf5_h.H5P_DEFAULT());
            assertTrue("Open object failed", isValidId(opened_did));

            // 6. Get name
            long name_size =
                hdf5_h_1.H5Rget_obj_name(obj_ref, hdf5_h.H5P_DEFAULT(), MemorySegment.NULL, 0);
            assertTrue("Get name size failed", name_size > 0);

            MemorySegment nameBuffer = arena.allocate(name_size + 1);
            hdf5_h_1.H5Rget_obj_name(obj_ref, hdf5_h.H5P_DEFAULT(), nameBuffer, name_size + 1);
            String obj_name = nameBuffer.getString(0);
            assertEquals("Object name should be /dset", "/dset", obj_name);

            // 7. Clean up
            hdf5_h_1.H5Dclose(opened_did);
            hdf5_h_1.H5Rdestroy(obj_ref);
