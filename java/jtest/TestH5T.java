...
public class TestH5T {
    ...
    public void testH5Tget_class()
    {
        try {
            ...
            String class_name = H5Tget_class_name(result);
    ...
    public void testH5Tarray_create()
    {
        ...
        try {
            filetype_id = H5Tarray_create(HDF5Constants.H5T_STD_I64LE, 2, adims);
    ...
    public void testH5Tget_array_dims()
    {
        ...
        try {
            ndims = H5Tget_array_dims(filetype_id, rdims);
    ...
    public void testH5Tenum_functions()
    {
        ...
        try {
            filetype_id = H5Tcreate(H5T_ENUM(), (long)1);
        }
        catch (Throwable err) {
            err.printStackTrace();
        ...
        try {
            ...
            try (Arena arena = Arena.ofConfined()) {
                ...
                status = H5Tenum_insert(filetype_id, name_segment, enum_segment);
            }
            enum_val[0] = 11;
            try (Arena arena = Arena.ofConfined()) {
                // Allocate a MemorySegment to hold the enum bytes
            ...
            assertTrue("Can't get correct index number", H5Tget_member_index(filetype_id, "ORANGE") == 3);

            // Commit enumeration datatype and close it */
            H5Tcommit(H5fid, enum_type, filetype_id, H5P_DEFAULT(), H5P_DEFAULT(), H5P_DEFAULT());

            H5Tclose(filetype_id);

            // Open the dataytpe for query
            filetype_id = H5Topen(H5fid, enum_type, H5P_DEFAULT());
            assertTrue("testH5Tenum_functions:H5Tcreate", filetype_id >= 0);
    ...
    public void testH5Tenum_create_functions()
    {
        ...
        try {
            enum_val[0] = 10;
            H5Tenum_insert(filetype_id, "RED", enum_val);
            enum_val[0] = 11;
            H5Tenum_insert(filetype_id, "GREEN", enum_val);
            enum_val[0] = 12;
            H5Tenum_insert(filetype_id, "BLUE", enum_val);
            enum_val[0] = 13;
            H5Tenum_insert(filetype_id, "ORANGE", enum_val);
            enum_val[0] = 14;
            H5Tenum_insert(filetype_id, "YELLOW", enum_val);
    ...
    public void testH5Topaque_functions()
    {
        ...
        try {
            H5Tset_tag(filetype_id, "opaque type");
            opaque_name = H5Tget_tag(filetype_id);
    ...
    public void testH5Tcompound_functions()
    {
        ...
        try {
            H5Tinsert(filetype_id, "Lon", 0, H5T_NATIVE_DOUBLE_g());
            H5Tinsert(filetype_id, "Lat", 8, H5T_NATIVE_DOUBLE_g());

            // Query member number and member index by member name, for enumeration type.
            assertTrue("Can't get member number", H5Tget_nmembers(filetype_id) == 2);
            assertTrue("Can't get correct index number", H5Tget_member_index(filetype_id, "Lat") == 1);
            ...
            String index_name = H5Tget_member_name(filetype_id, 0);
