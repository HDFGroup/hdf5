...
public class TestH5Lffm {
    ...
    public void createH5file()
    {
        ...
        try (Arena arena = Arena.ofConfined()) {
            ...
            long sid = hdf5_h_1.H5Screate_simple(1, dimsSegment, MemorySegment.NULL);
    ...
    public void testH5Lcreate_hard()
    {
        try (Arena arena = Arena.ofConfined()) {
            ...
            // Verify link exists
            result = hdf5_h_1.H5Lexists(H5fid, link_name, hdf5_h.H5P_DEFAULT());
            assertTrue("Link should exist", result > 0);

            // Open via hard link
            long did = hdf5_h_1.H5Dopen2(H5fid, link_name, hdf5_h.H5P_DEFAULT());
    ...
    public void testH5Lcopy()
    {
        try (Arena arena = Arena.ofConfined()) {
            ...
            int result = hdf5_h_1.H5Lcopy(H5fid, src_name, H5fid, dest_name, hdf5_h.H5P_DEFAULT(),
                                          hdf5_h.H5P_DEFAULT());
            assertTrue("H5Lcopy failed", isSuccess(result));

            // Verify copied link exists
            result = hdf5_h_1.H5Lexists(H5fid, dest_name, hdf5_h.H5P_DEFAULT());
    ...
    public void testH5Lmove()
    {
        try (Arena arena = Arena.ofConfined()) {
            ...
            // Move link
            result = hdf5_h_1.H5Lmove(H5fid, link_name, H5fid, moved_name, hdf5_h.H5P_DEFAULT(),
                                      hdf5_h.H5P_DEFAULT());
            ...
            result = hdf5_h_1.H5Lexists(H5fid, moved_name, hdf5_h.H5P_DEFAULT());
    ...
    public void testH5Lcreate_soft()
    {
        try (Arena arena = Arena.ofConfined()) {
            ...
            int result = hdf5_h_1.H5Lcreate_soft(target_path, H5fid, link_name, hdf5_h.H5P_DEFAULT(),
                                                 hdf5_h.H5P_DEFAULT());
    ...
    public void testH5Lcreate_external()
    {
        try (Arena arena = Arena.ofConfined()) {
            ...
            long ext_fid = hdf5_h_1.H5Fcreate(ext_filename, hdf5_h.H5F_ACC_TRUNC(), hdf5_h.H5P_DEFAULT(),
                                              hdf5_h.H5P_DEFAULT());
            ...
            int result = hdf5_h_1.H5Lcreate_external(ext_filename, obj_path, H5fid, link_name,
                                                     hdf5_h.H5P_DEFAULT(), hdf5_h.H5P_DEFAULT());
    ...
    public void testH5Ldelete()
    {
        try (Arena arena = Arena.ofConfined()) {
            ...
            // Delete link
            result = hdf5_h_1.H5Ldelete(H5fid, link_name, hdf5_h.H5P_DEFAULT());
    ...
    public void testH5Lexists()
    {
        try (Arena arena = Arena.ofConfined()) {
            ...
            result = hdf5_h_1.H5Lexists(H5fid, non_existing_name, hdf5_h.H5P_DEFAULT());
    ...
    public void testH5L_complete_workflow()
    {
        try (Arena arena = Arena.ofConfined()) {
            ...
            result = hdf5_h_1.H5Lcreate_soft(soft_target, H5fid, soft_link, hdf5_h.H5P_DEFAULT(),
                                             hdf5_h.H5P_DEFAULT());
            assertTrue("Create soft link failed", isSuccess(result));

            // 3. Verify both exist
            result = hdf5_h_1.H5Lexists(H5fid, hard_link, hdf5_h.H5P_DEFAULT());
            assertTrue("Hard link should exist", result > 0);

            result = hdf5_h_1.H5Lexists(H5fid, soft_link, hdf5_h.H5P_DEFAULT());
            assertTrue("Soft link should exist", result > 0);

            // 4. Copy hard link
            MemorySegment copied_link = stringToSegment(arena, "CopiedLinkWorkflow");
            result = hdf5_h_1.H5Lcopy(H5fid, hard_link, H5fid, copied_link, hdf5_h.H5P_DEFAULT(),
                                      hdf5_h.H5P_DEFAULT());
            assertTrue("Copy link failed", isSuccess(result));

            // 5. Move soft link
            MemorySegment moved_link = stringToSegment(arena, "MovedSoftLinkWorkflow");
            result = hdf5_h_1.H5Lmove(H5fid, soft_link, H5fid, moved_link, hdf5_h.H5P_DEFAULT(),
                                      hdf5_h.H5P_DEFAULT());
            ...
            result = hdf5_h_1.H5Lexists(H5fid, moved_link, hdf5_h.H5P_DEFAULT());
            assertTrue("Moved soft link should exist", result > 0);

            // 7. Delete copied link
            result = hdf5_h_1.H5Ldelete(H5fid, copied_link, hdf5_h.H5P_DEFAULT());
            assertTrue("Delete link failed", isSuccess(result));

            // 8. Verify deletion
            result = hdf5_h_1.H5Lexists(H5fid, copied_link, hdf5_h.H5P_DEFAULT());
    ...
    public void testH5Lget_info()
    {
        ...
        try (Arena arena = Arena.ofConfined()) {
            ...
            long gid                = hdf5_h_2.H5Gcreate2(H5fid, groupName, hdf5_h.H5P_DEFAULT(),
                                                           hdf5_h.H5P_DEFAULT(), hdf5_h.H5P_DEFAULT());
            assertTrue("H5Gcreate2 failed", isValidId(gid));

            // Create a link
            MemorySegment linkName = stringToSegment(arena, "/test_link_info");
            int result             = hdf5_h_1.H5Lcreate_hard(H5fid, groupName, H5fid, linkName,
                                                              hdf5_h.H5P_DEFAULT(), hdf5_h.H5P_DEFAULT());
            assertTrue("H5Lcreate_hard failed", isSuccess(result));

            // Get link info
            MemorySegment linfo = arena.allocate(56); // H5L_info_t size
            result              = hdf5_h_1.H5Lget_info2(H5fid, linkName, linfo, hdf5_h.H5P_DEFAULT());
    ...
    public void testH5Lget_info_by_idx()
    {
        ...
        try (Arena arena = Arena.ofConfined()) {
            ...
            long gid1               = hdf5_h_2.H5Gcreate2(gid, subgroup1, hdf5_h.H5P_DEFAULT(),
                                                           hdf5_h.H5P_DEFAULT(), hdf5_h.H5P_DEFAULT());
            assertTrue("H5Gcreate2 subgroup1 failed", isValidId(gid1));

            // Get link info by index
            MemorySegment linfo = arena.allocate(56); // H5L_info_t size
            int result          = hdf5_h_1.H5Lget_info_by_idx2(gid, stringToSegment(arena, "."),
                                                                hdf5_h.H5_INDEX_NAME(), hdf5_h.H5_ITER_INC(),
                                                                0, linfo, hdf5_h.H5P_DEFAULT());
    ...
    public void testH5Lget_val()
    {
        ...
        try (Arena arena = Arena.ofConfined()) {
            ...
            int result               = hdf5_h_1.H5Lcreate_soft(targetPath, H5fid, linkName,
                                                                hdf5_h.H5P_DEFAULT(), hdf5_h.H5P_DEFAULT());
            assertTrue("H5Lcreate_soft failed", isSuccess(result));

            // Get link value size first
            MemorySegment size = allocateLongArray(arena, 1);
            result             = hdf5_h_1.H5Lget_val(H5fid, linkName, MemorySegment.NULL, 0, hdf5_h.H5P_DEFAULT());
    ...
    public void testH5Lget_val_by_idx()
    {
        ...
        try (Arena arena = Arena.ofConfined()) {
            ...
            int result               = hdf5_h_1.H5Lcreate_soft(targetPath, gid, linkName,
                                                                hdf5_h.H5P_DEFAULT(), hdf5_h.H5P_DEFAULT());
            assertTrue("H5Lcreate_soft failed", isSuccess(result));

            // Get link value by index
            result = hdf5_h_1.H5Lget_val_by_idx(gid, stringToSegment(arena, "."), hdf5_h.H5_INDEX_NAME(),
    ...
    public void testH5Lget_name_by_idx_multiple()
    {
        ...
        try (Arena arena = Arena.ofConfined()) {
            ...
            for (int i = 0; i < 3; i++) {
                ...
                long subGid           = hdf5_h_2.H5Gcreate2(gid, subName, hdf5_h.H5P_DEFAULT(),
                                                             hdf5_h.H5P_DEFAULT(), hdf5_h.H5P_DEFAULT());
            ...
            long nameSize  = hdf5_h_1.H5Lget_name_by_idx(gid, stringToSegment(arena, "."),
            ...
            long actualSize       = hdf5_h_1.H5Lget_name_by_idx(gid, stringToSegment(arena, "."),
                                                                 hdf5_h.H5_INDEX_NAME(), hdf5_h.H5_ITER_INC(),
                                                                 0, nameBuf, nameSize + 1, hdf5_h.H5P_DEFAULT());
