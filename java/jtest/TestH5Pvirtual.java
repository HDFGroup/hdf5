...
public class TestH5Pvirtual {
    ...
    long H5dssid                              = H5I_INVALID_HID();
    long H5dvsid                              = H5I_INVALID_HID();
    long H5did                                = H5I_INVALID_HID();
    long H5dcplid                             = H5I_INVALID_HID();
    long H5dapl_id                            = H5I_INVALID_HID();
    ...
    private final long _createDataset(long fid, long dsid, String name, long dcpl, long dapl)
    {
        ...
        try {
            try (Arena arena = Arena.ofConfined()) {
                ...
                H5dssid                      = H5Screate_simple(1, H5dims_segment, null);
            }
            catch (Throwable err) {
                err.printStackTrace();
                fail("Arena: " + err);
            ...
            for (int i = 0; i < 3; i++) {
                ...
                H5Sselect_hyperslab(dsid, HDF5Constants.H5S_SELECT_SET, start, stride, count, block);
                H5Pset_virtual(dcpl, dsid, SRC_FILE[i], SRC_DATASET[i], H5dssid);
            }
            try (Arena arena = Arena.ofConfined()) {
                // Allocate a MemorySegment to hold the string bytes
                MemorySegment name_segment = arena.allocateFrom(name);
                did = H5Dcreate2(fid, name_segment, H5T_NATIVE_INT_g(), dsid, H5P_DEFAULT(), dcpl, dapl);
            }
            catch (Throwable err) {
                err.printStackTrace();
                fail("Arena: " + err);
    ...
    private final void _createH5File(long fcpl, long fapl)
    {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            try {
                try (Arena arena = Arena.ofConfined()) {
                    // Allocate a MemorySegment to hold the string bytes
                    MemorySegment filename_segment = arena.allocateFrom(SRC_FILE[i]);
                ...
                try (Arena arena = Arena.ofConfined()) {
                    // Allocate a MemorySegment to hold the dims bytes
                    MemorySegment H5dims_segment = MemorySegment.ofArray(H5dims);
                    space_id                     = H5Screate_simple(1, H5dims_segment, null);
                }
                catch (Throwable err) {
                    err.printStackTrace();
                    fail("Arena: " + err);
                ...
                try (Arena arena = Arena.ofConfined()) {
                    // Allocate a MemorySegment to hold the string bytes
                    MemorySegment name_segment = arena.allocateFrom(SRC_DATASET[i]);
                    dset_id = H5Dcreate2(file_id, name_segment, H5T_NATIVE_INT_g(), space_id, H5P_DEFAULT(),
                ...
                H5Dwrite(dset_id, H5T_NATIVE_INT_g(), H5S_ALL(), H5S_ALL(), H5P_DEFAULT(), dset_data);
            }
            catch (Throwable err) {
                err.printStackTrace();
        ...
        try {
            ...
            try (Arena arena = Arena.ofConfined()) {
                // Allocate a MemorySegment to hold the dims bytes
                MemorySegment VDSH5dims_segment = MemorySegment.ofArray(VDSH5dims);
                H5dsid                          = H5Screate_simple(2, VDSH5dims_segment, null);
            }
            catch (Throwable err) {
                err.printStackTrace();
                fail("Arena: " + err);
            }
            H5dcplid = H5Pcreate(H5P_CLS_DATASET_CREATE_ID_g());
            H5Pset_fill_value(H5dcplid, H5T_NATIVE_INT_g(), fill_value);
        }
        catch (Throwable err) {
            err.printStackTrace();
            fail("TestH5Pvirtual.createH5file: " + err);
    ...
    public void testH5Pget_source_filename() throws Throwable
    {
        ...
        H5did = _createDataset(H5fid, H5dsid, "VDS", H5dcplid, H5dapl_id);
        try {
            filename = (H5Pget_virtual_filename(H5dcplid, 2));
        }
        catch (Throwable err) {
            err.printStackTrace();
    ...
    public void testH5Pget_source_datasetname() throws Throwable
    {
        ...
        H5did = _createDataset(H5fid, H5dsid, "VDS", H5dcplid, H5dapl_id);
        try {
            datasetname = H5Pget_virtual_dsetname(H5dcplid, 1);
        }
        catch (Throwable err) {
            err.printStackTrace();
    ...
    {
        long src_space     = H5I_INVALID_HID();
        ...
        try {
            ...
            for (int i = 0; i < num_map; i++) {
                ...
                try {
                    ...
                    H5Sget_regular_hyperslab(vspace, q_start, q_stride, q_count, q_block);
    ...
    public void testH5Pset_virtual_prefix()
    {
        String prefix = "tmp";
        try {
            H5Pset_virtual_prefix(H5dapl_id, prefix);
        }
        catch (Throwable err) {
            err.printStackTrace();
    ...
    {
        H5Pset_virtual_prefix(H5dapl_id, null);
    }
    ...
    public void testH5Pget_virtual_prefix()
    {
        ...
        try {
            H5Pset_virtual_prefix(H5dapl_id, prefix);
            pre = H5Pget_virtual_prefix(H5dapl_id);
        }
        catch (Throwable err) {
            err.printStackTrace();
