...
public class TestH5Pfapl {
    ...
    long H5Fdsid                               = H5I_INVALID_HID();
    long H5Fdid                                = H5I_INVALID_HID();
    ...
    public void testH5Pget_libver_bounds()
    {
        ...
        try {
            ret_val = H5Pget_libver_bounds(fapl_id, libver);
        }
        catch (Throwable err) {
            err.printStackTrace();
        ...
        assertEquals(H5F_LIBVER_EARLIEST(), libver[0]);
        // Check the Latest Version if the library
        assertEquals(H5F_LIBVER_LATEST(), libver[1]);
    }
    ...
    public void testH5Pset_libver_bounds()
    {
        ...
        int ret_val  = -1;
        int low      = H5F_LIBVER_EARLIEST();
        int high     = H5F_LIBVER_LATEST();
        int[] libver = new int[2];

        try {
            ret_val = H5Pset_libver_bounds(fapl_id, low, high);
            H5Pget_libver_bounds(fapl_id, libver);
        }
        catch (Throwable err) {
            err.printStackTrace();
    ...
    public void testH5P_btree_ratios()
    {
        ...
        double[] right  = {0.7};
        try {
            H5Pset_btree_ratios(plist_id, left[0], middle[0], right[0]);
    ...
    public void testH5P_alignment()
    {
        long[] align = {0, 0};
        try {
            H5Pget_alignment(fapl_id, align);
        ...
        try {
            align[0] = 1024;
            align[1] = 2048;
            H5Pset_alignment(fapl_id, align[0], align[1]);
            H5Pget_alignment(fapl_id, align);
    ...
    public void testH5P_hyper_vector_size()
    {
        long[] align = {0};
        try {
            H5Pget_hyper_vector_size(plist_id, align);
        ...
        try {
            align[0] = 4096;
            H5Pset_hyper_vector_size(plist_id, align[0]);
            H5Pget_hyper_vector_size(plist_id, align);
    ...
    public void testH5P_cache()
    {
        ...
        double[] rdcc_w0   = {0};
        try {
            H5Pget_cache(fapl_id, null, rdcc_nelmts, rdcc_nbytes, rdcc_w0);
        ...
        try {
            rdcc_nelmts[0] = 4096;
            H5Pset_cache(fapl_id, 0, rdcc_nelmts[0], rdcc_nbytes[0], rdcc_w0[0]);
            H5Pget_cache(fapl_id, null, rdcc_nelmts, rdcc_nbytes, rdcc_w0);
    ...
    public void testH5P_chunk_cache()
    {
        ...
        try {
            rdcc_nslots[0] = 4096;
            H5Pset_chunk_cache(dapl_id, rdcc_nslots[0], rdcc_nbytes[0], rdcc_w0[0]);
            H5Pget_chunk_cache(dapl_id, rdcc_nslots, rdcc_nbytes, rdcc_w0);
    ...
    public void testH5Pget_mdc_config()
    {
        ...
        try {
            cache_config = H5Pget_mdc_config(fapl_id);
            assertTrue("H5Pget_mdc_config", cache_config.version == H5AC__CURR_CACHE_CONFIG_VERSION());
        }
        catch (Throwable err) {
            err.printStackTrace();
    ...
    public void testH5Pset_mdc_config()
    {
        ...
        try {
            cache_config = H5Pget_mdc_config(fapl_id);
            assertTrue("H5Pset_mdc_config", cache_config.version == H5AC__CURR_CACHE_CONFIG_VERSION());
        }
        catch (Throwable err) {
            err.printStackTrace();
        ...
        try {
            ...
            H5Pset_mdc_config(fapl_id, cache_config);
            cache_config = H5Pget_mdc_config(fapl_id);
            assertTrue("H5Pset_mdc_config", cache_config.version == H5AC__CURR_CACHE_CONFIG_VERSION());
            assertTrue("H5Pset_mdc_config", cache_config.decr_mode == H5C__decr_off());
        }
        catch (Throwable err) {
            err.printStackTrace();
            fail("H5Pset_mdc_config: " + err);
    ...
    public void testH5P_fapl_core()
    {
        ...
        try {
            ...
            assertTrue("H5Pget_driver: core = " + driver_type, H5FD_CORE_id_g() == driver_type);
        }
        catch (Throwable err) {
            err.printStackTrace();
        ...
        try {
            ...
            H5Pget_fapl_core(fapl_id, increment, backingstore);
    ...
    {
        if (H5FD_FAMILY_id_g() < 0)
            return;
        try {
            H5Pset_fapl_family(fapl_id, 1024, H5P_DEFAULT());
            long driver_type = H5Pget_driver(fapl_id);
            assertTrue("H5Pget_driver: family = " + driver_type, H5FD_FAMILY_id_g() == driver_type);
        }
        catch (Throwable err) {
            err.printStackTrace();
    ...
    public void testH5Pset_fapl_sec2()
    {
        ...
        try {
            ...
            long driver_type = H5Pget_driver(fapl_id);
            assertTrue("H5Pget_driver: sec2 = " + driver_type, H5FD_SEC2_id_g() == driver_type);
        }
        catch (Throwable err) {
            err.printStackTrace();
    ...
    {
        if (H5FD_STDIO_id_g() < 0)
            return;
        try {
            H5Pset_fapl_stdio(fapl_id);
            long driver_type = H5Pget_driver(fapl_id);
            assertTrue("H5Pget_driver: stdio = " + driver_type, H5FD_STDIO_id_g() == driver_type);
        }
        catch (Throwable err) {
            err.printStackTrace();
    ...
    public void testH5Pset_fapl_log()
    {
        ...
        try {
            ...
            long driver_type = H5Pget_driver(fapl_id);
            assertTrue("H5Pget_driver: log = " + driver_type, H5FD_LOG_id_g() == driver_type);
        }
        catch (Throwable err) {
            err.printStackTrace();
    ...
    public void testH5P_fapl_muti_nulls()
    {
        ...
        try {
            H5Pset_fapl_multi(fapl_id, member_map, member_fapl, member_name, member_addr, true);
            long driver_type = H5Pget_driver(fapl_id);
            assertTrue("H5Pget_driver: multi = " + driver_type, H5FD_MULTI_id_g() == driver_type);
        }
        catch (Throwable err) {
            err.printStackTrace();
        ...
        try {
            boolean relax = H5Pget_fapl_multi(fapl_id, member_map, member_fapl, member_name, member_addr);
    ...
    public void testH5P_fapl_split()
    {
        ...
        try {
            H5Pset_fapl_split(fapl_id, "-meta.h5", H5P_DEFAULT(), "-raw.h5", H5P_DEFAULT());
            long driver_type = H5Pget_driver(fapl_id);
            assertTrue("H5Pget_driver: split = " + driver_type, H5FD_MULTI_id_g() == driver_type);
        }
        catch (Throwable err) {
            err.printStackTrace();
    ...
    {
        if (H5FD_DIRECT_id_g() < 0)
            return;
        try {
            H5Pset_fapl_direct(fapl_id, 1024, 4096, 8 * 4096);
            long driver_type = H5Pget_driver(fapl_id);
            assertTrue("H5Pget_driver: direct = " + driver_type, H5FD_DIRECT_id_g() == driver_type);
        }
        catch (Throwable err) {
            err.printStackTrace();
    ...
    public void testH5Pset_fapl_windows()
    {
        ...
        try {
            ...
            long driver_type = H5Pget_driver(fapl_id);
            assertTrue("H5Pget_driver: windows = " + driver_type, H5FD_SEC2_id_g() == driver_type);
        }
        catch (Throwable err) {
            err.printStackTrace();
    ...
    public void testH5Pmulti_transform()
    {
        ...
        try {
            H5Pset_data_transform(plist_id, f_to_c);
        ...
        try {
            H5Dwrite(H5Fdid, H5T_NATIVE_DOUBLE_g(), H5S_ALL(), H5S_ALL(), plist_id, windchillF);
            H5Dread(H5Fdid, H5T_NATIVE_DOUBLE_g(), H5S_ALL(), H5S_ALL(), H5P_DEFAULT(), windchillFread);
        }
        catch (Throwable err) {
            err.printStackTrace();
            fail("H5Pdata_transform: " + err);
    ...
    public void testH5Fmdc_logging()
    {
        ...
        try {
            ...
            H5Pset_mdc_log_options(fapl_id, is_enabled, H5_LOG_FILE, start_on_access);

            String location = H5Pget_mdc_log_options(fapl_id, mdc_log_options);
        ...
        try {
            H5Fget_mdc_logging_status(H5fid, mdc_logging_status);
        }
        catch (Throwable err) {
