...
public class TestH5Rref {
    ...
    public void testH5Rget_object()
    {
        ...
        try {
            H5Dread(H5did, HDF5Constants.H5T_STD_REF, H5S_ALL(), H5S_ALL(), H5P_DEFAULT(), refbuf);
        }
        catch (Exception err) {
            err.printStackTrace();
    ...
    {
        long f_type     = H5I_INVALID_HID();
        long loc_id     = H5I_INVALID_HID();
        long loc_sid    = H5I_INVALID_HID();
        int ref_type    = HDF5Constants.H5R_DATASET_REGION2;
        int ndims       = 1;
        long ret_val    = -1;
        byte[][] refbuf = null;
                        ...
                        try {
                            ...
                            if (region_type == HDF5Constants.H5S_SEL_POINTS) {
                                ...
                                long coord[][] = {{6, 9}, {2, 2}, {8, 4}, {1, 6}, {2, 8},
                                                  {3, 2}, {0, 4}, {9, 0}, {7, 1}, {3, 3}};
                                try {
                                    H5Sget_select_elem_pointlist(loc_sid, 0, reg_npoints, getcoord);
                                ...
                                long block[]     = {8, 8};
                                try {
                                    H5Sget_select_hyper_blocklist(loc_sid, 0, reg_nblocks, getblocks);
        ...
        for (int i = 0; i < ndims; i++) {
            try {
                ...
                assertTrue("testH5Rget_region_attribute: H5Rget_type[" + i + "]=" + ret_val,
                           ret_val == ref_type);
                try {
                    loc_id = H5Ropen_attr(refbuf[i], H5P_DEFAULT(), H5P_DEFAULT());
                    assertTrue(loc_id >= 0);
                    if (!byteArrayCheck(refbuf[i])) {
                        try {
