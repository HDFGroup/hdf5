...
public class TestH5Effm {
    ...
    public void setupErrorClass()
    {
        ...
        try (Arena arena = Arena.ofConfined()) {
            ...
            hdf_java_classid = hdf5_h_1.H5Eregister_class(cls_name, lib_name, version);
            assertTrue("H5Eregister_class failed", isValidId(hdf_java_classid));

            // Get current error stack
            current_stackid = hdf5_h_1.H5Eget_current_stack();
    ...
    public void cleanup()
    {
        if (isValidId(hdf_java_classid)) {
            int result = hdf5_h_1.H5Eunregister_class(hdf_java_classid);
        ...
        if (isValidId(current_stackid)) {
            closeQuietly(current_stackid, hdf5_h_1::H5Eclose_stack);
    ...
    public void testH5Eregister_unregister_class()
    {
        try (Arena arena = Arena.ofConfined()) {
            ...
            // Get class name back
            long name_size = hdf5_h_1.H5Eget_class_name(class_id, MemorySegment.NULL, 0);
            ...
            // Unregister class
            int result = hdf5_h_1.H5Eunregister_class(class_id);
            assertTrue("H5Eunregister_class failed", isSuccess(result));
        }
    }
    ...
    public void testH5Ecreate_close_msg()
    {
        try (Arena arena = Arena.ofConfined()) {
            ...
            // Close message
            int result = hdf5_h_1.H5Eclose_msg(maj_err_id);
            ...
            // Close minor message
            result = hdf5_h_1.H5Eclose_msg(min_err_id);
    ...
    public void testH5Eget_major_minor()
    {
        try (Arena arena = Arena.ofConfined()) {
            ...
            long file_id = hdf5_h_1.H5Fopen(filename, hdf5_h.H5F_ACC_RDONLY(), hdf5_h.H5P_DEFAULT());
            ...
            if (isValidId(stack_id)) {
                long num_errors = hdf5_h_1.H5Eget_num(stack_id);
                ...
                hdf5_h_1.H5Eclose_stack(stack_id);
            }
        }
    }
    ...
    public void testH5Ecreate_close_stack()
    {
        ...
        long stack_id = hdf5_h_1.H5Ecreate_stack();
    ...
    public void testH5Eget_current_set_stack()
    {
        ...
        result = hdf5_h_1.H5Eset_current_stack(stack1);
    ...
    public void testH5Eget_num_pop()
    {
        ...
        // Pop one error
        int result = hdf5_h_1.H5Epop(stack_id, 1);
        assertTrue("H5Epop failed", isSuccess(result));

        // Verify count decreased
        num_errors = hdf5_h_1.H5Eget_num(stack_id);
    ...
    public void testH5Eappend_stack()
    {
        try (Arena arena = Arena.ofConfined()) {
            ...
            long file_id1 = hdf5_h_1.H5Fopen(filename1, hdf5_h.H5F_ACC_RDONLY(), hdf5_h.H5P_DEFAULT());
            if (isValidId(file_id1))
                hdf5_h_1.H5Fclose(file_id1);

            long stack1 = hdf5_h_1.H5Eget_current_stack();
            assertTrue("H5Eget_current_stack stack1 failed", isValidId(stack1));

            long num1 = hdf5_h_1.H5Eget_num(stack1);

            // Stack 2: trigger another error
            MemorySegment filename2 = stringToSegment(arena, "nonexistent2.h5");
            long file_id2 = hdf5_h_1.H5Fopen(filename2, hdf5_h.H5F_ACC_RDONLY(), hdf5_h.H5P_DEFAULT());
            if (isValidId(file_id2))
                hdf5_h_1.H5Fclose(file_id2);

            long stack2 = hdf5_h_1.H5Eget_current_stack();
            assertTrue("H5Eget_current_stack stack2 failed", isValidId(stack2));

            long num2 = hdf5_h_1.H5Eget_num(stack2);

            // Append stack2 to stack1 (close_source = false)
            int result = hdf5_h_1.H5Eappend_stack(stack1, stack2, false);
            assertTrue("H5Eappend_stack failed", isSuccess(result));

            // Verify stack1 now has combined errors
            long combined_num = hdf5_h_1.H5Eget_num(stack1);
            ...
            hdf5_h_1.H5Eclose_stack(stack1);
            hdf5_h_1.H5Eclose_stack(stack2);
        }
    }
    ...
    public void testH5Epause_resume_stack()
    {
        try (Arena arena = Arena.ofConfined()) {
            ...
            // Pause the stack
            result = hdf5_h_1.H5Epause_stack(stack_id);
            assertTrue("H5Epause_stack failed", isSuccess(result));

            // Verify it's paused
            result = hdf5_h_1.H5Eis_paused(stack_id, isPausedPtr);
            ...
            // Resume the stack
            result = hdf5_h_1.H5Eresume_stack(stack_id);
    ...
    public void testH5E_complete_workflow()
    {
        try (Arena arena = Arena.ofConfined()) {
            ...
            long num = hdf5_h_1.H5Eget_num(stack_id);
            ...
            hdf5_h_1.H5Eclose_stack(stack_id);
            hdf5_h_1.H5Eclose_msg(min_id);
            hdf5_h_1.H5Eclose_msg(maj_id);
