...
public class TestH5Ocopy {
    ...
    public void createH5file() throws NullPointerException, HDF5Exception
    {
        ...
        try {
            ...
            H5dsid2 = H5Screate(HDF5Constants.H5S_SCALAR);
    ...
    public void testH5OcopyRefsAttr()
    {
        ...
        byte[][] dset_data = new byte[2][H5R_REF_BUF_SIZE()];
        long attribute_id  = H5I_INVALID_HID();

        try {
            try {
                dset_data[0] = H5Rcreate_object(H5fid, "/G1", H5P_DEFAULT());
            }
            catch (Throwable err) {
                err.printStackTrace();
            ...
            try {
                dset_data[1] = H5Rcreate_object(H5fid, "DS2", H5P_DEFAULT());
            }
            catch (Throwable err) {
                err.printStackTrace();
                fail("testH5OcopyRefsAttr: H5Rcreate_object " + err);
            ...
            try {
                attribute_id = H5Acreate(H5did2, "A1", H5T_STD_REF(), H5dsid, H5P_DEFAULT(), H5P_DEFAULT());
                assertTrue("testH5OcopyRefsAttr.H5Acreate: ", attribute_id >= 0);
                H5Awrite(attribute_id, H5T_STD_REF(), dset_data);
    ...
    public void testH5OcopyRefsDatasettodiffFile()
    {
        ...
        try {
            ...
            try {
                ...
                H5Dwrite(dataset_id, H5T_STD_REF(), H5S_ALL(), H5S_ALL(), H5P_DEFAULT(), dset_data);
                H5Dclose(dataset_id);
            }
            catch (Exception ex) {
        ...
        try {
            ...
            try (Arena arena = Arena.ofConfined()) {
                // Allocate a MemorySegment to hold the string bytes
                MemorySegment filename_segment = arena.allocateFrom("copy.h5");
    ...
    {
        byte[][] dset_data = new byte[2][H5R_REF_BUF_SIZE()];
        byte[][] read_data = new byte[2][H5R_REF_BUF_SIZE()];
        long ocp_plist_id  = H5I_INVALID_HID();
        long dataset_id    = H5I_INVALID_HID();
        long did           = H5I_INVALID_HID();
        int obj_type       = -1;
        ...
        try {
            ...
            try {
                did = H5Dopen(H5fid, "DSREF", H5P_DEFAULT());
            ...
            try {
                H5Dread(did, H5T_STD_REF(), H5S_ALL(), H5S_ALL(), H5P_DEFAULT(), read_data);
            }
            catch (Exception e) {
                e.printStackTrace();
        ...
        try {
            ...
            obj_type = H5Rget_obj_type3(read_data[1], H5R_OBJECT());
            assertEquals(obj_type, H5O_TYPE_DATASET());

            obj_type = H5Rget_obj_type3(read_data[0], H5R_OBJECT());
            ...
            try {
                H5Rdestroy(read_data[1]);
            }
            catch (Exception ex) {
            }
            try {
                H5Rdestroy(read_data[0]);
            }
            catch (Exception ex) {
            }
        }
    ...
    public void testH5OcopyNullRef() throws Throwable
    {
        ...
        try {
            ...
            try (Arena arena = Arena.ofConfined()) {
                ...
                did = H5Dcreate2(H5fid, name_segment, H5T_NATIVE_INT_g(), sid, sid, _pid_, _pid_, _pid_);
            }
            catch (Throwable err) {
                err.printStackTrace();
                fail("Arena: " + err);
            ...
            aid = H5Acreate(did, "Null_Ref", H5T_STD_REF(), sid, _pid_, _pid_);
