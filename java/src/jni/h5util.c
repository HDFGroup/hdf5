                                ...
                                default: {
                                    ...
                                    /* Print OID */
                                    {
                                        ...
                                        if (NULL == (this_str = (char *)HDmalloc(this_len)))
                                            H5_OUT_OF_MEMORY_ERROR(
                                                ENVONLY, "h5str_sprintf: failed to allocate string buffer");
                                        if (HDsnprintf(this_str, this_len, "%lu:%s", oi.fileno, token_str) < 0)
                                            H5_JNI_FATAL_ERROR(ENVONLY, "h5str_sprintf: HDsnprintf failure");
    ...
    if (expand_data) {
        ...
        for (i = 0; i < nblocks; i++) {
            ...
            for (j = 0; j < ndims; j++) {
                ...
                if (HDsnprintf(tmp_str, sizeof(tmp_str), "%s%lu", j ? "," : "(", (unsigned long)ptdata[i * 2 * ndims + j]) < 0)
    ...
    if (expand_data) {
        ...
        for (i = 0; i < npoints; i++) {
            ...
            for (j = 0; j < ndims; j++) {
                ...
                if (HDsnprintf(tmp_str, sizeof(tmp_str), "%s%lu", j ? "," : "(", (unsigned long)(ptdata[i * ndims + j])) < 0)
