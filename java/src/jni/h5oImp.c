...
Java_hdf_hdf5lib_H5_H5Oget_1info(JNIEnv *env, jclass clss, jlong loc_id, jint fields)
{
    ...
    CALL_CONSTRUCTOR(ENVONLY, "hdf/hdf5lib/structs/H5O_info_t",
                     "(JJIIJJJJJLhdf/hdf5lib/structs/H5O_hdr_info_t;Lhdf/hdf5lib/structs/H5_ih_info_t;Lhdf/"
                     "hdf5lib/structs/H5_ih_info_t;)V",
                     args, ret_obj);
                                           ...
                                           jlong access_id)
{
    ...
    if ((status =
             H5Oget_info_by_name2((hid_t)loc_id, objName, &infobuf, (unsigned)fields, (hid_t)access_id)) < 0)
        H5_LIBRARY_ERROR(ENVONLY);
                                          ...
                                          jlong access_id)
{
    ...
    if ((status = H5Oget_info_by_idx2((hid_t)loc_id, grpName, (H5_index_t)index_field, (H5_iter_order_t)order,
...
static herr_t
H5O_iterate_cb(hid_t g_id, const char *name, const H5O_info_t *info, void *cb_data)
{
    ...
    if (NULL ==
        (mid = CBENVPTR->GetMethodID(
             ...
             "(JLjava/lang/String;Lhdf/hdf5lib/structs/H5O_info_t;Lhdf/hdf5lib/callbacks/H5O_iterate_t;)I")))
        CHECK_JNI_EXCEPTION(CBENVONLY, JNI_FALSE);
                             ...
                             jobject callback_op, jobject op_data, jint fields)
{
    ...
    if ((status = H5Ovisit2((hid_t)grp_id, (H5_index_t)idx_type, (H5_iter_order_t)order,
                            (H5O_iterate_t)H5O_iterate_cb, (void *)&wrapper, (unsigned)fields)) < 0)
        H5_LIBRARY_ERROR(ENVONLY);
                                       ...
                                       jlong access_id)
{
    ...
    if ((status = H5Ovisit_by_name2((hid_t)grp_id, objName, (H5_index_t)idx_type, (H5_iter_order_t)order,
                                    (H5O_iterate_t)H5O_iterate_cb, (void *)&wrapper, (unsigned)fields,
                                    (hid_t)access_id)) < 0)
        H5_LIBRARY_ERROR(ENVONLY);
