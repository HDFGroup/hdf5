...
main(int argc, char *argv[])
{
    hsize_t             dsetDims[2];
    hid_t               fcpl;
    H5AC_cache_config_t mdc_config = {.version = H5AC__CURR_CACHE_CONFIG_VERSION};
    ...
    std::cout << "original MDC min size " << mdc_config.min_size << " max size " << mdc_config.max_size
              << std::endl;
    ...
    std::cout << "new MDC min size " << mdc_config.min_size << " max size " << mdc_config.max_size
              << std::endl;
    ...
    for (size_t cnt = 0; cnt < 3; cnt++) {
        ...
        for (size_t cntDsets = 0; cntDsets < dsetsAndNumElemsMap[cnt][0]; cntDsets++) {
            ...
            hid_t dsetID = H5Dcreate(groupID, dsetName.c_str(), H5T_NATIVE_UINT8, spaceID, H5P_DEFAULT,
