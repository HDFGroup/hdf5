...
test_rw_noupdate(void)
{
    ...
    if (diff > 0.0F) {
        ...
        MESSAGE(
            1,
            ("Testing to verify that nothing is written if nothing is changed: This test is skipped on this "
             "system because the modification time from stat is the same as the last access time.\n"));
    } /* end if */
    else {
