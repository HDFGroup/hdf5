 ...
 *
 * Purpose:     Create a golden regression file for the RFC-HDFG-2026-001
 *              filter pipeline "Version 3" object header message: a
 *              built-in filter (scaleoffset) configured through the
 *              string-based API, so the pipeline message stores a
 *              canonicalized TOML configuration string alongside the raw
 *              cd_values.  Uses only a built-in filter -- unlike the test
 *              suite's private test filters -- so this file remains
 *              readable by any unmodified HDF5 3.0+ build, or by an
 *              independent reader implementing only the format spec
 *              (docs/doxygen/dox/H5.format.4.0.dox, "Filter Pipeline
 *              Message - Version 3"), with no plugin registration needed.
 *
 *              This file is generated once and checked in; it must NOT be
 *              regenerated by routine test runs, or it stops catching
 *              encode/decode regressions against a fixed historical byte
 *              layout.  To regenerate deliberately (e.g. after an
 *              intentional, version-bumped format change), rebuild and
 *              rerun this generator, then copy TESTFILE3 over
 *              test/testfiles/test_filters_v3.h5.
 *
 * Return:      Success:        0
 *
 *              Failure:        -1
...
create_file_with_pline_v3(void)
{
    ...
    if ((fid = H5Fcreate(TESTFILE3, H5F_ACC_TRUNC, H5P_DEFAULT, fapl)) < 0)
        goto error;
    ...
    /* A quoted-string value (scale_type) alongside an integer value
     * (scale_factor), exercising both TOML value kinds this format
     * message is meant to persist verbatim. */
