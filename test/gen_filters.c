 ...
 *
 * Purpose:     Create a golden regression file for the filter pipeline
 *              message version 3: a built-in filter (scaleoffset)
 *              configured through the string-based API, so the pipeline
 *              message stores a canonicalized TOML configuration string
 *              alongside the raw cd_values.  Uses only a built-in filter -- unlike the test
