...
test_non_extendible(hid_t file)
{
    ...
    hoff_t  file_offset = 0;               /* external file offset                 */
 ...
 */
static int
add_external_files(hid_t dcpl_id, unsigned int n_external_files, hoff_t offset, hsize_t max_ext_size)
{
...
test_read_file_set(hid_t fapl)
{
    ...
    for (i = 0; i < N_EXT_FILES; i++) {
        ...
        if (H5Pset_external(dcpl, filename, (hoff_t)(i * GARBAGE_PER_FILE), (hsize_t)sizeof(part)) < 0)
            FAIL_STACK_ERROR;
    }
...
test_write_file_set(hid_t fapl)
{
    ...
    for (i = 0; i < N_EXT_FILES; i++) {
        ...
        if (H5Pset_external(dcpl, filename, (hoff_t)(i * GARBAGE_PER_FILE), size) < 0)
            FAIL_STACK_ERROR;
    } /* end for */
