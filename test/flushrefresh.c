...
refresh_verification(const char *obj_pathname)
{
    ...
    /* Get Object info */
    if ((status = H5Oget_info2(oid, &flushed_oinfo, H5O_INFO_BASIC | H5O_INFO_NUM_ATTRS | H5O_INFO_HDR)) < 0)
        PROCESS_ERROR;
    ...
    if ((status = H5Oget_info2(oid, &refreshed_oinfo, H5O_INFO_BASIC | H5O_INFO_NUM_ATTRS | H5O_INFO_HDR)) <
        0)
        PROCESS_ERROR;
    ...
    if (flushed_oinfo.type != refreshed_oinfo.type)
        PROCESS_ERROR;
    if (flushed_oinfo.hdr.version != refreshed_oinfo.hdr.version)
        PROCESS_ERROR;
    if (flushed_oinfo.hdr.flags != refreshed_oinfo.hdr.flags)
        PROCESS_ERROR;
    if (flushed_oinfo.num_attrs != refreshed_oinfo.num_attrs)
        PROCESS_ERROR;
    if (flushed_oinfo.hdr.nmesgs != refreshed_oinfo.hdr.nmesgs)
        PROCESS_ERROR;
    if (flushed_oinfo.hdr.nchunks != refreshed_oinfo.hdr.nchunks)
        PROCESS_ERROR;
    if (flushed_oinfo.hdr.space.total != refreshed_oinfo.hdr.space.total)
        PROCESS_ERROR;
    ...
    do {
        ...
        if ((status =
                 H5Oget_info2(oid, &refreshed_oinfo, H5O_INFO_BASIC | H5O_INFO_NUM_ATTRS | H5O_INFO_HDR)) < 0)
            PROCESS_ERROR;

        /* Confirm following (first 4) attributes are the same: */
        /* Confirm following (last 4) attributes are different */
        if ((flushed_oinfo.addr == refreshed_oinfo.addr) && (flushed_oinfo.type == refreshed_oinfo.type) &&
    ...
    if (!ok) {
        HDprintf("FLUSHED: num_attrs=%d, nmesgs=%d, nchunks=%d, total=%d\n", (int)flushed_oinfo.num_attrs,
                 (int)flushed_oinfo.hdr.nmesgs, (int)flushed_oinfo.hdr.nchunks,
                 (int)flushed_oinfo.hdr.space.total);
        HDprintf("REFRESHED: num_attrs=%d, nmesgs=%d, nchunks=%d, total=%d\n", (int)refreshed_oinfo.num_attrs,
                 (int)refreshed_oinfo.hdr.nmesgs, (int)refreshed_oinfo.hdr.nchunks,
                 (int)refreshed_oinfo.hdr.space.total);
        PROCESS_ERROR;
    }
