...
test_mf_fs_persist(const char *env_h5_drvr, hid_t fapl, hbool_t new_format)
{
    ...
    if (HDstrcmp(env_h5_drvr, "split") != 0 && HDstrcmp(env_h5_drvr, "multi") != 0) {
         ...
         */
        if ((f->shared->first_alloc_dealloc) && (SUCCEED != H5MF_tidy_self_referential_fsm_hack(f)))
            FAIL_STACK_ERROR
...
test_mf_fs_gone(const char *env_h5_drvr, hid_t fapl, hbool_t new_format)
{
    ...
    if (contig_addr_vfd) {
         ...
         */
        if ((!ran_H5MF_tidy_self_referential_fsm_hack) && (!H5F_addr_defined(fs_stat.addr)))
            TEST_ERROR
    ...
    if (!multi && !split) {
        ...
        for (fs_persist = FALSE; fs_persist <= TRUE; fs_persist++) {
            ...
            if (fs_persist) {
                ...
                H5MF__alloc_to_fs_type(f, H5FD_MEM_OHDR, f->shared->fs_page_size - 1,
                                       (H5F_mem_page_t *)&fs_type);
                if (!H5F_addr_defined(f->shared->fs_addr[fs_type]))
                    TEST_ERROR
