...
const int windchillFint[ROWS][COLS] =
...
int compare_int(const int* a, const int* b, int tol);
int compare_float(const float* a,const  float* b, double tol);
int compare_char(const char* a,const  float* b, int tol);
int compare_double(const double* a, const  float* b, int tol);
int compare_short(const short* a, const  float* b, int tol);


int main(void)
{
    hid_t dxpl_id_c_to_f, dxpl_id_c_to_f_copy, dxpl_id_simple, dxpl_id_polynomial, dxpl_id_polynomial_copy;
...
int init_test(hid_t* dxpl_id_c_to_f, hid_t* dxpl_id_simple, hid_t* dxpl_id_polynomial, hid_t* dxpl_id_polynomial_copy, hid_t* dxpl_id_c_to_f_copy)
{
    ...
    if((datatype = H5Tcopy(H5T_NATIVE_UCHAR))<0) TEST_ERROR;
    if((dset_id_uchar = H5Dcreate(file_id, "/transformtest_uchar", datatype, dataspace, H5P_DEFAULT))<0) TEST_ERROR;
    if((err = H5Dwrite(dset_id_uchar, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, dxpl_id_f_to_c, windchillFint))<0) TEST_ERROR;
    if((err = H5Tclose(datatype))<0) TEST_ERROR;


    if((datatype = H5Tcopy(H5T_NATIVE_SCHAR))<0) TEST_ERROR;
    if((dset_id_schar = H5Dcreate(file_id, "/transformtest_schar", datatype, dataspace, H5P_DEFAULT))<0) TEST_ERROR;
    if((err = H5Dwrite(dset_id_schar, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, dxpl_id_f_to_c, windchillFint))<0) TEST_ERROR;
    if((err = H5Tclose(datatype))<0) TEST_ERROR;


    if((datatype = H5Tcopy(H5T_NATIVE_UINT))<0) TEST_ERROR;
    if((dset_id_uint = H5Dcreate(file_id, "/transformtest_uint", datatype, dataspace, H5P_DEFAULT))<0) TEST_ERROR;
    if((err = H5Dwrite(dset_id_uint, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, dxpl_id_f_to_c, windchillFint))<0) TEST_ERROR;
    if((err = H5Tclose(datatype))<0) TEST_ERROR;

    if((datatype = H5Tcopy(H5T_NATIVE_USHORT))<0) TEST_ERROR;
    if((dset_id_ushort = H5Dcreate(file_id, "/transformtest_ushort", datatype, dataspace, H5P_DEFAULT))<0) TEST_ERROR;
    if((err = H5Dwrite(dset_id_ushort, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, dxpl_id_f_to_c, windchillFint))<0) TEST_ERROR;
    if((err = H5Tclose(datatype))<0) TEST_ERROR;

    if((datatype = H5Tcopy(H5T_NATIVE_ULONG))<0) TEST_ERROR;
    if((dset_id_ulong = H5Dcreate(file_id, "/transformtest_ulong", datatype, dataspace, H5P_DEFAULT))<0) TEST_ERROR;
    if((err = H5Dwrite(dset_id_ulong, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, dxpl_id_f_to_c, windchillFint))<0) TEST_ERROR;
    if((err = H5Tclose(datatype))<0) TEST_ERROR;

    if((datatype = H5Tcopy(H5T_NATIVE_LONG))<0) TEST_ERROR;
    if((dset_id_long = H5Dcreate(file_id, "/transformtest_long", datatype, dataspace, H5P_DEFAULT))<0) TEST_ERROR;
    if((err = H5Dwrite(dset_id_long, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, dxpl_id_f_to_c, windchillFint))<0) TEST_ERROR;
    if((err = H5Tclose(datatype))<0) TEST_ERROR;

    if((datatype = H5Tcopy(H5T_NATIVE_SHORT))<0) TEST_ERROR;
    if((dset_id_short = H5Dcreate(file_id, "/transformtest_short", datatype, dataspace, H5P_DEFAULT))<0) TEST_ERROR;
    if((err = H5Dwrite(dset_id_short, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, dxpl_id_f_to_c, windchillFint))<0) TEST_ERROR;
    if((err = H5Tclose(datatype))<0) TEST_ERROR;

    if((datatype = H5Tcopy(H5T_NATIVE_DOUBLE))<0) TEST_ERROR;
    if((dset_id_double = H5Dcreate(file_id, "/transformtest_double", datatype, dataspace, H5P_DEFAULT))<0) TEST_ERROR;
    if((err = H5Dwrite(dset_id_double, H5T_NATIVE_INT, H5S_ALL, H5S_ALL, dxpl_id_f_to_c, windchillFint))<0) TEST_ERROR;
    if((err = H5Tclose(datatype))<0) TEST_ERROR;
