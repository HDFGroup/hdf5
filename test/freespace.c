...
static unsigned test_fs_create(hid_t fapl, hid_t dxpl);
static unsigned test_fs_sect_add(hid_t fapl, hid_t dxpl);
static unsigned test_fs_sect_merge(hid_t fapl, hid_t dxpl);
static unsigned test_fs_sect_shrink(hid_t fapl, hid_t dxpl);
static unsigned test_fs_sect_find(hid_t fapl, hid_t dxpl);
static unsigned test_fs_sect_change_class(hid_t fapl, hid_t dxpl);
static unsigned test_fs_sect_extend(hid_t fapl, hid_t dxpl);
static unsigned test_fs_sect_iterate(hid_t fapl, hid_t dxpl);
 ...
 */
static unsigned
test_fs_create(hid_t fapl, hid_t dxpl)
{
    ...
    if(NULL == (frsp = H5FS_open(f, dxpl, fs_addr,
 ...
 */
static unsigned
test_fs_sect_add(hid_t fapl, hid_t dxpl)
{
    hid_t		file = -1;              /* File ID */
    char		filename[FILENAME_LEN]; /* Filename to use */
    H5F_t		*f = NULL;              /* Internal file object pointer */
...
test_fs_sect_change_class(hid_t fapl, hid_t dxpl)
{
    ...
    if (H5FS_sect_change_class(f, dxpl, frsp, (H5FS_section_info_t *)sect_node2,
		    TEST_FSPACE_SECT_TYPE) < 0)
	TEST_ERROR

    /* change the class of C to A's class */
    if (H5FS_sect_change_class(f, dxpl, frsp, (H5FS_section_info_t *)sect_node3,
		    TEST_FSPACE_SECT_TYPE) < 0)
	TEST_ERROR
