...
#define DSETS_INITIALIZER (dsets_state_t)   {   \
    ...
    , .compact_sid = H5I_INVALID_HID            \
    , .contig_did = H5I_INVALID_HID             \
    , .contig_sid = H5I_INVALID_HID             \
    , .single_did = H5I_INVALID_HID             \
    , .single_sid = H5I_INVALID_HID             \
    , .implicit_did = H5I_INVALID_HID           \
    , .implicit_sid = H5I_INVALID_HID           \
    , .fa_did = H5I_INVALID_HID                 \
    , .fa_sid = H5I_INVALID_HID                 \
    , .ea_did = H5I_INVALID_HID                 \
    , .ea_sid = H5I_INVALID_HID                 \
    , .bt2_did = H5I_INVALID_HID                \
    , .bt2_sid = H5I_INVALID_HID                }
...
#define NP_INITIALIZER (np_state_t) {		                        \
    ...
    , .fifo_reader_to_writer = "./fifo_dsetops_reader_to_writer"   \
...
static bool dset_setup(unsigned action, unsigned which, const state_t *s, 
    hsize_t *start, hsize_t *stride, hsize_t *count, hsize_t *block, 
    hid_t *mem_sid, unsigned int **buf);
static bool write_dset(hid_t did, hid_t tid, hid_t mem_sid, hid_t file_sid,
...
static bool verify_read_dset(hid_t did, hid_t tid, hid_t mem_sid, hid_t file_sid,
    hsize_t *start, hsize_t *stride, hsize_t *count, hsize_t *block, unsigned int *vbuf);
...
create_dsets(const state_t *s, dsets_state_t *ds)
{
    ...
    *ds = DSETS_INITIALIZER;
    ...
    if(s->compact) {
        ...
        if((ds->compact_did = H5Dcreate2(s->file, DSET_COMPACT_NAME, dtid,
                             ds->compact_sid, H5P_DEFAULT, dcpl, H5P_DEFAULT)) < 0) {
    ...
    if(s->contig) {
        ...
        if((ds->contig_did = H5Dcreate2(s->file, DSET_CONTIG_NAME, dtid,
                            ds->contig_sid, H5P_DEFAULT, dcpl, H5P_DEFAULT)) < 0) {
    ...
    if(s->chunked) {
        ...
        if((ds->single_did = H5Dcreate2(s->file, DSET_SINGLE_NAME, dtid,
                            ds->single_sid, H5P_DEFAULT, dcpl, H5P_DEFAULT)) < 0) {
        ...
        if((ds->implicit_did = H5Dcreate2(s->file, DSET_IMPLICIT_NAME, dtid,
                            ds->implicit_sid, H5P_DEFAULT, dcpl, H5P_DEFAULT)) < 0) {
        ...
        if((ds->fa_did = H5Dcreate2(s->file, DSET_FA_NAME, dtid,
                            ds->fa_sid, H5P_DEFAULT, dcpl, H5P_DEFAULT)) < 0) {
        ...
        if((ds->ea_did = H5Dcreate2(s->file, DSET_EA_NAME, dtid,
                            ds->ea_sid, H5P_DEFAULT, dcpl, H5P_DEFAULT)) < 0) {
        ...
        if((ds->bt2_did = H5Dcreate2(s->file, DSET_BT2_NAME, dtid,
                            ds->bt2_sid, H5P_DEFAULT, dcpl, H5P_DEFAULT)) < 0) {
...
dsets_action(unsigned action, const state_t *s, const dsets_state_t *ds, unsigned which)
{
    ...
    if(s->contig) {
        ...
        if(!write_dset(ds->contig_did, s->filetype, mem_sid, ds->contig_sid, start, stride, count, block, wbuf)) {
    ...
    if(s->chunked) {
        ...
        if(!write_dset(ds->single_did, s->filetype, mem_sid, ds->single_sid, start, stride, count, block, wbuf)) {
            printf("H5Dwrite to chunked dataset: single index dataset failed\n");
            TEST_ERROR;
        }

        if(!write_dset(ds->implicit_did, s->filetype, mem_sid, ds->implicit_sid, start, stride, count, block, wbuf)) {
 ...
 */
static bool
dset_setup(unsigned action, unsigned which, const state_t *s, 
    hsize_t *start, hsize_t *stride, hsize_t *count, hsize_t *block, 
    hid_t *mem_sid, unsigned int **buf)
{
    hsize_t mem_dims[1];
 ...
 */
static bool 
write_dset(hid_t did, hid_t tid, hid_t mem_sid, hid_t file_sid,
    hsize_t *start, hsize_t *stride, hsize_t *count, hsize_t *block, unsigned int *buf)
{
...
verify_dsets_action(unsigned action, const state_t *s, const dsets_state_t *ds, unsigned which)
{
    ...
    if(s->contig) {
        if(!verify_read_dset(ds->contig_did, s->filetype, mem_sid, ds->contig_sid, start, stride, count, block, vbuf)) {
            printf("H5Dwrite to contiguous dataset failed\n");
            TEST_ERROR;
        }
    }
    ...
    if(s->chunked) {
        ...
        if(!verify_read_dset(ds->single_did, s->filetype, mem_sid, ds->single_sid, start, stride, count, block, vbuf)) {
            printf("H5Dwrite to chunked dataset: single index dataset failed\n");
            TEST_ERROR;
        }

        if(!verify_read_dset(ds->implicit_did, s->filetype, mem_sid, ds->implicit_sid, start, stride, count, block, vbuf)) {
            printf("H5Dwrite to chunked dataset: implicit index dataset failed\n");
            TEST_ERROR;
        }

        if(!verify_read_dset(ds->fa_did, s->filetype, mem_sid, ds->fa_sid, start, stride, count, block, vbuf)) {
            printf("H5Dwrite to chunked dataset: fa index dataset failed\n");
            TEST_ERROR;
        }

        if(!verify_read_dset(ds->ea_did, s->filetype, mem_sid, ds->ea_sid, start, stride, count, block, vbuf)) {
            printf("H5Dwrite to chunked dataset: ea index dataset failed\n");
            TEST_ERROR;
        }

        if(!verify_read_dset(ds->bt2_did, s->filetype, mem_sid, ds->bt2_sid, start, stride, count, block, vbuf)) {
            printf("H5Dwrite to chunked dataset: bt2 index dataset failed\n");
            TEST_ERROR;
        }
    }
 ...
 */
static bool 
verify_read_dset(hid_t did, hid_t tid, hid_t mem_sid, hid_t file_sid,
    hsize_t *start, hsize_t *stride, hsize_t *count, hsize_t *block, unsigned int *vbuf)
{
