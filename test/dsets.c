...
const char *FILENAME[] = {
    "dataset",      "compact_dataset",   "dset_offset",   "max_compact_dataset", "simple",
    "set_local",    "random_chunks",     "huge_chunks",   "chunk_cache",         "big_chunk",
    "chunk_expand", "copy_dcpl_newfile", "layout_extend", "zero_chunk",          NULL};
...
test_nbit_double(hid_t file)
{
    ...
    double orig_data[2][5] = {
        {H5_DOUBLE(1.6081706885101836e+60), H5_DOUBLE(-255.32099170994480), H5_DOUBLE(1.2677579992621376e-61),
         H5_DOUBLE(64568.289448797700), H5_DOUBLE(-1.0619721778839084e-75)},
        {H5_DOUBLE(2.1499497833454840e+56), H5_DOUBLE(6.6562295504670740e-3), H5_DOUBLE(-1.5747263393432150),
         H5_DOUBLE(1.0711093225222612), H5_DOUBLE(-9.8971679387636870e-1)}};
    double new_data[2][5];
    size_t precision, offset;
    size_t i, j;
    ...
    for (i = 0; i < size[0]; i++) {
        for (j = 0; j < size[1]; j++) {
            if (((unsigned)new_data[i][j].i & i_mask) != ((unsigned)orig_data[i][j].i & i_mask) ||
                ...
                (orig_data[i][j].f == orig_data[i][j].f &&
                 !H5_FLT_ABS_EQUAL(new_data[i][j].f, orig_data[i][j].f))) {
                H5_FAILED();
                HDprintf("    Read different values than written.\n");
                HDprintf("    At index %lu,%lu\n", (unsigned long)i, (unsigned long)j);
                goto error;
...
out:
            if (((unsigned)new_data[i][j].a.i & i_mask) != ((unsigned)orig_data[i][j].a.i & i_mask) ||
                ...
                (new_data[i][j].a.f == new_data[i][j].a.f &&
                 !H5_FLT_ABS_EQUAL(new_data[i][j].a.f, new_data[i][j].a.f)) ||
...
test_nbit_compound_3(hid_t file)
{
     ...
     *----------------------------------------------------------------------
     */
    if (H5Dvlen_reclaim(cmpd_tid, space, H5P_DEFAULT, new_data) < 0)
        goto error;
    if (H5Dvlen_reclaim(cmpd_tid, space, H5P_DEFAULT, orig_data) < 0)
        goto error;
    if (H5Tclose(i_tid) < 0)
        goto error;
...
test_missing_chunk(hid_t file)
{
    hid_t   s = -1, d = -1, dcpl = -1;
    hsize_t hs_start[1];
    hsize_t hs_stride[1], hs_count[1], hs_block[1];
    int     wdata[MISSING_CHUNK_DIM], rdata[MISSING_CHUNK_DIM];
...
test_random_chunks(hid_t fapl)
{
    ...
    hsize_t dsize[2] = {100, 100}, dmax[2] = {H5S_UNLIMITED, H5S_UNLIMITED}, csize[2] = {10, 10},
            nsize[2]    = {200, 200};
    ...
    if ((s = H5Screate_simple(2, dsize, dmax)) < 0)
        TEST_ERROR;
...
test_big_chunks_bypass_cache(hid_t fapl)
{
    ...
    static int wdata[BYPASS_CHUNK_DIM / 2], rdata1[BYPASS_DIM], rdata2[BYPASS_CHUNK_DIM / 2];
    int        i, j;
...
test_chunk_expand(hid_t fapl)
{
    ...
    if (size <= 4) {
        SKIPPED();
        HDputs("    Current machine can't test for error");
    } /* end if */
    else {
