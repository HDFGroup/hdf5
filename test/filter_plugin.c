...
test_creating_groups_using_plugins(hid_t fid)
{
    ...
    for (i = 0; i < N_SUBGROUPS; i++) {
        ...
        sp += sprintf(subgroup_name, SUBGROUP_PREFIX);
        sprintf(sp, "%d", i);
...
test_path_api_calls(void)
{
    ...
    if ((path_len = H5PLget(n_starting_paths - 1, path, 256)) <= 0)
        TEST_ERROR;
    sprintf(temp_name, "a_path_%u", n_starting_paths - 1);
    ...
    /* Prepend one path */
    sprintf(path, "a_path_%d", n_starting_paths + 1);
    ...
    if (H5PLget(0, path, 256) <= 0)
        TEST_ERROR;
    sprintf(temp_name, "a_path_%d", n_starting_paths + 1);
    if (strcmp(path, temp_name) != 0) {
        fprintf(stderr, "    get 0: %s\n", path);
        TEST_ERROR;
    }
    ...
    sprintf(path, "a_path_%u", n_starting_paths + 4);
    ...
    sprintf(path, "a_path_%d", n_starting_paths + 5);
