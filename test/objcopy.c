...
const char *FILENAME[] = {"objcopy_src", "objcopy_dst", "objcopy_ext", "objcopy_src2", NULL};
...
compare_attribute(hid_t aid, hid_t aid2, hid_t pid, const void *wbuf, hid_t obj_owner)
{
    ...
    if (H5Tdetect_class(tid, H5T_VLEN) == TRUE)
        if (H5Dvlen_reclaim(tid, sid, H5P_DEFAULT, rbuf) < 0)
            TEST_ERROR
    if (H5Tdetect_class(tid2, H5T_VLEN) == TRUE)
        if (H5Dvlen_reclaim(tid2, sid2, H5P_DEFAULT, rbuf2) < 0)
            TEST_ERROR
...
compare_std_attributes(hid_t oid, hid_t oid2, hid_t pid)
{
    ...
    if (H5Oget_info(oid, &oinfo1) < 0)
        TEST_ERROR

    /* Check the number of attributes on destination dataset */
    if (H5Oget_info(oid2, &oinfo2) < 0)
        TEST_ERROR
    ...
    if ((H5Tget_class(tid) == H5T_COMPOUND) && (H5Tdetect_class(tid, H5T_VLEN) == TRUE)) {
        ...
        if (H5Tequal(tid, H5T_STD_REF_OBJ) > 0) {
            ...
            for (u = 0; u < nelmts; u++, ref_buf1++, ref_buf2++) {
                ...
                if ((obj1_id = H5Rdereference(parent1, H5R_OBJECT, ref_buf1)) < 0)
                    TEST_ERROR
                if ((obj2_id = H5Rdereference(parent2, H5R_OBJECT, ref_buf2)) < 0)
                    TEST_ERROR
                ...
                if (obj_owner > 0) {
                    ...
                    if (H5Oget_info(obj_owner, &oinfo1) < 0)
                        TEST_ERROR
                    if (H5Oget_info(obj1_id, &oinfo2) < 0)
                        TEST_ERROR
            ...
            for (u = 0; u < nelmts; u++, ref_buf1++, ref_buf2++) {
                ...
                /* Open referenced objects */
                if ((obj1_id = H5Rdereference(parent1, H5R_DATASET_REGION, ref_buf1)) < 0)
                    TEST_ERROR
                if ((obj2_id = H5Rdereference(parent2, H5R_DATASET_REGION, ref_buf2)) < 0)
                    TEST_ERROR
                ...
                if (H5S_select_shape_same_test(obj1_sid, obj2_sid) < 0)
                    TEST_ERROR
...
compare_datasets(hid_t did, hid_t did2, hid_t pid, const void *wbuf)
{
     ...
     */
    if ((ext_count = H5Pget_external_count(dcpl)) < 0)
        TEST_ERROR
    ...
    if (ext_count > 0) {
        ...
        if (H5Pget_external_count(dcpl2) != ext_count)
            TEST_ERROR

        /* Ensure that all external file information is the same */
        for (x = 0; x < (unsigned)ext_count; ++x) {
            if (H5Pget_external(dcpl, x, (size_t)NAME_BUF_SIZE, name1, &offset1, &size1) < 0)
                TEST_ERROR
            if (H5Pget_external(dcpl2, x, (size_t)NAME_BUF_SIZE, name2, &offset2, &size2) < 0)
                TEST_ERROR
        ...
        */
        ...
        if (H5P_reset_external_file_test(dcpl) < 0)
            TEST_ERROR
        if (H5P_reset_external_file_test(dcpl2) < 0)
            TEST_ERROR
    }
...
compare_groups(hid_t gid, hid_t gid2, hid_t pid, int depth, unsigned copy_flags)
{
    ...
    if (ginfo2.nlinks > 0) {
        ...
        for (idx = 0; idx < ginfo.nlinks; idx++) {
            ...
            /* Get link info */
            if (H5Lget_info(gid, objname, &linfo, H5P_DEFAULT) < 0)
                TEST_ERROR
            if (H5Lget_info(gid2, objname2, &linfo2, H5P_DEFAULT) < 0)
                TEST_ERROR
            ...
            if (linfo.type == H5L_TYPE_HARD) {
                ...
                if (H5Oget_info_by_name(gid, objname, &oinfo, H5P_DEFAULT) < 0)
                    TEST_ERROR
                if (H5Oget_info_by_name(gid2, objname2, &oinfo2, H5P_DEFAULT) < 0)
                    TEST_ERROR
                ...
                if (H5O_COPY_PRESERVE_NULL_FLAG & copy_flags) {
                    if (oinfo.hdr.nmesgs != oinfo2.hdr.nmesgs)
                        ;
                    else if (oinfo.hdr.nmesgs < oinfo2.hdr.nmesgs)
                        TEST_ERROR
                }
...
test_copy_named_datatype(hid_t fcpl_src, hid_t fcpl_dst, hid_t src_fapl, hid_t dst_fapl)
{
    ...
    /* Create an uncopied object in destination file so that addresses in source and destination files aren't
     * the same */
...
compare_attribute_compound_vlstr(hid_t loc, hid_t loc2)
{
    ...
    if (H5Dvlen_reclaim(tid, sid, H5P_DEFAULT, &rbuf) < 0)
        TEST_ERROR
    if (H5Dvlen_reclaim(tid, sid, H5P_DEFAULT, &rbuf2) < 0)
        TEST_ERROR
...
test_copy_group_links(hid_t fcpl_src, hid_t fcpl_dst, hid_t src_fapl, hid_t dst_fapl)
{
    ...
    for (expand_soft = 0; expand_soft <= 1; expand_soft++) {
        for (expand_ext = 0; expand_ext <= 1; expand_ext++) {
            ...
            /* Create an uncopied object in destination file so that addresses in source and destination files
             * aren't the same */
            if (H5Gclose(H5Gcreate2(fid_dst, NAME_GROUP_UNCOPIED, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0)
                TEST_ERROR
            ...
            if (expand_soft) {
                /* Check link type */
                if (H5Lget_info(fid_dst, NAME_LINK_SOFT, &linfo, H5P_DEFAULT) < 0)
                    TEST_ERROR
                if (linfo.type != H5L_TYPE_HARD)
            ...
            if (expand_ext) {
                /* Check link type */
                if (H5Lget_info(fid_dst, NAME_LINK_EXTERN, &linfo, H5P_DEFAULT) < 0)
                    TEST_ERROR
                if (linfo.type != H5L_TYPE_HARD)
...
test_copy_ext_link(hid_t fcpl_src, hid_t fcpl_dst, hid_t src_fapl, hid_t dst_fapl)
{
    ...
    if ((fid_ext = H5Fcreate(ext_filename, H5F_ACC_TRUNC, H5P_DEFAULT, src_fapl)) < 0)
        TEST_ERROR
...
test_copy_old_layout(hid_t fcpl_dst, hid_t fapl, hbool_t test_open)
{
    ...
    if ((fid_src = H5Fopen(src_filename, H5F_ACC_RDONLY, H5P_DEFAULT)) < 0)
        TEST_ERROR
...
test_copy_dataset_compact_named_vl(hid_t fcpl_src, hid_t fcpl_dst, hid_t src_fapl, hid_t dst_fapl)
{
    ...
    if (H5Tdetect_class(tid_copy, H5T_VLEN) == TRUE) {
        if (H5Dvlen_reclaim(tid_copy, sid, H5P_DEFAULT, buf) < 0)
            TEST_ERROR
    }
...
test_copy_dataset_compact_vl_vl(hid_t fcpl_src, hid_t fcpl_dst, hid_t src_fapl, hid_t dst_fapl)
{
    ...
    /* Reclaim vlen buffer */
    if (H5Tdetect_class(tid2, H5T_VLEN) == TRUE) {
        if (H5Dvlen_reclaim(tid2, sid, H5P_DEFAULT, buf) < 0)
            TEST_ERROR
    }
 ...
 */
static herr_t
test_copy_iterate_cb(hid_t loc_id, const char *name, const H5L_info_t H5_ATTR_UNUSED *link_info,
                     void *op_data)
{
