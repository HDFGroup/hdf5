...
compare_std_attributes(hid_t oid, hid_t oid2, hid_t pid)
{
    ...
    if (H5Oget_info2(oid, &oinfo1, H5O_INFO_NUM_ATTRS) < 0)
        TEST_ERROR

    /* Check the number of attributes on destination dataset */
    if (H5Oget_info2(oid2, &oinfo2, H5O_INFO_NUM_ATTRS) < 0)
        TEST_ERROR
        ...
        if (H5Tequal(tid, H5T_STD_REF_OBJ) > 0) {
            ...
            for (u = 0; u < nelmts; u++, ref_buf1++, ref_buf2++) {
                ...
                if (obj_owner > 0) {
                    ...
                    if (H5Oget_info2(obj_owner, &oinfo1, H5O_INFO_BASIC) < 0)
                        TEST_ERROR
                    if (H5Oget_info2(obj1_id, &oinfo2, H5O_INFO_BASIC) < 0)
                        TEST_ERROR
    ...
    if (ginfo2.nlinks > 0) {
        ...
        for (idx = 0; idx < ginfo.nlinks; idx++) {
            ...
            if (linfo.type == H5L_TYPE_HARD) {
                ...
                if (H5Oget_info_by_name2(gid, objname, &oinfo, H5O_INFO_BASIC | H5O_INFO_HDR, H5P_DEFAULT) <
                    0)
                    TEST_ERROR
                if (H5Oget_info_by_name2(gid2, objname2, &oinfo2, H5O_INFO_BASIC | H5O_INFO_HDR,
                                         H5P_DEFAULT) < 0)
                    TEST_ERROR
...
test_copy_dataset_versionbounds(hid_t fcpl_src, hid_t fapl_src)
{
    ...
    if ((dsetp = (H5D_t *)H5I_object(did_src)) == NULL)
        TEST_ERROR
...
test_copy_dataset_contig_vl(hid_t fcpl_src, hid_t fcpl_dst, hid_t src_fapl, hid_t dst_fapl)
{
    ...
    if (H5Tdetect_class(tid, H5T_VLEN) == TRUE) {
        ...
        if (H5Pset_vlen_mem_manager(dxpl_id, NULL, NULL, NULL, NULL) < 0)
            TEST_ERROR
        if (H5Dvlen_reclaim(tid, sid, dxpl_id, buf) < 0)
            TEST_ERROR
...
compare_attribute_compound_vlstr(hid_t loc, hid_t loc2)
{
    ...
    if (H5Pset_vlen_mem_manager(dxpl_id, NULL, NULL, NULL, NULL) < 0)
        TEST_ERROR
    if (H5Dvlen_reclaim(tid, sid, dxpl_id, &rbuf) < 0)
        TEST_ERROR
    if (H5Dvlen_reclaim(tid, sid, dxpl_id, &rbuf2) < 0)
        TEST_ERROR
    if (H5Pclose(dxpl_id) < 0)
        TEST_ERROR
...
test_copy_dataset_compact_named_vl(hid_t fcpl_src, hid_t fcpl_dst, hid_t src_fapl, hid_t dst_fapl)
{
    ...
    if (H5Tdetect_class(tid_copy, H5T_VLEN) == TRUE) {
        ...
        if (H5Pset_vlen_mem_manager(dxpl_id, NULL, NULL, NULL, NULL) < 0)
            TEST_ERROR
        if (H5Dvlen_reclaim(tid_copy, sid, dxpl_id, buf) < 0)
            TEST_ERROR
        if (H5Pclose(dxpl_id) < 0)
            TEST_ERROR
    } /* end if */
...
test_copy_dataset_compact_vl_vl(hid_t fcpl_src, hid_t fcpl_dst, hid_t src_fapl, hid_t dst_fapl)
{
    ...
    if (H5Tdetect_class(tid2, H5T_VLEN) == TRUE) {
        ...
        if (H5Pset_vlen_mem_manager(dxpl_id, NULL, NULL, NULL, NULL) < 0)
            TEST_ERROR
        if (H5Dvlen_reclaim(tid2, sid, dxpl_id, buf) < 0)
            TEST_ERROR
        if (H5Pclose(dxpl_id) < 0)
            TEST_ERROR
    } /* end if */
