...
****************************************************************/
static void
check_fcpl_values(hid_t fcpl_id, const unsigned nindexes_in, const unsigned *flags_in,
                  const unsigned *minsizes_in, unsigned l2b, unsigned b2l)
{
...
size1_helper(hid_t file, const char *filename, hid_t fapl_id, int test_file_closing)
{
    ...
    if ((dset_id =
             H5Dcreate2(file, DSETNAME[0], dtype1_id, space_id, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0)
        FAIL_STACK_ERROR
    ...
    if ((dset_id =
             H5Dcreate2(file, DSETNAME[1], dtype1_id, space_id, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0)
        FAIL_STACK_ERROR
    if (H5Dclose(dset_id) < 0)
        FAIL_STACK_ERROR
    ...
    if ((dset_id =
             H5Dcreate2(file, DSETNAME[2], dtype1_id, space_id, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0)
        TEST_ERROR
    if (H5Dclose(dset_id) < 0)
        TEST_ERROR
    ...
    if ((dset_id =
             H5Dcreate2(file, DSETNAME[3], dtype1_id, space_id, H5P_DEFAULT, H5P_DEFAULT, H5P_DEFAULT)) < 0)
        TEST_ERROR
    ...
    if ((dset_id = H5Dopen2(file, DSETNAME[0], H5P_DEFAULT)) < 0)
        TEST_ERROR
    if ((dtype1_id = H5Dget_type(dset_id)) < 0)
        TEST_ERROR
    ...
    /* Create several copies of the dataset (this increases the amount of space saved by sharing the datatype
     * message) */
    for (x = 0; x < SOHM_HELPER_NUM_EX_DSETS; x++) {
        if ((dset_id = H5Dcreate2(file, EXTRA_DSETNAME[x], dtype1_id, space_id, H5P_DEFAULT, H5P_DEFAULT,
                                  H5P_DEFAULT)) < 0)
            TEST_ERROR
        if (H5Dclose(dset_id) < 0)
            TEST_ERROR
    ...
    if ((dset_id = H5Dopen2(file, DSETNAME[3], H5P_DEFAULT)) < 0)
        TEST_ERROR
    if ((dtype1_id = H5Dget_type(dset_id)) < 0)
        TEST_ERROR
...
test_sohm_attrs(void)
{
    ...
    ret = H5Pset_shared_mesg_index(fcpl_id, 0,
                                   H5O_SHMESG_SDSPACE_FLAG | H5O_SHMESG_DTYPE_FLAG | H5O_SHMESG_ATTR_FLAG, 2);
    CHECK_I(ret, "H5Pset_shared_mesg_nindexes");
...
test_sohm_size2(int close_reopen)
{
     ...
     * will take.
     */
    if ((mult_index_med.attrs2 - mult_index_med.attrs1) != (list_index_med.attrs2 - list_index_med.attrs1))
        VERIFY(0, 1, "h5_get_file_size");
    if ((mult_index_btree.attrs2 - mult_index_btree.attrs1) != (btree_index.attrs2 - btree_index.attrs1))
        VERIFY(0, 1, "h5_get_file_size");
 ...
 */
static void
test_sohm_extlink_helper(hid_t src_fcpl_id, hid_t dst_fcpl_id)
{
    hid_t   src_file_id = -1;
    hid_t   dst_file_id = -1;
    hid_t   space_id    = -1;
