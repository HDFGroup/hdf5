 ...
 * a fixed, checked-in artifact -- generated by test/gen_filters.c -- so a
 * decode regression against that historical byte layout actually fails
 * here.  It uses a built-in filter (scaleoffset) with no custom plugin, so
 * it is also readable by any unmodified HDF5 3.0+ build or an independent
 * reader implementing only docs/doxygen/dox/H5.format.4.0.dox's "Filter
 * Pipeline Message - Version 3" layout.
 *
 * This file must NOT be regenerated by a routine test run, or it stops
 * catching the class of regression above.  It HAS been regenerated once
 * already, deliberately: the extension-block-list encoding it now pins
 * (type/flags/reserved/length-delimited blocks) replaced an earlier,
 * incompatible pipeline-v3 layout (a fixed config-string-length field)
 * under the same version byte, before either shipped in a release -- see
 * test/gen_filters.c's create_file_with_pline_v3() for the regeneration
 * procedure, which is the only sanctioned way to update this file, and
 * only for an intentional, reviewed format change like that one.
 * ---------------------------------------------------------------------- */
static int
