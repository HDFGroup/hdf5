 ...
 * checks the 2-byte config_length field two ways: rejecting anything over
 ...
 * config_length field is known to live at (verified against the checked-in
...
patch_config_length(const char *src, const char *dst, long byte_offset, uint16_t expect_len, uint16_t new_len)
{
    ...
    if ((fsize = ftell(in)) < 0)
        goto done;
    if (byte_offset < 0 || byte_offset + 2 > fsize)
        goto done;
    rewind(in);
...
{
    const char *golden;
    /* Byte offset of the 2-byte config_length field in
     * test/testfiles/test_filters_v3.h5, immediately preceding the stored
     * "scale_type = \"int\", scale_factor = 3" (36-byte) string -- located
     * by inspection when the golden file was generated. patch_config_length()
