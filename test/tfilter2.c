 ...
 *   - H5Pappend_filter / H5Pmodify_filter_by_idx / H5Pget_filter_params_by_idx
...
/* -----------------------------------------------------------------------
 * Round-trip tests: write and read a chunked dataset configured through
 * H5Pappend_filter
 * ---------------------------------------------------------------------- */
...
test_get_filter_info2_builtin(void)
{
    ...
    TESTING("H5Zget_filter_class_info: shuffle built-in (set_config but no get_config)");
    memset(&info, 0xAA, sizeof(info));
...
static herr_t
cfgstr_set_config(const char *params, unsigned H5_ATTR_UNUSED *flags, size_t *cd_nelmts, unsigned cd_values[],
                  size_t cd_values_size)
{
    int64_t level = 0;
...
static herr_t
cfgstr_get_config(unsigned H5_ATTR_UNUSED flags, size_t cd_nelmts, const unsigned cd_values[], char *buf,
                  size_t *buf_size)
{
...
static size_t
cfgstr_filter_func(unsigned int H5_ATTR_UNUSED flags, size_t H5_ATTR_UNUSED cd_nelmts,
...
static const H5Z_class3_t cfgstr_cls = {
    2,                  /* version         */
    CFGSTR_FILTER_ID,   /* id              */
    1,                  /* encoder_present */
    1,                  /* decoder_present */
    ...
    NULL,               /* can_apply       */
    NULL,               /* set_local       */
    cfgstr_filter_func, /* filter          */
    cfgstr_set_config,  /* set_config      */
    cfgstr_get_config,  /* get_config      */
    NULL,               /* description     */
};

/* Build a chunked, filter-configured DCPL from a parameter string */
static hid_t
cfgstr_make_dcpl(const char *params)
{
    hid_t        dcpl     = H5I_INVALID_HID;
    hsize_t      chunk[2] = {4, 4};
    H5Z_params_t p;
    ...
    p.u.str = params;
    if (H5Pappend_filter(dcpl, CFGSTR_FILTER_ID, 0, &p) < 0)
        goto error;
    return dcpl;
error:
    H5E_BEGIN_TRY
...
/* Fetch filter IDX's parameter string from a DCPL into buf */
static herr_t
cfgstr_get_params(hid_t dcpl, unsigned idx, char *buf, size_t buf_size)
{
    size_t len = 0;

    if (H5Pget_filter_params_by_idx(dcpl, idx, buf, buf_size, &len) < 0)
        return FAIL;
    return SUCCEED;
}

static int
test_config_string_plist(hid_t fapl)
{
    hid_t   file = H5I_INVALID_HID, sid = H5I_INVALID_HID, dcpl = H5I_INVALID_HID, dset = H5I_INVALID_HID;
    hid_t   dcpl_out = H5I_INVALID_HID, dcpl_dec = H5I_INVALID_HID, dcpl_cpy = H5I_INVALID_HID;
    hsize_t dims[2] = {8, 8};
    char    filename[1024];
    ...
    /* The stored string is returned as given, not the get_config form */
    TESTING("config string: stored in the DCPL and returned as given");
    if ((dcpl = cfgstr_make_dcpl("level=7")) < 0)
        TEST_ERROR;
    if (cfgstr_get_params(dcpl, 0, pbuf, sizeof(pbuf)) < 0)
        TEST_ERROR;
    if (strcmp(pbuf, "level=7") != 0)
        TEST_ERROR;
    PASSED();

    /* H5Pcopy carries the string; closing either copy leaves the other intact */
    TESTING("config string: survives H5Pcopy");
    if ((dcpl_cpy = H5Pcopy(dcpl)) < 0)
        TEST_ERROR;
    if (H5Pclose(dcpl) < 0)
        TEST_ERROR;
    dcpl = H5I_INVALID_HID;
    if (cfgstr_get_params(dcpl_cpy, 0, pbuf, sizeof(pbuf)) < 0)
        TEST_ERROR;
    if (strcmp(pbuf, "level=7") != 0)
        TEST_ERROR;
    if (H5Pclose(dcpl_cpy) < 0)
        TEST_ERROR;
    dcpl_cpy = H5I_INVALID_HID;
    PASSED();

    /* H5Pmodify_filter replaces cd_values and clears the stored string */
    TESTING("config string: H5Pmodify_filter clears the stored string");
    if ((dcpl = cfgstr_make_dcpl("level=3")) < 0)
        TEST_ERROR;
    {
        unsigned cd[1] = {8};
        if (H5Pmodify_filter(dcpl, CFGSTR_FILTER_ID, 0, 1, cd) < 0)
            TEST_ERROR;
    }
    if (cfgstr_get_params(dcpl, 0, pbuf, sizeof(pbuf)) < 0)
        TEST_ERROR;
    ...
    /* H5Pencode/H5Pdecode carries the string, next to a filter without one */
    TESTING("config string: survives H5Pencode/H5Pdecode");
    if ((dcpl = cfgstr_make_dcpl("level=9")) < 0)
        TEST_ERROR;
    ...
    if ((dcpl_dec = H5Pdecode(enc_buf)) < 0)
        TEST_ERROR;
    if (H5Pget_nfilters(dcpl_dec) != 2)
        TEST_ERROR;
    if (cfgstr_get_params(dcpl_dec, 0, pbuf, sizeof(pbuf)) < 0)
        TEST_ERROR;
    if (strcmp(pbuf, "level=9") != 0)
        TEST_ERROR;
    /* Shuffle has no stored string and no get_config: cd_values listing */
    if (cfgstr_get_params(dcpl_dec, 1, pbuf, sizeof(pbuf)) < 0)
        TEST_ERROR;
    ...
    /* A pipeline without strings encodes to the same size as one built with
     * H5Pset_filter: the base encoding, readable by earlier releases */
    TESTING("config string: pipeline without strings keeps the base encoding");
    {
        hid_t    dcpl_raw = H5I_INVALID_HID;
        size_t   raw_size = 0, str_size = 0;
        unsigned cd[1]    = {4};
        hsize_t  chunk[2] = {4, 4};

        if ((dcpl_raw = H5Pcreate(H5P_DATASET_CREATE)) < 0)
            TEST_ERROR;
        if (H5Pset_chunk(dcpl_raw, 2, chunk) < 0)
            TEST_ERROR;
        if (H5Pset_filter(dcpl_raw, CFGSTR_FILTER_ID, 0, 1, cd) < 0)
            TEST_ERROR;
        if (H5Pencode2(dcpl_raw, NULL, &raw_size, H5P_DEFAULT) < 0)
            TEST_ERROR;
        if (H5Pclose(dcpl_raw) < 0)
            TEST_ERROR;

        /* Same filter through the string API, then the string cleared */
        if ((dcpl = cfgstr_make_dcpl("level=4")) < 0)
            TEST_ERROR;
        if (H5Pencode2(dcpl, NULL, &str_size, H5P_DEFAULT) < 0)
            TEST_ERROR;
        /* marker + version + has_config + length size + length + "level=4",
         * plus the canonical name H5Pappend_filter records */
        if (str_size <= raw_size)
            TEST_ERROR;
        if (H5Pmodify_filter(dcpl, CFGSTR_FILTER_ID, 0, 1, cd) < 0)
            TEST_ERROR;
        if (H5Pencode2(dcpl, NULL, &str_size, H5P_DEFAULT) < 0)
            TEST_ERROR;
        /* Only the name slot differs from the H5Pset_filter encoding */
        if (str_size != raw_size + 12)
            TEST_ERROR;
        if (H5Pclose(dcpl) < 0)
            TEST_ERROR;
        dcpl = H5I_INVALID_HID;
    ...
    /* At the default library version bounds the string is written to the
     * file, so the reopened dataset's DCPL returns it as given rather than
     * the get_config reconstruction of the stored cd_values */
 ...
 *
 * A string-carrying pipeline is written as a version 3 message when the
 * file's high library version bound admits it, and is recovered from the
 * file without the filter plugin.  When the bound does not admit version 3,
 * H5Dcreate, H5Dcreate_anon and H5Ocopy fail with H5E_BADRANGE rather than
 * writing the pipeline without its string.
 * ---------------------------------------------------------------------- */
...
test_config_string_ondisk(hid_t fapl)
{
    ...
    /* --- A low bound of V300 writes a version 3 message even without
     *     strings; every entry then has an empty string and reads back
     *     through get_config --- */
 ...
 * symmetrically (e.g. both sides swap a length field's byte order) would
 * round-trip cleanly and go undetected.  test/testfiles/test_filters_v3.h5
 * is a fixed, checked-in file -- generated once by test/gen_filters.c and
 * never regenerated by a routine test run -- so a decode regression against
 * that byte layout fails here.  It uses a built-in filter (scaleoffset), so
 * it is readable by any HDF5 build that reads pipeline message version 3,
 * or by an independent reader implementing only the "Filter Pipeline
 * Message - Version 3" layout in docs/doxygen/dox/H5.format.4.0.dox.
 * ---------------------------------------------------------------------- */
static int
...
/* -----------------------------------------------------------------------
 * Corrupted on-disk config string length must be rejected cleanly
 *
 * H5O__pline_decode() checks each version 3 filter's 2-byte config string
 * length two ways: it rejects a length over H5Z_CONFIG_STRING_MAX, and a
 * length that would read past the end of the message.  No other test
 * reaches those checks, since every other decode reads a message this
 * library just encoded.
 *
 * The test writes a dataset with a version 1 object header, which has no
 * checksum, so a patched length reaches the pipeline decoder instead of
 * failing the object header checksum.  The length field is found as the
 * two bytes preceding the only occurrence of the stored string in the file,
 * and is verified to hold the string's length before it is patched.  Each
 * case checks that the error stack carries the decoder's own error.
 * ---------------------------------------------------------------------- */

#define CORRUPT_CONFIG_STR "scale_type = \"int\", scale_factor = 3"
...
/* Copy SRC to DST, replacing the 2-byte little-endian length that precedes
 * the only occurrence of CORRUPT_CONFIG_STR with NEW_LEN.  Returns 0 on
 * success, -1 on an I/O failure or if the field is not found as expected. */
static int
patch_config_length(const char *src, const char *dst, uint16_t new_len)
{
    FILE       *in = NULL, *out = NULL;
    char       *buf       = NULL;
    const char *needle    = CORRUPT_CONFIG_STR;
    size_t      nlen      = strlen(CORRUPT_CONFIG_STR);
    long        fsize     = 0;
    long        offset    = -1;
    int         ret_value = -1;
    ...
    for (long i = 2; i + (long)nlen <= fsize; i++)
    ...
    /* The length field must hold the string's length before it is patched */
    if ((uint8_t)buf[offset] != (nlen & 0xff) || (uint8_t)buf[offset + 1] != ((nlen >> 8) & 0xff))
        goto done;

    buf[offset]     = (char)(new_len & 0xff);
    buf[offset + 1] = (char)((new_len >> 8) & 0xff);
...
static bool
corrupt_open_fails_with(const char *filename, hid_t maj, hid_t min)
{
...
{
    const char  *good_file        = "tfilter2_v3_good.h5";
    ...
    TESTING("config string: corrupted on-disk config string length is rejected");
    ...
    if (H5Dclose(dset) < 0 || H5Pclose(dcpl) < 0 || H5Sclose(sid) < 0 || H5Fclose(file) < 0 ||
        H5Pclose(fapl) < 0)
        TEST_ERROR;
    dset = dcpl = sid = file = fapl = H5I_INVALID_HID;

    /* Case 1: the length exceeds H5Z_CONFIG_STRING_MAX */
    if (patch_config_length(good_file, over_max_file, 65535) < 0)
        TEST_ERROR;
    if (!corrupt_open_fails_with(over_max_file, H5E_PLINE, H5E_CANTLOAD))
        TEST_ERROR;

    /* Case 2: the length is under H5Z_CONFIG_STRING_MAX but runs past the
     * end of the message */
    if (patch_config_length(good_file, over_buffer_file, 2000) < 0)
        TEST_ERROR;
    if (!corrupt_open_fails_with(over_buffer_file, H5E_OHDR, H5E_OVERFLOW))
        TEST_ERROR;
 ...
 *
 * The other tests cover class2 and class3 filters separately.  They do not
 * cover a pipeline that carries both at once, which is the shape a real file
 * takes while an ecosystem is mid-migration: an old third-party compressor
 * stacked with a newly ported one, plus a built-in.  The pipeline
...
test_config_string_pdecode_corrupted(void)
{
    ...
    if ((dcpl = cfgstr_make_dcpl(padded)) < 0) {
        free(padded);
        TEST_ERROR;
    }
    ...
    /* Restore the length; the buffer must decode again */
    needle[-2] = (uint8_t)(padded_len & 0xff);
    needle[-1] = (uint8_t)((padded_len >> 8) & 0xff);
    if ((dcpl_dec = H5Pdecode(enc_buf)) < 0)
        TEST_ERROR;
    if (H5Pclose(dcpl_dec) < 0)
        TEST_ERROR;
    ...
    for (i = 0; i + 8 <= enc_size; i++) {
        if (memcmp((uint8_t *)enc_buf + i, "pline", 6) == 0) {
            needle = (uint8_t *)enc_buf + i + 6;
            break;
        }
    }
    if (NULL == needle || needle[0] != 0 || needle[1] != 1)
        TEST_ERROR;
    needle[1] = 2;
    H5E_BEGIN_TRY
    {
    ...
    if (dcpl_dec >= 0) {
        H5Pclose(dcpl_dec);
        TEST_ERROR; /* must fail: unknown pipeline encoding version */
    }
    dcpl_dec = H5I_INVALID_HID;
...
test_config_canonicalization(hid_t fapl)
{
    ...
    /* --- canon-04: both normalizations at once --- */
...
test_set_local_keeps_config(hid_t fapl)
{
    ...
    /* Still the compact form in the new dataset's DCPL, after set_local */
    if ((dcpl_out = H5Dget_create_plist(dset)) < 0)
        TEST_ERROR;
    if (H5Pget_filter_params_by_idx(dcpl_out, 0, pbuf, sizeof(pbuf), &plen) < 0)
        TEST_ERROR;
...
test_modify_filter_by_idx(hid_t fapl)
{
    ...
    if (canon_stored_double(dcpl_out, &got) < 0)
        TEST_ERROR;
    if (memcmp(&got, &(double){7.25}, sizeof(got)) != 0)
        TEST_ERROR;
    if (H5Pclose(dcpl_out) < 0 || H5Dclose(dset) < 0 || H5Fclose(file) < 0 || H5Sclose(sid) < 0)
        TEST_ERROR;
    dcpl_out = dset = file = sid = H5I_INVALID_HID;
...
main(void)
{
    ...
    /* H5Pappend_filter / H5Pget_filter_params_by_idx callback contracts */
    ...
    /* Additional H5Pappend_filter coverage */
    ...
    /* Configuration strings in property lists, H5Pencode/H5Pdecode */
