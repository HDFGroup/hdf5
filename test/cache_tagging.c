...
check_file_creation_tags(hid_t fcpl_id, int type)
{
    ...
    if ((fid = H5Fcreate(FILENAME, H5F_ACC_TRUNC, fcpl_id, H5P_DEFAULT)) < 0)
        TEST_ERROR;
...
check_attribute_rename_tags(hid_t fcpl, int type)
{
    ...
    if (type == TEST_SHMESG) {
         ...
         * cache.
         */
        if (!persistent_fsms && verify_tag(fid, H5AC_FSPACE_HDR_ID, H5AC__FREESPACE_TAG) < 0)
            TEST_ERROR;
        if (!persistent_fsms && verify_tag(fid, H5AC_FSPACE_SINFO_ID, H5AC__FREESPACE_TAG) < 0)
            TEST_ERROR;
...
check_object_info_tags(void)
{
    ...
    if (H5Oget_info_by_name2(fid, GROUPNAME, &oinfo, H5O_INFO_ALL, H5P_DEFAULT) < 0)
        TEST_ERROR;
