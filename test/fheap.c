...
#define DTABLE_MAX_DROWS(fh)                                                                                 \
    H5HF_get_dtable_max_drows_test(fh) /* Max. # of direct block rows in any indirect block */
...
static int del_objs(H5F_t *f, hid_t dxpl, H5HF_t **fh, fheap_test_param_t *tparam, fheap_heap_state_t *state,
 ...
 */
static int
add_obj(H5HF_t *fh, hid_t dxpl, size_t obj_off, size_t obj_size, fheap_heap_state_t *state,
        fheap_heap_ids_t *keep_ids)
{
 ...
 */
static int
reopen_file(hid_t *file, H5F_t **f, const char *filename, hid_t fapl, hid_t dxpl, H5HF_t **fh,
            haddr_t fh_addr, const fheap_test_param_t *tparam)
{
...
open_heap(char *filename, hid_t fapl, hid_t dxpl, const H5HF_create_t *cparam,
          const fheap_test_param_t *tparam, hid_t *file, H5F_t **f, H5HF_t **fh, haddr_t *fh_addr,
          fheap_heap_state_t *state, h5_stat_size_t *empty_size)
{
    size_t id_len; /* Size of fractal heap IDs */
 ...
 */
static int
reopen_heap(H5F_t *f, hid_t dxpl, H5HF_t **fh, haddr_t fh_addr, const fheap_test_param_t *tparam)
{
    /* Check for closing & re-opening the heap */
    if (tparam->reopen_heap) {
        /* Close (empty) heap */
 ...
 */
static int
close_heap(char *filename, hid_t fapl, hid_t dxpl, fheap_test_param_t *tparam, hid_t file, H5F_t *f,
           H5HF_t **fh, haddr_t fh_addr, fheap_heap_state_t *state, fheap_heap_ids_t *keep_ids,
 ...
 */
static int
del_objs(H5F_t *f, hid_t dxpl, H5HF_t **fh, fheap_test_param_t *tparam, fheap_heap_state_t *state,
         fheap_heap_ids_t *keep_ids)
{
    haddr_t fh_addr = HADDR_UNDEF; /* Address of fractal heap */
    size_t  id_len;                /* Size of fractal heap IDs */
 ...
 */
static int
fill_heap(H5HF_t *fh, hid_t dxpl, unsigned block_row, size_t obj_size, fheap_heap_state_t *state,
          fheap_heap_ids_t *keep_ids)
{
    unsigned char *wobj;         /* Buffer for object to insert */
 ...
 */
static int
fill_root_row(H5HF_t *fh, hid_t dxpl, unsigned row, size_t obj_size, fheap_heap_state_t *state,
              fheap_heap_ids_t *keep_ids)
{
 ...
 */
static int
fill_partial_row(H5HF_t *fh, hid_t dxpl, unsigned row, unsigned width, size_t obj_size,
 ...
 */
static int
fill_row(H5HF_t *fh, hid_t dxpl, unsigned row, size_t obj_size, fheap_heap_state_t *state,
         fheap_heap_ids_t *keep_ids)
{
    /* Sanity check */
    HDassert(fh);
 ...
 */
static int
fill_root_direct(H5HF_t *fh, hid_t dxpl, size_t obj_size, fheap_heap_state_t *state,
                 fheap_heap_ids_t *keep_ids)
{
 ...
 */
static int
fill_2nd_indirect(H5HF_t *fh, hid_t dxpl, unsigned pos, size_t obj_size, fheap_heap_state_t *state,
                  fheap_heap_ids_t *keep_ids)
{
    unsigned max_dblock_rows; /* Max. # of direct block rows in indirect block */
 ...
 */
static int
fill_all_direct(H5HF_t *fh, hid_t dxpl, size_t obj_size, fheap_heap_state_t *state,
                fheap_heap_ids_t *keep_ids)
{
    unsigned max_dblock_rows; /* Max. # of direct block rows in indirect block */
    unsigned row;             /* Row being created */
 ...
 */
static int
fill_2nd_indirect_row(H5HF_t *fh, hid_t dxpl, unsigned pos, size_t obj_size, fheap_heap_state_t *state,
                      fheap_heap_ids_t *keep_ids)
{
    unsigned width; /* Width of heap's doubling table */
    unsigned u;     /* Local index variable */
 ...
 */
static int
fill_all_2nd_indirect_rows(H5HF_t *fh, hid_t dxpl, size_t obj_size, fheap_heap_state_t *state,
                           fheap_heap_ids_t *keep_ids)
{
    unsigned width; /* Width of heap's doubling table */
    unsigned u;     /* Local index variable */
 ...
 */
static int
fill_3rd_indirect(H5HF_t *fh, hid_t dxpl, unsigned pos, size_t obj_size, fheap_heap_state_t *state,
                  fheap_heap_ids_t *keep_ids)
{
    unsigned u; /* Local index variable */
 ...
 */
static int
fill_3rd_indirect_row(H5HF_t *fh, hid_t dxpl, unsigned pos, size_t obj_size, fheap_heap_state_t *state,
                      fheap_heap_ids_t *keep_ids)
{
    unsigned width; /* Width of heap's doubling table */
    unsigned u;     /* Local index variable */
 ...
 */
static int
fill_all_3rd_indirect_rows(H5HF_t *fh, hid_t dxpl, size_t obj_size, fheap_heap_state_t *state,
                           fheap_heap_ids_t *keep_ids)
{
    unsigned width; /* Width of heap's doubling table */
    unsigned u;     /* Local index variable */
 ...
 */
static int
fill_4th_indirect_row(H5HF_t *fh, hid_t dxpl, unsigned pos, size_t obj_size, fheap_heap_state_t *state,
                      fheap_heap_ids_t *keep_ids)
{
    unsigned width; /* Width of heap's doubling table */
 ...
 */
static int
fill_all_4th_indirect_rows(H5HF_t *fh, hid_t dxpl, size_t obj_size, fheap_heap_state_t *state,
                           fheap_heap_ids_t *keep_ids)
{
    unsigned width; /* Width of heap's doubling table */
    unsigned u;     /* Local index variable */
                                                                          ...
                                                                          fheap_test_param_t *tparam)
{
    ...
    if (fill_partial_row(fh, dxpl, num_first_indirect_rows, cparam->managed.width - 1, fill_size, &state,
                         &keep_ids))
        TEST_ERROR
...
test_filtered_huge(hid_t fapl, H5HF_create_t *cparam, fheap_test_param_t *tparam)
{
    ...
    /* Perform common file & heap open operations */
    if (open_heap(filename, fapl, dxpl, &tmp_cparam, tparam, &file, &f, &fh, &fh_addr, &state, &empty_size) <
        0)
        TEST_ERROR
...
/* tparam.drain_half = FHEAP_DEL_DRAIN_HALF; */
            ...
            for (id_len = 0; id_len < 3; id_len++) {
                ...
                switch (id_len) {
                    ...
                    case 1:
                        ...
                        /* Adjust actual length of heap IDs for directly storing 'huge' object's file offset &
                         * length in heap ID */
                    ...
                    case 2:
                        ...
                        puts("Using 'direct' heap ID format for 'huge' objects and larger IDs for 'tiny' "
                             "objects");
    ...
    HDfprintf(stderr, "Uncomment tests!\n");
        ...
        /* Try several different methods of deleting objects */
        {
            ...
            for (del_dir = FHEAP_DEL_FORWARD; del_dir < FHEAP_DEL_NDIRS; del_dir++) {
                ...
                /* Controlled tests */
                /* XXX: Re-enable file size checks in these tests, after the file has persistent free space
                 * tracking working */
    ...
    HDfprintf(stderr, "Uncomment tests!\n");
        ...
        if (ExpressMode > 1)
            ...
            tparam.del_dir = FHEAP_DEL_FORWARD;
            nerrors += test_random(
                (curr_test == FHEAP_TEST_NORMAL ? (hsize_t)(100 * 1000 * 1000) : (hsize_t)(50 * 1000 * 1000)),
                fapl, &small_cparam, &tparam);
            nerrors += test_random_pow2(
                (curr_test == FHEAP_TEST_NORMAL ? (hsize_t)(100 * 1000 * 1000) : (hsize_t)(4 * 1000 * 1000)),
                fapl, &small_cparam, &tparam);
