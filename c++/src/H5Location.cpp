...
//--------------------------------------------------------------------------
// Function:	H5Location constructor
///\brief	Dereferences into an HDF5 object, given an HDF5 location.
///\param	loc - IN: H5Location instance to copy
///\param	ref - IN: Name of the object to be referenced
///\param	ref_type - IN: Dataspace with selection
// Programmer	Binh-Minh Ribler - 2000
//--------------------------------------------------------------------------
        // Dereferences into an HDF5 object, given an HDF5 location.
 /* H5Location::H5Location(const H5Location& loc, const void* ref, H5R_type_t ref_type, const PropList& plist)
{
    hid_t temp_id;
    try {
        temp_id = p_dereference(loc.getId(), ref, ref_type, plist);
    }
    catch (ReferenceException deref_error) {
        throw ReferenceException("H5Location constructor - by dereferenced",
...
void H5Location::setComment(const char* name, const char* comment) const
{
   ...
   if( ret_value < 0 )
      throw Exception("setComment", "H5Oset_comment_by_name failed");
}
...
void H5Location::removeComment(const char* name) const
{
   ...
   if( ret_value < 0 )
      throw Exception("removeComment", "H5Oset_comment_by_name failed");
}
...
H5std_string H5Location::getComment(const char* name, size_t bufsize) const
{
   ...
   if (ret_value < 0) {
       delete []comment_C;
       throw Exception("getComment", "H5Oget_comment_by_name failed");
   }
...
//--------------------------------------------------------------------------
hid_t H5Location::p_dereference(hid_t loc_id, const void* ref, H5R_type_t ref_type, const PropList& plist)
{
   hid_t plist_id;
   ...
   hid_t temp_id;
   temp_id = H5Rdereference2(loc_id, plist_id, ref_type, ref);
   if (temp_id < 0)
   {
...
void H5Location::dereference(const Attribute& attr, const void* ref, H5R_type_t ref_type, const PropList& plist)
{
   hid_t temp_id;
   try {
      temp_id = p_dereference(attr.getId(), ref, ref_type, plist);
   }
   catch (ReferenceException E) {
...
DataSpace H5Location::getRegion(void *ref, H5R_type_t ref_type) const
{
   ...
   try {
      ...
      throw ReferenceException("H5Location::getRegion", E.getDetailMsg());
   }
}
