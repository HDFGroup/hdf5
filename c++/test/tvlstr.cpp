    ...
    try {
        ...
        for (i = 0; i < SPACE1_DIM1; i++) {
            if (HDstrlen(wdata[i]) != strlen(rdata[i])) {
                TestErrPrintf("VL data length don't match!, strlen(wdata[%d])=%u, strlen(rdata[%d])=%u\n",
                              static_cast<int>(i), static_cast<unsigned>(strlen(wdata[i])),
                              static_cast<int>(i), static_cast<unsigned>(strlen(rdata[i])));
                continue;
            }
