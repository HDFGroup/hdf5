		  ...
		  const void UNUSED *_rt_key, void *_udata)
{
    ...
    for (i=0, ret_value=H5B_ITER_CONT; i<nsyms && !ret_value; i++) {
        if (bt_udata->skip>0) {
            ...
            if (H5HL_unprotect(f, dxpl_id, heap, bt_udata->stab->heap_addr) < 0)
    ...
    if (heap && H5HL_unprotect(f, dxpl_id, heap, bt_udata->stab->heap_addr) < 0)
		  ...
		  const void UNUSED *_rt_key, void *_udata)
{
    ...
    if(bt_udata->idx >= bt_udata->num_objs && bt_udata->idx < (bt_udata->num_objs+sn->nsyms)) {
        ...
        if (H5HL_unprotect(f, dxpl_id, heap, bt_udata->mesg->heap_addr) < 0)
            HGOTO_ERROR(H5E_SYM, H5E_PROTECT, H5B_ITER_ERROR, "unable to unprotect symbol name");
        heap=NULL; name=NULL;
