...
/* XXX: Change to using free-list factories */
    ...
    if(hdr->filter_len > 0) {
        ...
        if(H5Z_pipeline(&(hdr->pline), H5Z_FLAG_REVERSE, &filter_mask, H5Z_ENABLE_EDC, filter_cb, &nbytes, &read_size, &read_buf, FALSE, dxpl_id) < 0)
    ...
    if(dblock->cache_info.is_dirty) {
        ...
        *p++ = H5HF_DBLOCK_VERSION;
        ...
        if(hdr->filter_len > 0) {
            ...
            /* Push direct block data through I/O filter pipeline */
            nbytes = write_size;
            if(H5Z_pipeline(&(hdr->pline), 0, &filter_mask, H5Z_ENABLE_EDC, filter_cb, &nbytes, &write_size, &write_buf, FALSE, dxpl_id) < 0)
