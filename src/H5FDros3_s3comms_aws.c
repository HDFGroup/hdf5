...
/* TODO: consider deprecating h5dump and other tools options for specifying S3 creds */
...
static herr_t H5FD__s3comms_get_aws_region(const H5FD_ros3_fapl_t *fa, char **aws_region_out);

static herr_t H5FD__s3comms_format_http_request_message(H5FD__s3comms_aws_params_t *aws_params,
                                                        const char *HTTP_method, const char *bucket_name,
                                                        const char               *object_key,
                                                        struct aws_http_message **message_out);
static herr_t H5FD__s3comms_format_host_header(H5FD__s3comms_aws_params_t *aws_params,
                                               struct aws_http_message *message, const char *bucket_name,
                                               const char *host_name, const char *port);
static herr_t H5FD__s3comms_format_range_header(H5FD__s3comms_aws_params_t *aws_params,
                                                struct aws_http_message *message, haddr_t offset, size_t len);
static herr_t H5FD__s3comms_format_user_agent_header(H5FD__s3comms_aws_params_t *aws_params,
...
H5FD__s3comms_init(void)
{
...
#else
    if (getenv(HDF5_ROS3_VFD_DEBUG)) /* TODO: better semantics around enabling debugging */
        H5FD_ros3_debug_g = true;
#endif
     ...
     */
    /* TODO: Look for any different solution here */
 ...
 *              issues when the aws-c-s3 library is cleaned up from HDF5's
 *              default atexit() handler. A better solution would be to
 *              require a call to H5close() to explicitly terminate access to
 *              HDF5 before exiting main(), but this will suffice for the
 *              time being.
 *
 * Return:      Non-negative on success/Negative on failure
 *----------------------------------------------------------------------------
...
H5FD__s3comms_term(void)
{
    ...
    /* Currently handled by atexit() function above to work around cleanup ordering issues. */
...
H5FD__s3comms_s3r_open(const char *url, const H5FD_ros3_fapl_t *fa, const char *fapl_token)
{
    ...
    /* Setup AWS region */
    if (H5FD__s3comms_get_aws_region(fa, &handle->aws_region) < 0)
    ...
    if (!handle->aws_region)
        HGOTO_ERROR(H5E_VFL, H5E_BADVALUE, NULL,
                    "AWS region wasn't specified"); /* TODO: Document that region through FAPL or
                                                       environment variable is now required */

    H5_GCC_CLANG_DIAG_OFF("aggregate-return")
    region_cursor = aws_byte_cursor_from_c_str(handle->aws_region);
    H5_GCC_CLANG_DIAG_ON("aggregate-return")

    /* Setup credentials provider for authentication */
    /* TODO: document in release note about how credentials are found this way
     * Environment - pays attention to AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY,
     * AWS_SESSION_TOKEN and AWS_ACCOUNT_ID environment variables
     * Profile - reads from ~/.aws/config and ~/.aws/credentials by default,
     * can override with AWS_CONFIG_FILE and AWS_SHARED_CREDENTIALS_FILE
     * environment variables
     */
    if (fa->authenticate) {
        /* If authentication parameters are in FAPL use those, otherwise
         * let the AWS library figure it out.
         */
        if (fa->secret_id[0] != '\0' && fa->secret_key[0] != '\0') {
             ...
             */
            credentials_provider_opts.bootstrap = client_bootstrap;
            credentials_provider =
        ...
        /* Use anonymous credential provider if not authenticating */
...
H5FD__s3comms_s3r_read(s3r_t *handle, haddr_t offset, size_t len, void *dest, size_t dest_size)
{
    ...
    if (offset + len > handle->filesize)
        HGOTO_ERROR(H5E_VFL, H5E_BADVALUE, FAIL, "unable to read past EOF");
    ...
    /*
     * Wait for request to complete. For now, there's not much else
     * to do while waiting, so just busy-wait. However, this can
     * likely be done more efficiently.
     */
    while (!read_params.request.finished)
        ;
...
H5FD__s3comms_s3r_getsize(s3r_t *handle)
{
     ...
     * likely be done more efficiently.
     */
    while (!getsize_params.request.finished)
        ;
...
H5FD__s3comms_parse_url(s3r_t *handle, const char *url)
{
    ...
    if (is_s3_url) {
        if (NULL == (purl->bucket_name = H5MM_strndup((char *)aws_params->parsed_uri.host_name.ptr,
        ...
        if (NULL == (purl->key = H5MM_strndup((char *)aws_params->parsed_uri.path.ptr + 1,
 ...
 *----------------------------------------------------------------------------
 */
/* TODO: document that AWS_REGION + AWS_DEFAULT_REGION are paid attention to and
 * maybe add config file parsing
 */
static herr_t
H5FD__s3comms_get_aws_region(const H5FD_ros3_fapl_t *fa, char **aws_region_out)
{
    ...
    if (fa->aws_region[0] != '\0') {
        if (NULL == (region_copy = strdup(fa->aws_region)))
            HGOTO_ERROR(H5E_VFL, H5E_BADVALUE, FAIL, "could not copy AWS region from FAPL");
    }
    else {
        char *env_region = NULL;

        env_region = getenv("AWS_REGION");
        if (env_region && (*env_region != '\0')) {
            if (NULL == (region_copy = strdup(env_region)))
                HGOTO_ERROR(H5E_VFL, H5E_BADVALUE, FAIL,
                            "could not copy AWS region string from environment variable");
        }
        else {
            char *env_default_region = NULL;

            env_default_region = getenv("AWS_DEFAULT_REGION");
            if (env_default_region && (*env_default_region != '\0')) {
                if (NULL == (region_copy = strdup(env_default_region)))
                    HGOTO_ERROR(H5E_VFL, H5E_BADVALUE, FAIL,
                                "could not copy default AWS region string from environment variable");
            }
        }
    }
 ...
 */
static herr_t
H5FD__s3comms_format_http_request_message(H5FD__s3comms_aws_params_t *aws_params, const char *HTTP_method,
                                          const char *bucket_name, const char *object_key,
                                          struct aws_http_message **message_out)
{
    struct aws_http_message *request_http_message = NULL;
 ...
 */
static herr_t
H5FD__s3comms_format_host_header(H5FD__s3comms_aws_params_t *aws_params, struct aws_http_message *message,
                                 const char *bucket_name, const char *host_name, const char *port)
{
 ...
 */
static herr_t
H5FD__s3comms_format_range_header(H5FD__s3comms_aws_params_t *aws_params, struct aws_http_message *message,
                                  haddr_t offset, size_t len)
{
 ...
 */
static herr_t
H5FD__s3comms_format_user_agent_header(H5FD__s3comms_aws_params_t *aws_params,
                                       struct aws_http_message    *message)
{
    struct aws_http_header user_agent_header = {0};
    int                    ret               = 0;
    char                   platform_str[S3COMMS_MAX_RANGE_STRING_SIZE];   /* TODO: Different macro */
    char                   user_agent_str[S3COMMS_MAX_RANGE_STRING_SIZE]; /* TODO: Different macro */
    herr_t                 ret_value = SUCCEED;
    ...
    ret = snprintf(platform_str, sizeof(platform_str),
                   "TODO: PLATFORM_INFO"); /* TODO: Determine platform info? */

    if (ret < 0)
        HGOTO_ERROR(H5E_VFL, H5E_SYSTEM, FAIL, "snprintf error");
    if ((size_t)ret >= sizeof(platform_str))
        HGOTO_ERROR(H5E_VFL, H5E_BADVALUE, FAIL, "unable to format HTTP User-Agent value");

    ret = snprintf(user_agent_str, sizeof(user_agent_str), "libhdf5/%u.%u.%u (%s; vfd:ros3)", H5_VERS_MAJOR,
                   H5_VERS_MINOR, H5_VERS_RELEASE, platform_str);
