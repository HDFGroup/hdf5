...
extern herr_t H5FD__write_vector_internal(hid_t h5_fid, hssize_t count, haddr_t addrs[], hsize_t sizes[],
                                          void *bufs[] /* data_in */);
extern herr_t H5FD__read_vector_internal(hid_t h5_fid, hssize_t count, haddr_t addrs[], hsize_t sizes[],
                                         void *bufs[] /* data_out */);
...
H5FD__ioc_open(const char *name, unsigned flags, hid_t ioc_fapl_id, haddr_t maxaddr)
{
    ...
    file_ptr->fa.common.ioc_fapl_id = H5I_INVALID_HID;
...
#if 0 /* JRM */ /* it seems we are decrementing the ref-count on the ioc fapl elsewhere. */
                /* thus this is not needed at present */
  /* if set, close the copy of the plist for the underlying VFD. */
  if ( (H5I_INVALID_HID != file->fa.common.ioc_fapl_id) &&
       (H5I_dec_ref(file->fa.common.ioc_fapl_id) < 0) )
...
static herr_t
H5FD__ioc_ctl(H5FD_t *file, uint64_t op_code, uint64_t flags, const void *input, void **result)
{
    herr_t ret_value = SUCCEED; /* Return value */
