...
H5A__dense_create(H5F_t *f, H5O_ainfo_t *ainfo)
{
        ...
        if (ainfo->index_corder) {
            ...
            if (NULL == (bt2_corder = H5B2_create(f, &bt2_cparam, NULL)))
                HGOTO_ERROR(H5E_ATTR, H5E_CANTINIT, FAIL,
                            "unable to create v2 B-tree for creation order index");

            /* Retrieve the v2 B-tree's address in the file */
            if (H5B2_get_addr(bt2_corder, &ainfo->corder_bt2_addr) < 0)
                HGOTO_ERROR(H5E_ATTR, H5E_CANTGET, FAIL,
                            "can't get v2 B-tree address for creation order index");
        } /* end if */
     ...
     *-------------------------------------------------------------------------
     */
    static herr_t H5A__dense_fnd_cb(const H5A_t *attr, bool *took_ownership, void *_user_attr)
    {
        ...
        if (attr_sharable) {
            ...
            if ((shared_mesg = H5O_msg_is_shared(H5O_ATTR_ID, attr)) < 0)
                ...
                if (H5SM_try_share(f, NULL, 0, H5O_ATTR_ID, attr, &mesg_flags) < 0)
                    HGOTO_ERROR(H5E_ATTR, H5E_WRITEERROR, FAIL,
                                "error determining if message should be shared");
     ...
     *-------------------------------------------------------------------------
     */
    static herr_t H5A__dense_write_bt2_cb2(void *_record, void *_op_data, bool *changed)
    {
     ...
     *-------------------------------------------------------------------------
     */
    static herr_t H5A__dense_write_bt2_cb(void *_record, void *_op_data, bool *changed)
    {
...
/* Sanity check */
     ...
     *-------------------------------------------------------------------------
     */
    static herr_t H5A__dense_copy_fh_cb(const void *obj, size_t obj_len, void *_udata)
    {
     ...
     *-------------------------------------------------------------------------
     */
    static int H5A__dense_iterate_bt2_cb(const void *_record, void *_bt2_udata)
    {
     ...
     *-------------------------------------------------------------------------
     */
    herr_t H5A__dense_iterate(H5F_t * f, hid_t loc_id, const H5O_ainfo_t *ainfo, H5_index_t idx_type,
     ...
     *-------------------------------------------------------------------------
     */
    static herr_t H5A__dense_remove_bt2_cb(const void *_record, void *_udata)
    {
    ...
    herr_t H5A__dense_remove(H5F_t * f, const H5O_ainfo_t *ainfo, const char *name)
    {
        ...
        if (H5B2_remove(bt2_name, &udata, H5A__dense_remove_bt2_cb, &udata) < 0)
            HGOTO_ERROR(H5E_ATTR, H5E_CANTREMOVE, FAIL,
                        "unable to remove attribute from name index v2 B-tree");
    ...
    static herr_t H5A__dense_remove_by_idx_bt2_cb(const void *_record, void *_bt2_udata)
    {
        ...
        H5B2_t                         *bt2 = NULL; /* v2 B-tree handle for index */
        const H5A_dense_bt2_name_rec_t *record =
            (const H5A_dense_bt2_name_rec_t *)_record;                  /* v2 B-tree record */
     ...
     *-------------------------------------------------------------------------
     */
    herr_t H5A__dense_remove_by_idx(H5F_t * f, const H5O_ainfo_t *ainfo, H5_index_t idx_type,
                                    H5_iter_order_t order, hsize_t n)
    {
        ...
        if (H5_addr_defined(bt2_addr)) {
            ...
            if (H5B2_remove_by_idx(bt2, order, n, H5A__dense_remove_by_idx_bt2_cb, &udata) < 0)
                HGOTO_ERROR(H5E_ATTR, H5E_CANTREMOVE, FAIL,
                            "unable to remove attribute from v2 B-tree index");
        } /* end if */
        else {
            /* Build the table of attributes for this object */
            /* (build table using the name index, but sort according to idx_type) */
     ...
     *-------------------------------------------------------------------------
     */
    herr_t H5A__dense_exists(H5F_t * f, const H5O_ainfo_t *ainfo, const char *name, bool *attr_exists)
    {
        H5A_bt2_ud_common_t udata;               /* User data for v2 B-tree modify */
        H5HF_t             *fheap        = NULL; /* Fractal heap handle */
        H5HF_t             *shared_fheap = NULL; /* Fractal heap handle for shared header messages */
