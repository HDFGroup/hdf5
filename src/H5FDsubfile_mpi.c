...
int
init__indep_io(void *_sf_context, size_t maxdepth, int ioc_total, int64_t *sf_source_data_offset,
               int64_t *sf_datasize, int64_t *sf_offset, int *first_index, int *n_containers, int64_t offset,
    ...
    if (subfile_rank >= 0) {
        ...
        if (sf_context->sf_fid < 0) {
            ...
            if ((prefix = sf_context->subfile_prefix)) {
                ...
                subfile_dir = strrchr(filepath, '/');
                assert(subfile_dir);
                sprintf(subfile_dir + 1, SF_FILENAME_TEMPLATE, h5_file_id, subfile_rank, n_io_concentrators);
            }
