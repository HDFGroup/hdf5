...
typedef struct H5AC_cache_config_t {
    ...
    hbool_t  rpt_fcn_enabled;
             /**<IN: Boolean flag indicating whether the adaptive cache resize
              * report function is enabled. This field should almost always be
              * set to disabled (0). Since resize algorithm activity is reported
              * via stdout, it MUST be set to disabled (0) on Windows machines.\n
              * The report function is not supported code, and can be expected
              * to change between versions of the library. Use it at your own
              * risk. */
    ...
    hbool_t  close_trace_file;
              ...
              * See the above comments on the \ref
              * H5AC_cache_config_t.open_trace_file * "open_trace_file" field.
              * This field should be set to 0 unless there is an open trace
              * file on the cache that you wish to close.\n
              * The trace file feature is unsupported unless used at the
              * direction of The HDF Group. It is intended to allow The HDF
              * Group to collect a trace of cache activity in cases of occult
              * failures and/or poor performance seen in the field, so as to
              * aid in reproduction in the lab. If you use it absent the
              * direction of The HDF Group, you are on your own. */
    char     trace_file_name[H5AC__MAX_TRACE_FILE_NAME_LEN + 1];
             /**<IN: Full path of the trace file to be opened
              * if the \ref H5AC_cache_config_t.open_trace_file
              * "open_trace_file" field is set to 1.\n
              * In the parallel case, an ascii representation of the MPI rank
              * of the process will be appended to the file name to yield a
              * unique trace file name for each process.\n
    ...
    hbool_t  evictions_enabled;
             /**<IN: A boolean flag indicating whether evictions from the
              * metadata cache are enabled. This flag is initially set to
              * enabled (1).\n
              * In rare circumstances, the raw data throughput quirements may
              * be so high that the user wishes to postpone metadata writes so
              * as to reserve I/O throughput for raw data. The \p
              * evictions_enabled field exists to allow this. However, this is
              * an extreme step, and you have no business doing it unless you
              * have read the User Guide section on metadata caching, and
              ...
              * When this flag is set to disabled (\c 0), the metadata cache
              * will not attempt to evict entries to make space for new entries,
              * and thus will grow without bound.\n
    ...
    size_t   initial_size;
             /**<IN: If \ref H5AC_cache_config_t.set_initial_size
              * "set_initial_size" is set to 1, \p initial_size must contain
              * the desired initial size in bytes. This value must lie in the
              * closed interval [ \p min_size, \p max_size ]. (see below) */

    double   min_clean_fraction;
             /**<IN: This field specifies the minimum fraction of the cache
              * that must be kept either clean or empty.\n
              * The value must lie in the interval [0.0, 1.0]. 0.01 is a good
              * place to start in the serial case. In the parallel case, a
              * larger value is needed -- see the overview of the metadata cache
              * in the “Metadata Caching in HDF5” section of the --
              * <em>HDF5 User’s Guide</em> for details. */

    size_t   max_size;
             /**<IN: Upper bound (in bytes) on the range of values that the
              * adaptive cache resize code can select as the maximum cache
              * size. */
    size_t   min_size;
             /**<IN: Lower bound (in bytes) on the range of values that the
              * adaptive cache resize code can select as the mininum cache
              * size. */
    ...
    enum H5C_cache_incr_mode       incr_mode;
                                   /**<IN: Enumerated value indicating the
                                    * operational mode of the automatic cache
                                    * size increase code. At present, only two
                                    * values listed in #H5C_cache_incr_mode are
                                    * legal. */

    double                         lower_hr_threshold;
                                   /**<IN: Hit rate threshold used by the hit
                                    * rate threshold cache size increment
                                    * algorithm.\n
                                    * * When the hit rate over an epoch is below
                                    * this threshold and the cache is full, the
                                    * maximum size of the cache is multiplied by
                                    * increment (below), and then clipped as
                                    * necessary to stay within max_size, and
                                    * possibly max_increment.\n
                                    * This field must lie in the interval
                                    * [0.0, 1.0]. 0.8 or 0.9 is a good place to
                                    * start. */

    double                         increment;
                                   /**<IN: Factor by which the hit rate
                                    * threshold cache size increment algorithm
                                    * multiplies the current cache max size to
                                    * obtain a tentative new cache
                                    * size.\n
                                    * The actual cache size increase will be
                                    * clipped to satisfy the max_size specified
                                    * in the general configuration, and
                                    * possibly max_increment below.\n
                                    * The parameter must be greater than or
                                    * equal to 1.0 -- 2.0 is a reasonable
                                    * value.\n
                                    * If you set it to 1.0, you will
                                    * effectively disable cache size increases.
                                    * */

    hbool_t                        apply_max_increment;
                                   /**<IN: Boolean flag indicating whether an
                                     * upper limit should be applied to the size
                                     * of cache size increases. */
    size_t                         max_increment;
                                   /**<IN: Maximum number of bytes by which
                                    * cache size can be increased in a single
                                    * step -- if applicable. */

    enum H5C_cache_flash_incr_mode flash_incr_mode;
                                   /**<IN: Enumerated value indicating the
                                    * operational mode of the flash cache size
                                    * increase code. At present, only two listed
                                    * values in #H5C_cache_flash_incr_mode
                                    * are legal.*/
    double                         flash_multiple;
                                   /**<IN: The factor by which the size of the
                                    * triggering entry / entry size increase
                                    * is multiplied to obtain the initial cache
                                    * size increment. This increment may be
                                    * reduced to reflect existing free space in
                                    * the cache and the \p max_size field
                                    * above.\n
                                    * The parameter must lie in the interval
                                    * [0.0, 1.0]. 0.1 or 0.05 is a good place
                                    * to start.\n
                                    * At present, this field must lie in the
                                    * range [0.1, 10.0]. */
    double                         flash_threshold;
                                   /**<IN: The factor by which the current
                                    * maximum cache size is multiplied to
                                    * obtain the minimum size entry / entry size
                                    * increase which may trigger a flash cache
                                    * size increase. \n
                                    * At present, this value must lie in the
                                    * range [0.1, 1.0]. */
    ...
    enum H5C_cache_decr_mode decr_mode;
                              ...
                              * mode of the tomatic cache size decrease code.
                              * At present, the values listed in
                              * #H5C_cache_decr_mode are legal.*/

    double                   upper_hr_threshold;
                             /**<IN: Hit rate threshold for the hit rate
                              * threshold and ageout with hit rate threshold
                              * cache size decrement algorithms.\n
                              * When \p decr_mode is #H5C_decr__threshold, and
                              * the hit rate over a given epoch exceeds the
                              * supplied threshold, the current maximum cache
                              * size is multiplied by decrement to obtain a
                              * tentative new (and smaller) maximum cache
                              * size.\n
                              * When \p decr_mode is
                              * #H5C_decr__age_out_with_threshold, there is
                              * no attempt to find and evict aged out entries
                              * unless the hit rate in the previous epoch
                              * exceeded the supplied threshold.\n
                              * This field must lie in the interval
                              * [0.0, 1.0].\n
                              * For #H5C_incr__threshold, .9995 or .99995 is a
                              * good place to start.\n
                              * For #H5C_decr__age_out_with_threshold, .999
                              * might be more useful.*/

    double                   decrement;
                             /**<IN: In the hit rate threshold cache size
                              * decrease algorithm, this parameter contains the
                              * factor by which the current max cache size is
                              * multiplied to produce a tentative new cache
                              * size. \n
                              * The actual cache size decrease will be clipped
                              * to satisfy the
                              * \ref H5AC_cache_config_t.min_size
                              * "min_size" specified in the general
                              * configuration, and possibly
                              * \ref H5AC_cache_config_t.max_decrement
                              * "max_decrement".\n
                              * The parameter must be be in the interval
                              * [0.0, 1.0].\n
                              * If you set it to 1.0, you will effectively
                              * disable cache size decreases. 0.9 is a
                              * reasonable starting point. */

    hbool_t                  apply_max_decrement;
                             /**<IN: Boolean flag indicating ether an upper
                              * limit should be applied to the size of
                              * cache size decreases. */
    size_t                   max_decrement;
                             /**<IN: Maximum number of bytes by which the
                              * maximum cache size can be decreased in any
                              * single step -- if applicable.*/

    int                      epochs_before_eviction;
                             /**<IN: In the ageout based cache size reduction
                              * algorithms, this field contains the minimum
                              * number of epochs an entry must remain unaccessed
                              * in cache before the cache size reduction
                              * algorithm tries to evict it. 3 is a reasonable
                              * value. */

    hbool_t                  apply_empty_reserve;
                             /**<IN: Boolean flag indicating whether the ageout
                              * based decrement algorithms will maintain a empty
                              * reserve when decreasing cache size. */
    double                   empty_reserve;
                             /**<IN: Empty reserve as a fraction maximum cache
                              * size if applicable.\n
                              * When so directed, the ageout based algorithms
                              * will not decrease the maximum cache size unless
                              * the empty reserve can be met.\n
                              * The parameter must lie in the interval
                              * [0.0, 1.0]. 0.1 or 0.05 is a good place to
                              * start. */
    ...
    size_t dirty_bytes_threshold;
            ...
            * triggering synchronizations of the metadata caches serving the
            * target file in the parallel case.\n
            * Synchronization occurs whenever the number of bytes of dirty
            * metadata created since the last synchronization exceeds this
            * limit.\n
    ...
    int    metadata_write_strategy;
            /**<IN: Desired metadata write strategy. The valid values for this
             *field are:\n
             * #H5AC_METADATA_WRITE_STRATEGY__PROCESS_0_ONLY: Specifies that
             * only process zero is allowed to write dirty metadata to disk.\n
             * #H5AC_METADATA_WRITE_STRATEGY__DISTRIBUTED: Specifies that
             * process zero still makes the decisions as to what entries should
             * be flushed, but the actual flushes are distributed across the
             * processes in the computation to the extent possible.\n
             * The src/H5ACpublic.h include file in the HDF5 library has
