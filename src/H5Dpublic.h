 ...
 * \return \hid_t{dataset}
 *
 * \details H5Dcreate_anon() creates a dataset in the file specified by
 * \p loc_id.
 *
 * \p loc_id may be a file, group, dataset, named datatype or attribute.
 * If an attribute, dataset, or named datatype is specified for \p loc_id
 ...
 * specified by \p dset_id. The function returns an identifier for the
 ...
 * the dataset creation property list associated with the dataset specified
 * by \p dset_id.
 *
...
/**\ingroup H5D
 *
 * \brief Returns the dataset access property list associated with a dataset
 *
 * \dset_id
 *
 * \return \hid_t{dataset access property list}
 *
 * \details H5Dget_access_plist() returns a copy of the dataset access
 * property list used to open the specified dataset. Modifications to
 * the returned property list will have no effect on the dataset it was
 * retrieved from.
 *
 * The chunk cache parameters in the returned property lists will be
 * those * used by the dataset, \p dset_id. If the properties in the file
 * access property list were used to determine the dataset's chunk cache
 ...
 * layout, then the chunk cache properties will be set to the default. The
 * chunk cache properties in the returned list are considered to be
 ...
 * default values.
 *
 * The access property list identifier should be released with H5Pclose()
 * when the identifier is no longer needed so that resource leaks will
 * not develop.
 *
 * \since 1.8.3
 *
 ...
 * \note The returned allocated size does not include that of the metadata.  
 * \note H5Dget_storage_size() does not differentiate between 0 (zero),
 * the value returned for the storage size of a dataset with no stored
 * values, and 0 (zero), the value returned to indicate an error.
 *
 * \details H5Dget_storage_size() returns the amount of storage, in bytes,
 * that is allocated in the file for the raw data of the dataset specified
 * by dataset_id.
 *
 * Note that the amount of storage in this case is the storage allocated
 * in the written file, which will typically differ from the space required
 * to hold a dataset in working memory.
 *
 * - For contiguous datasets, the returned size equals the current allocated
 * size of the raw data.
 *
 * - For unfiltered chunked datasets, the returned size is the number of
 * allocated chunks times the chunk size.
 *
 * - For filtered chunked datasets, the returned size is the space required
 * to store the filtered data. For example, if a compression filter is in
 * use, H5Dget_storage_size() returns the total space required to store the
 * compressed chunks.
 *
 * H5Dget_storage_size() reports only the space required to store the data;
 * the report does not include any metadata.
 *
 * The return value may be zero if no data has been stored.
 *
 * \note Note that H5Dget_storage_size() is not generally an appropriate
 * function to use when determining the amount of memory required to
 * work with a dataset. In such circumstances, you must determine the
 * number of data points in a dataset and the size of an individual data
 * element. H5Sget_simple_extent_npoints() and H5Tget_size() can be used to
 ...
 * \param[out] chunk_nbytes The size in bytes for the chunk
 *
 * \return \herr_t
 *
 * \details H5Dget_chunk_storage_size() returns the size in bytes
 * allocated in the file for a raw data chunk as specified by its
 * logical \p offset in the dataset \p dset_id. The size is returned in
 * \p chunk_nbytes. It is the size of the compressed data if the chunk is
 * filtered and the size may be zero if no storage is allocated yet for
 * the dataset.
 *
 * \since 1.10.2
 *
 ...
 * \return \herr_t
 *
 * \details H5Dget_num_chunks() retrieves the number of chunks nchunks
 * in a set of selected elements specified by \p fspace_id for a dataset
 * specified by the identifier \p dset_id. If \p fspace_id is #H5S_ALL, the
 * function will retrieve the total number of chunks stored for the dataset.
 *
 * \note Please be aware that this function currently does not support
 * non-trivial selections, thus \p fspace_id has no effect. Also, the
 * implementation does not handle the #H5S_ALL macro as an oversight.
 * As a workaround, application can get the dataspace for the dataset
 * using H5Dget_space() and pass that in for \p fspace_id.  This will be
 * fixed in coming releases.
 *
 * \since 1.10.5
 *
 *-------------------------------------------------------------------------
 ...
 *
 * \dset_id
 * \param[in]  offset   Pointer to a one-dimensional array with a size equal
 *  to the dataset’s rank. Each element is the logical position of the chunk’s
 *  first element in a dimension.
 ...
 * \return \herr_t
 *
 * \details H5Dget_chunk_info_by_coord() retrieves the filter mask, \p
 * filter_mask, size, \p size, and address, \p addr for a chunk in the dataset
 * specified by the dataset identifier \p dset_id, using the coordinates
 * specified by \p offset.
 *
 * If the queried chunk does not exist in the file, \p size will be set
 * to 0, \p address to #HADDR_UNDEF, and the buffer \p filter_mask will
 * not be modified.
 *
 * \note Please be aware that in the HDF5-1.10.5 release, the file
 * space identifier does not (yet) support non-trivial selections. The
 * implementation also does not currently support the #H5S_ALL macro.
 * As a workaround, get the dataspace for the dataset (H5Dget_space()),
 * select all elements in the dataspace (H5Sselect_all()) and pass that in
 * for \p fspace_id.
 * NULL can be passed in for any OUT parameters.
 *
 * \since 1.10.5
 *
 *-------------------------------------------------------------------------
 ...
 * \param[in]  fspace_id   File dataspace selection identifier; H5S_ALL if the
 * selection is the current extent of the dataset (See Note below)
 * \param[in]  chk_idx    Chunk index in the selection. Index value may have a value of 0 up to the number of chunks stored in the file that have a nonempty intersection with the file dataspace selection
 * \param[out]  offset   Logical position of the chunk’s first element in the dataspace
 * \param[out] filter_mask Indicating filters used with the chunk when written
 * \param[out] addr Chunk address in the file
 * \param[out] size Chunk size in bytes, 0 if chunk doesn’t exist
 *
 * \return \herr_t
 *
 * \details H5Dget_chunk_info() retrieves the offset coordinates offset,
 * filter mask filter_mask, size size and address addr for the dataset
 * specified by the identifier dset_id and the chunk specified by the
 * index index. The chunk belongs to a set of chunks in the selection
 * specified by fspace_id. If the queried chunk does not exist in the file,
 * the size will be set to 0 and address to HADDR_UNDEF. The value pointed
 * to by filter_mask will not be modified. NULL can be passed in for any
 * OUT parameters.
 *
 * \note As of 1.10.5, the dataspace intersection is not yet supported,
 * hence, the index is of all the written chunks.
 *
 * \note Please be aware that this function currently does not support
 * non-trivial selections, thus \p fspace_id has no effect. Also, the
 * implementation does not handle the #H5S_ALL macro as an oversight.
 ...
 * a negative value.
 *
 * \details H5Dget_offset() returns the address in the file of the dataset,
 * \p dset_id. That address is expressed as the offset in bytes from the
 ...
 *                      dataspace
 * \param[in]  chunk_nbytes The size in bytes for the chunk
 ...
 * \return \herr_t
 *
 * \details H5Dwrite_chunk() writes a raw data chunk as specified by
 * its logical offset \p offset in a chunked dataset \p dset_id from the
 * application memory buffer \p buf to the dataset in the file. Typically,
 * the data in \p buf is preprocessed in memory by a custom transformation,
 * such as compression. The chunk will bypass the library’s internal data
 * transfer pipeline, including filters, and will be written directly to
 * the file. Only one chunk can be written with this function.
 *
 * H5Dwrite_chunk() replaces the now deprecated H5DOwrite_chunk() function,
 * which was located in the high level optimization library. The parameters
 * and behavior are identical to the original.
 *
 * \p dxpl_id is a data transfer property list identifier.
 *
 * \p filters is a mask providing a record of which filters are used with the
 * the chunk. The default value of the mask is zero (0), indicating that all
 * enabled filters are applied. A filter is skipped if the bit corresponding
 * to the filter’s position in the pipeline (0 ≤ position < 32) is turned on.
 ...
 * dataspace. The values in offset must not exceed the dimension limits
 ...
 * number of bytes to be read from the buffer buf. If the data chunk has
 * been precompressed, data_size should be the size of the compressed data.
 *
 * \p buf is the memory buffer containing data to be written to the chunk in
 * the file.
 *
 * \note Exercise caution when using H5Dread_chunk() and H5Dwrite_chunk(),
 * as they read and write data chunks directly in a file. H5Dwrite_chunk()
 * bypasses hyperslab selection, the conversion of data from one datatype to
 * another, and the filter pipeline to write the chunk. Developers should
 * have experience with these processes before using this function. Please
 * see Using the Direct Chunk Write Function for more information.
 *
 * Also note that H5Dread_chunk() and H5Dwrite_chunk() are not supported
 * under parallel and do not support variable length types.
 *
 * \since 1.10.2
 *
 *-------------------------------------------------------------------------
 ...
 * \return \herr_t
 *
 * \details H5Dread_chunk() reads a raw data chunk as specified by its
 * logical offset \p offset in a chunked dataset \p dset_id from the dataset
 * in the file into the application memory buffer \p buf. The data in \p
 * buf is read directly from the file bypassing the library’s internal
 ...
 * and must specify a point that falls on a dataset chunk boundary.
 * 
 * \p The mask filters indicates which filters are used with the chunk when
 * written. A zero value indicates that all enabled filters are applied on
 * the chunk. A filter is skipped if the bit corresponding to the filter’s
 ...
 *
 * \param[in/out]  buf   Pointer to the buffer in memory containing the
 * elements to iterate over
 * \type_id
 * \space_id
 * \param[in] operator Function pointer to the routine to be called for
 * each element in buf iterated over
 * \param[in/out] operator_data Pointer to any user-defined data associated
 * with the operation
 *
 * \return \herr_t
 *
 * \details H5Diterate() iterates over all the data elements in the memory buffer \p buf, executing the callback function \p operator once for each such data element.

The protoype of the callback function \p operator is as follows (as defined in the source code file H5Lpublic.h):
...
Look at H5Literate for example of format
 ...
 * required to store the VL data from the dataset, using \p space_id for
 * the selection in the dataset on disk and the \p type_id for the memory
 * representation of the VL data in memory.
 *
 * \p size is returned with the number of bytes required to store the VL data
 * in memory.
 *
 * \since 1.10.2
 *
 ...
 * \return \hid_t{dataset creation property list}
 *
 * \details H5Dfill() explicitly fills the dataspace selection in memory,
 * \p space_id, with the fill value specified in \p fill. If \p fill is NULL,
 * a fill value of 0 (zero) is used.
 *
 * \p fill_type_id specifies the datatype of the fill value.
 *
 * \p buf specifies the buffer in which the dataspace elements will be written.
 *
 * \p buf_type_id specifies the datatype of those data elements.
 *
 * Note that if the fill value datatype differs from the memory buffer
 * datatype, the fill value will be converted to the memory buffer datatype
 * before filling the selection.
 *
 * \note Applications sometimes write data only to portions of an allocated
 * dataset. It is often useful in such cases to fill the unused space with
 * a known fill value. See the following function for more information:
 * - H5Pset_fill_value
 * - H5Pget_fill_value
 ...
 * \return \herr_t
 *
 * \details H5Dflush() causes all buffers associated with a dataset to be
 * immediately flushed to disk without removing the data from the cache.
 *
 * \note HDF5 does not possess full control over buffering.  H5D_FLUSH
 * flushes the internal HDF5 buffers and then asks the operating system
 * (the OS) to flush the system buffers for the open files. After that,
 * the OS is responsible for ensuring that the data is actually flushed
 * to disk.
 *-------------------------------------------------------------------------
 */
 ...
 * \return \herr_t
 *
 * \details H5Drefresh() causes all buffers associated with a dataset
 * to be cleared and immediately re-loaded with updated contents from disk.

 * This function essentially closes the dataset, evicts all metadata
 * associated with it from the cache, and then re-opens the dataset. The
 * reopened dataset is automatically re-registered with the same identifier.
 *
 * \since 1.10.2
 *
 *-------------------------------------------------------------------------
 ...
 * describing both the dimensions of the destination buffer and the
 * selection within the destination buffer that data will be scattered to
 *
 * \return \herr_t
 *
 * \details H5Dscatter() retrieves data from the supplied callback \p op
 *  and scatters it to the supplied buffer \p dst_buf in a manner similar
 *  to data being written to a dataset.  *
 *
 * \p dst_space_id is a dataspace which defines the extent of \p dst_buf
 * and the selection within it to scatter the data to.
 *
 * \p type_id is the datatype of the data to be scattered in both the source
 * and destination buffers.
 *
 * \p dst_buf must be at least as large as the number of elements in the
 * extent of \p dst_space_id times the size in bytes of \p type_id.
 *
 * To retrieve the data to be scattered, H5Dscatter() repeatedly calls \p op,
 * which should return a valid source buffer, until enough data to fill the
 * selection has been retrieved. The prototype of the callback function \p
 * op is as follows (as defined in the source code file H5Dpublic.h):
 *
 ...
 *
<beginning of table>
 * \p src_buf Pointer to the buffer holding the next set of elements to scatter
 * On entry, the value of where \p src_buf points to is undefined. The
 * callback function should set src_buf to point to the next set of
 * elements.
 *
 * \p src_buf_bytes_used Pointer to the number of valid bytes in \p src_buf
 * On entry, the value where \p src_buf_bytes_used points to is
 * undefined. The callback function should set *src_buf_bytes_used to the
 * number of valid bytes in \p src_buf. This number must be a multiple of
 * the datatype size.
 *
 * \p op_data User-defined pointer to data required by the callback function
 * A pass-through of the \p op_data pointer provided with the H5Dscratter() function call.
 *

<end of table>
 * The callback function should always return at least one element in \p
 * src_buf, and must not return more elements than are remaining to be
 * scattered. This function will be repeatedly called until all elements to
 * be scattered have been returned. The callback function should return zero
 * (0) to indicate success, and a negative value to indicate failure.
 *
 * \note Note for C++ Developers Using C Functions:
 ...
 *
 * Exiting the routine in its normal fashion allows the HDF5 C library
 * to clean up its work properly. In other words, if the C++ application
 * jumps out of the routine back to the C++ “catch” statement, the
 * HDF5 C library is not given the opportunity to close any temporary
 * data structures that were set up when the routine was called. The C++
 * application should save some state as the routine is started so that
 * any problem that occurs might be diagnosed.  *
 *
 * \since 1.10.2
 *
 *-------------------------------------------------------------------------
 ...
 * \return \herr_t
 *
 * \details functionname() returns the size in bytes
 * allocated in the file for a raw data chunk as specified by its
 * logical \p offset in the dataset \p dset_id. The size is returned in
 * \p chunk_nbytes. It is the size of the compressed data if the chunk is
 * filtered and the size may be zero if no storage is allocated yet for
