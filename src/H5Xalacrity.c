...
typedef struct H5X_alacrity_t {
    hbool_t metadata_read;  /* Has metadata been read already */
...
static herr_t
H5X__alacrity_get_query_range(hid_t query_id, value_types_t *query_lb,
...
/*-------------------------------------------------------------------------
 * Function:    H5X__alacrity_configure
 *
...
H5X__alacrity_configure(H5X_alacrity_t *alacrity, hid_t dataset_id)
{
    ...
    if (ALErrorNone != ALEncoderConfigure(&alacrity->config, 16, al_type,
            ALInvertedIndex))
        HGOTO_ERROR(H5E_INDEX, H5E_CANTSET, FAIL, "can't configure ALACRITY encoder");
...
/*-------------------------------------------------------------------------
 * Function:    H5X__alacrity_get_query_range
 *
        ...
        value_types_t *query_ub)
{
    ...
    if (query->is_combined) {
        ...
        if (query->query.select.match_op == H5Q_MATCH_EQUAL) {
            ...
            query_lb->asFloat = *((float *) query->query.select.elem.data_elem.value);
        }
    }

    /* Find bins that satisfy query */
    query_lb->asFloat = 39.1f;
    query_ub->asFloat = 42.1f;
...
/*-------------------------------------------------------------------------
 * Function:    H5X__alacrity_readIndex
 *
        ...
        size_t metadata_size, void *metadata)
{
    ...
    /* Create transaction from version */
        ...
        hid_t *dataspace_id)
{
    ...
    value_types_t query_lb, query_ub;
    hid_t rcxt_id;
    bool found_bin;
    hid_t ret_space_id = FAIL;
    herr_t ret_value = SUCCEED; /* Return value */
    ...
    /* Get range value from query */
    if (FAIL == H5X__alacrity_get_query_range(query_id, &query_lb, &query_ub))
     ...
     */
    found_bin = H5X__alacrity_find_bin_range(&alacrity->metadata, query_lb,
    ...
    if (found_bin) {
        ...
        size_t al_index_size;
        hsize_t dims;
        hsize_t start_coord, end_coord, nelmts;
        unsigned int i;
        ...
        dims = al_index_size / (sizeof(rid_t));
        ...
        if (FAIL == H5Sget_select_bounds(ret_space_id, &start_coord, &end_coord))
            HGOTO_ERROR(H5E_DATASPACE, H5E_CANTSELECT, FAIL, "unable to get bounds");
        if (0 == (nelmts = (hsize_t) H5Sget_select_npoints(ret_space_id)))
            HGOTO_ERROR(H5E_DATASPACE, H5E_BADVALUE, FAIL, "invalid number of elements");
        H5X_ALACRITY_LOG_DEBUG("Created dataspace from index with %llu elements [%llu:%llu]",
                nelmts, start_coord, end_coord);

        H5MM_free(al_index);
    } else {
        if (FAIL == (ret_space_id = H5Screate(H5S_NULL)))
            HGOTO_ERROR(H5E_DATASPACE, H5E_CANTINIT, FAIL, "unable to create NULL dataspace");
    }
