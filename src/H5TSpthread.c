...
H5TS__update_stats_rd_lock(H5TS_rw_lock_t *rw_lock, const H5TS_rec_entry_count_t *count)
{
    ...
    if (count->rec_lock_count == 1) {
        ...
        if (rw_lock->active_reader_threads > rw_lock->stats.max_read_locks)
            rw_lock->stats.max_read_locks = rw_lock->active_reader_threads;
    }
...
H5TS__update_stats_wr_lock(H5TS_rw_lock_t *rw_lock)
{
    ...
    assert(rw_lock);
    assert(WRITE == rw_lock->lock_type);
        ...
        if (WRITE == rw_lock->lock_type) {
            ...
            do {
                if (H5_UNLIKELY(H5TS_cond_wait(&rw_lock->readers_cv, &rw_lock->mutex)))
                    HGOTO_DONE(FAIL);
            } while (WRITE == rw_lock->lock_type);
        }

        /* Set counter's lock type (which might already be set) & increment
         * number of reader threads
         */
        rw_lock->lock_type = READ;
        rw_lock->active_reader_threads++;
    }
        ...
        if (UNUSED != rw_lock->lock_type) {
            ...
            do {
                ...
                if (H5_UNLIKELY(result != 0))
                    HGOTO_DONE(FAIL);
            } while (UNUSED != rw_lock->lock_type);
        }
...
H5TS__rw_unlock(H5TS_rw_lock_t *rw_lock)
{
    ...
    if (WRITE == rw_lock->lock_type) { /* Drop a write lock */
        /* Sanity checks */
        assert(0 == rw_lock->active_reader_threads);
        assert(rw_lock->rec_write_lock_count > 0);
	...
	if (H5_UNLIKELY(H5TS_key_get_value(rw_lock->rec_read_lock_count_key, (void **)&count) < 0))
        ...
        if (0 == count->rec_lock_count) {
            ...
            /* Check if lock is unused now */
            if (0 == rw_lock->active_reader_threads)
                rw_lock->lock_type = UNUSED;
        }
    }
