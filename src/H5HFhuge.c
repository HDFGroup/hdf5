...
HDfprintf(stderr, "%s: obj_size = %Zu\n", FUNC, obj_size);
    ...
    if(hdr->filter_len > 0) {
        ...
        if(H5Z_pipeline(&(hdr->pline), 0, &filter_mask, H5Z_NO_EDC, filter_cb,
                &nbytes, &write_size, &write_buf, FALSE, dxpl_id) < 0)
    ...
    hbool_t is_read, H5HF_operator_t op, void *op_data)
{
    ...
    if(hdr->filter_len > 0) {
        ...
        if(H5Z_pipeline(&(hdr->pline), H5Z_FLAG_REVERSE, &filter_mask, H5Z_NO_EDC, filter_cb, &nbytes, &read_size, &read_buf, FALSE, dxpl_id) < 0)
