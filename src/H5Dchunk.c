...
static ssize_t H5D__nonexistent_readvv(const H5D_io_info_t *io_info, const H5D_dset_io_info_t *dset_info, size_t chunk_max_nseq,
...
static herr_t  H5D__create_piece_map_single(H5D_dset_io_info_t *di, const H5D_io_info_t *io_info);
static herr_t  H5D__create_piece_file_map_all(H5D_dset_io_info_t *di, const H5D_io_info_t *io_info);
static herr_t  H5D__create_piece_file_map_hyper(H5D_dset_io_info_t *di, const H5D_io_info_t *io_info);
...
static void    *H5D__chunk_lock(const H5D_io_info_t *io_info, const H5D_dset_io_info_t *dset_info, H5D_chunk_ud_t *udata, hbool_t relax,
                                hbool_t prev_unfilt_chunk);
static herr_t   H5D__chunk_unlock(const H5D_io_info_t *io_info, const H5D_dset_io_info_t *dset_info, const H5D_chunk_ud_t *udata, hbool_t dirty,
 ...
 */
herr_t
H5D__chunk_direct_write(H5D_t *dset, uint32_t filters, hsize_t *offset, uint32_t data_size,
                        const void *buf)
{
...
{
    const H5D_t       *dataset    = dinfo->dset;         /* Local pointer to dataset info */
    const H5T_t       *mem_type   = dinfo->type_info.mem_type; /* Local pointer to memory datatype */
 ...
 */
static herr_t
H5D__create_piece_map_single(H5D_dset_io_info_t *di, const H5D_io_info_t *io_info)
{
 ...
 */
static herr_t
H5D__create_piece_file_map_all(H5D_dset_io_info_t *di, const H5D_io_info_t *io_info)
{
 ...
 */
static herr_t
H5D__create_piece_file_map_hyper(H5D_dset_io_info_t *dinfo, const H5D_io_info_t *io_info)
{
    H5S_t   *tmp_fchunk = NULL;              /* Temporary file dataspace */
    hsize_t  sel_start[H5O_LAYOUT_NDIMS];    /* Offset of low bound of file selection */
    hsize_t  sel_end[H5O_LAYOUT_NDIMS];      /* Offset of high bound of file selection */
...
H5D__chunk_read(H5D_io_info_t *io_info, H5D_dset_io_info_t *dset_info)
{
    ...
    H5D_dset_io_info_t *nonexistent_dset_info = NULL;  /* "nonexistent" I/O dset info object */
    H5D_dset_io_info_t *ctg_dset_info         = NULL;  /* Contiguous I/O dset info object */
    H5D_dset_io_info_t *cpt_dset_info         = NULL;  /* Compact I/O dset info object */
    ...
    if (NULL == (nonexistent_dset_info = H5FL_CALLOC(H5D_dset_io_info_t)))
    ...
    if (io_info->use_select_io) {
        ...
        if (NULL == (ctg_dset_info = H5FL_CALLOC(H5D_dset_io_info_t)))
            HGOTO_ERROR(H5E_DATASET, H5E_CANTALLOC, FAIL, "couldn't allocate dset info array buffer")
        ...
        if (NULL == (cpt_dset_info = H5FL_CALLOC(H5D_dset_io_info_t)))
            HGOTO_ERROR(H5E_DATASET, H5E_CANTALLOC, FAIL, "couldn't allocate dset info array buffer")
        ...
        while (chunk_node) {
            ...
            if (H5F_addr_defined(udata.chunk_block.offset) || UINT_MAX != udata.idx_hint ||
                ...
                if (chunk && H5D__chunk_unlock(io_info, dset_info, &udata, FALSE, chunk, src_accessed_bytes) < 0)
        ...
        /* Clean up */
        HDassert(ctg_dset_info);
        ctg_dset_info = H5FL_FREE(H5D_dset_io_info_t, ctg_dset_info);
        HDassert(cpt_dset_info);
        cpt_dset_info = H5FL_FREE(H5D_dset_io_info_t, cpt_dset_info);
    } /* end else */

    HDassert(nonexistent_dset_info);
    nonexistent_dset_info = H5FL_FREE(H5D_dset_io_info_t, nonexistent_dset_info);
    ...
    if (io_info->use_select_io) {
        ...
        while (chunk_node) {
            ...
            if (cacheable) {
                ...
                H5_CHECK_OVERFLOW(chunk_info->piece_points, /*From:*/ size_t, /*To:*/ uint32_t);
                dst_accessed_bytes = (uint32_t)chunk_info->piece_points * (uint32_t)dset_info->type_info.dst_type_size;
 ...
 */
herr_t
H5D__piece_io_term(H5D_io_info_t H5_ATTR_UNUSED *io_info, H5D_dset_io_info_t *di)
{
    herr_t ret_value = SUCCEED; /*return value        */
 ...
 */
static void *
H5D__chunk_lock(const H5D_io_info_t *io_info, const H5D_dset_io_info_t *dset_info, H5D_chunk_ud_t *udata, hbool_t relax, hbool_t prev_unfilt_chunk)
{
 ...
 */
static herr_t
H5D__chunk_unlock(const H5D_io_info_t *io_info, const H5D_dset_io_info_t *dset_info, const H5D_chunk_ud_t *udata, hbool_t dirty, void *chunk,
                  uint32_t naccessed)
{
    const H5O_layout_t *layout; /* Dataset layout */
...
H5D__chunk_update_old_edge_chunks(H5D_t *dset, hsize_t old_dim[])
{
    ...
    H5D_dset_io_info_t *chk_dset_info = NULL;             /* Chunked I/O dset info object */
    ...
    if (NULL == (chk_dset_info = H5FL_CALLOC(H5D_dset_io_info_t)))
        HGOTO_ERROR(H5E_DATASET, H5E_CANTALLOC, FAIL, "couldn't allocate dset info array buffer")
    ...
    for (op_dim = 0; op_dim < space_ndims; op_dim++) {
        ...
        while (!carry) {
            ...
            if (H5F_addr_defined(chk_udata.chunk_block.offset) || (UINT_MAX != chk_udata.idx_hint)) {
                ...
                if (NULL == (chunk = (void *)H5D__chunk_lock(&chk_io_info, chk_dset_info, &chk_udata, FALSE, TRUE)))
    ...
    /* Clean up */
    if (chk_dset_info)
        chk_dset_info = H5FL_FREE(H5D_dset_io_info_t, chk_dset_info);
 ...
 */
static ssize_t
H5D__nonexistent_readvv(const H5D_io_info_t *io_info, const H5D_dset_io_info_t *dset_info, size_t chunk_max_nseq, size_t *chunk_curr_seq,
