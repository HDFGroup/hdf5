...
typedef struct H5D_threaded_chunk_info_t {
    struct H5D_threaded_io_info_t *threaded_io_info;
...
typedef struct H5D_threaded_io_info_t {
    H5D_threaded_chunk_info_t *chunk_info;
    size_t                     num_chunks;
    haddr_t                   *addrs;
    size_t                    *sizes;
    void                     **bufs;
    ...
    static ssize_t H5D__nonexistent_readvv(const H5D_io_info_t *io_info, const H5D_dset_io_info_t *dset_info,
                                           size_t chunk_max_nseq, size_t *chunk_curr_seq,
                                           size_t chunk_len_arr[], hsize_t chunk_offset_arr[],
                                           size_t mem_max_nseq, size_t *mem_curr_seq, size_t mem_len_arr[],
    ...
    H5D__chunk_set_info(const H5D_t *dset)
    {
        ...
        if (H5D__chunk_set_info_real(&dset->shared->layout.u.chunk, dset->shared->ndims,
    ...
    H5D__chunk_construct(H5F_t *f, H5D_t *dset)
    {
        ...
        unsigned      version_perf =
            H5O_LAYOUT_VERSION_1;     /* Version we would like to upgrade to for performance */
    ...
    H5D__chunk_init(H5F_t *f, H5D_t *dset, hid_t dapl_id, bool open_op)
    {
        ...
        if (dset->shared->ndims > 1) {
            ...
            for (u = 0; u < dset->shared->ndims; u++) {
                ...
                /* Round up to the next integer # of chunks, to accommodate partial chunks */
                rdcc->scaled_dims[u] =
                    (dset->shared->curr_dims[u] + dset->shared->layout.u.chunk.dim[u] - 1) /
    ...
    H5D__chunk_io_init(H5D_io_info_t *io_info, H5D_dset_io_info_t *dinfo)
    {
        ...
        if (H5F_SHARED_HAS_FEATURE(io_info->f_sh, H5FD_FEAT_HAS_MPI) &&
            ...
            if (sc->ops->load_metadata && sc->ops->load_metadata(&idx_info) < 0)
                HGOTO_ERROR(H5E_DATASET, H5E_CANTINIT, FAIL,
                            "unable to load additional chunk index metadata");
        }
#endif
            ...
            if (NULL == dataset->shared->cache.chunk.sel_chunks)
                if (NULL == (dataset->shared->cache.chunk.sel_chunks = H5SL_create(H5SL_TYPE_HSIZE, NULL)))
                    HGOTO_ERROR(H5E_DATASET, H5E_CANTCREATE, FAIL,
                                "can't create skip list for chunk selections");
    ...
    H5D__create_piece_file_map_all(H5D_dset_io_info_t *di, H5D_io_info_t *io_info)
    {
        ...
        while (sel_points) {
            ...
            if (num_partial_dims > 0)
                if (H5S_select_hyperslab(new_piece_info->fspace, H5S_SELECT_SET, zeros, NULL,
    ...
    H5D__create_piece_file_map_hyper(H5D_dset_io_info_t *dinfo, H5D_io_info_t *io_info)
    {
        ...
        while (sel_points) {
            ...
            if (true == H5S_SELECT_INTERSECT_BLOCK(dinfo->file_space, coords, end)) {
                 ...
                 *  the current chunk.
                 */
                if (H5S_combine_hyperslab(dinfo->file_space, H5S_SELECT_AND, coords, NULL, fm->chunk_dim,
                                          NULL, &tmp_fchunk) < 0)
                       ...
                       const hsize_t *coords, void *_opdata)
    {
        ...
        H5D_dset_io_info_t *dinfo =
            (H5D_dset_io_info_t *)opdata->dinfo;             /* File<->memory piece mapping info */
        H5D_chunk_map_t  *fm;                                /* Convenience pointer to chunk map */
            ...
            if (NULL == (piece_info = (H5D_piece_info_t *)H5SL_search(fm->dset_sel_pieces, &chunk_index))) {
                ...
                if (H5SL_insert(fm->dset_sel_pieces, piece_info, &piece_info->index) < 0) {
                    ...
                    HGOTO_ERROR(H5E_DATASPACE, H5E_CANTINSERT, FAIL,
                                "can't insert chunk into dataset skip list");
                } /* end if */
                      ...
                      const hsize_t *coords, void *_opdata)
    {
        H5D_io_info_wrap_t *opdata = (H5D_io_info_wrap_t *)_opdata;
        H5D_dset_io_info_t *dinfo =
            (H5D_dset_io_info_t *)opdata->dinfo;       /* File<->memory chunk mapping info */
        ...
        if (H5S_SELECT_ITER_NEXT(&fm->mem_iter, (size_t)1) < 0)
            HGOTO_ERROR(H5E_DATASPACE, H5E_CANTNEXT, H5_ITER_ERROR,
                        "unable to move to next iterator location");
    ...
    H5D__chunk_read(H5D_io_info_t *io_info, H5D_dset_io_info_t *dset_info)
    {
        ...
        if (io_info->use_select_io == H5D_SELECTION_IO_MODE_ON) {
            ...
            if (do_threading) {
                ...
                if (NULL == (threaded_read_info.chunk_info = (H5D_threaded_chunk_info_t *)H5MM_malloc(
                                 num_chunks * sizeof(H5D_threaded_chunk_info_t))))
                    HGOTO_ERROR(H5E_DATASET, H5E_CANTALLOC, FAIL,
                                "can't allocate array of threaded chunk info structs");

                /* Initialize num chunks in threaded read info to 0 (will be incremented as info is filled in
                 * for each chunk */
                threaded_read_info.num_chunks = 0;

                /* Initialize other fields in threaded_io_info */
                threaded_io_info.

                    /* Retrieve filter settings from API context */
                    if (H5CX_get_err_detect(&threaded_io_info.err_detect) < 0)
                        HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, FAIL, "can't get error detection info");
                if (H5CX_get_filter_cb(&threaded_io_info.filter_cb) < 0)
                    HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, FAIL, "can't get I/O filter callback function");

                    /* Allocate initial vector I/O arrays */
#error write me
            }
             ...
             */
            dset_info->dset->shared->cache.sieve.sieve_buf_size =
                H5F_SIEVE_BUF_SIZE(dset_info->dset->oloc.file);
            dset_info->dset->shared->cache.sieve.sieve_loc  = HADDR_UNDEF;
            dset_info->dset->shared->cache.sieve.sieve_size = 0;
            assert(
                !dset_info->dset->shared->cache.sieve.sieve_dirty); /* Any writes should have been flushed */
            ...
            while (chunk_node) {
                H5D_piece_info_t                    *chunk_info; /* Chunk information */
                H5D_chunk_ud_t                       udata;      /* Chunk index pass-through    */
                H5D_chunk_ud_t                      *udata_p;
                / *Pointer to chunk udata * / htri_t cacheable; /* Whether the chunk is cacheable */

                /* Set udata_p */
                udata_p =
                    do_threading ? &(threaded_io_info.chunk_info[threaded_io_info.num_chunks].udata) : &udata;
                ...
                if (H5_addr_defined(udata.chunk_block.offset) || UINT_MAX != udata.idx_hint ||
                    ...
                    if ((cacheable = H5D__chunk_cacheable(io_info, dset_info, udata_p->chunk_block.offset,
                                                          false)) < 0)
                    ...
                    if (cacheable) {
                        ...
                        if (NULL == (chunk = H5D__chunk_lock(io_info, dset_info, udata_p, false, false,
                                                             do_threading ? &threaded_read_info : NULL,
                                                             &threaded_chunk)))
                            HGOTO_ERROR(H5E_IO, H5E_READERROR, FAIL, "unable to read raw data chunk");
                    ...
                    if (!threaded_chunk) {
                        ...
                        if (chunk && H5D__chunk_unlock(io_info, dset_info, &udata, false, chunk,
            ...
            if (threaded_read_info.nused > 0) {
                H5FD_mem_t types[2] = {H5FD_MEM_DRAW, H5FD_MEM_NOLIST};
                assert(do_threading);
                assert(H5_thread_pool_g);

                /* Create semaphore for completed tasks */
                if (H5_UNLIKELY(H5TS_semaphore_init(&threaded_read_info.sem, 0) < 0))
                    HGOTO_ERROR(H5E_DATASET, H5E_CANTINIT, FAIL,
                                "can't create semaphore for completed tasks");

                /* Read all chunks from disk */
                H5_CHECK_OVERFLOW(threaded_read_info.vec_nused, size_t, uint32_t);
                if (H5F_shared_vector_read(io_info->f_sh, threaded_read_info.vec_nused, types,
                                           threaded_read_info.addrs, threaded_read_info.sizes,
                                           threaded_read_info.bufs) < 0)
                ...
                for (size_t i = 0; i < threaded_read_info.num_chunks; i++) {
                    if (H5TS_pool_add_task(H5_thread_pool_g, H5D__chunk_thread_read,
                                           &(threaded_read_info.chunk_info[i])) < 0)
                        HGOTO_ERROR(H5E_DATASET, H5E_CANTINIT, FAIL, "can't launch worker thread");
                }

                /* Wait for all worker tasks to complete */
                for (size_t i = 0; i < threaded_read_info.num_chunks; i++)
                    if (H5_UNLIKELY(H5TS_semaphore_wait(&threaded_read_info.sem) < 0))
            ...
            assert(threaded_read_info.vec_nused == 0);
#endif    /* H5_HAVE_CONCURRENCY */
        } /* end else */
    ...
    H5D__chunk_thread_read(void *_chunk_info)
    {
        H5D_threaded_chunk_info_t *chunk_info = (H5D_threaded_chunk_info_t *)_chunk_info;
        herr_t ret_value = SUCCEED; /* "Return value" for error macros (actual return value is always
                                       (H5TS_thread_ret_t)0) */

        FUNC_ENTER_NOAPI_NOINIT

        if (chunk_info->old_pline) {
            /* Perform filter pipeline */
            if (H5Z_pipeline(chunk_info->old_pline, H5Z_FLAG_REVERSE, &(chunk_info->udata.filter_mask),
                             err_detect, filter_cb, &chunk_nbytes, &buf_alloc, &chunk) < 0)
            ...
            assert(!chunk_info->udata.new_unfilt_chunk);
        }

        /* Scatter data from chunk buffer to application buffer */
        if ((dset_info->io_ops.single_read)(chk_io_info, &chk_io_info->dsets_info[0]) < 0)
            HGOTO_ERROR(H5E_DATASET, H5E_READERROR, FAIL, "chunked read failed");
    ...
    H5D__chunk_write(H5D_io_info_t *io_info, H5D_dset_io_info_t *dset_info)
    {
        ...
        if (io_info->use_select_io == H5D_SELECTION_IO_MODE_ON) {
            ...
            while (chunk_node) {
                ...
                /* Determine if we should use the chunk cache */
                if ((cacheable = H5D__chunk_cacheable(io_info, dset_info, udata.chunk_block.offset, true)) <
                    0)
                    HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, FAIL, "can't tell if chunk is cacheable");
                if (cacheable) {
                    ...
                    dst_accessed_bytes =
                        chunk_info->piece_points * (hsize_t)dset_info->type_info.dst_type_size;
                    ...
                    if (!H5_addr_defined(udata.chunk_block.offset)) {
                        ...
                        if (need_insert && dset_info->dset->shared->layout.storage.u.chunk.ops->insert)
                            if ((dset_info->dset->shared->layout.storage.u.chunk.ops->insert)(
                                    &idx_info, &udata, NULL) < 0)
                                HGOTO_ERROR(H5E_DATASET, H5E_CANTINSERT, FAIL,
    ...
    H5D__chunk_flush_entry(const H5D_t *dset, H5D_rdcc_ent_t *ent, bool reset)
    {
        ...
        if (reset) {
            ...
            if (buf == ent->chunk)
                buf = NULL;
            if (ent->chunk != NULL)
                ent->chunk = (uint8_t *)H5D__chunk_mem_xfree(
                    ent->chunk,
    ...
    H5D__chunk_lock(const H5D_io_info_t H5_ATTR_NDEBUG_UNUSED *io_info, const H5D_dset_io_info_t *dset_info,
                    H5D_chunk_ud_t *udata, bool relax, bool prev_unfilt_chunk,
                    H5D_threaded_read_info_t *threaded_read_info)
    {
        ...
        if (UINT_MAX != udata->idx_hint) {
            ...
            if (relax) {
                ...
                if (H5_addr_defined(chunk_addr)) {
                    ...
                    /* Ideally we should allocate a buffer at least as large as chunk_size, to give the filter
                     * the opportunity to avoid doing a realloc when uncompressing. This causes problems now,
                     * however, and needs more investigation. -NAF */
                    ...
                    if (threaded_read_info) {
                        ...
                        if (threaded_read_info->vec_nused == threaded_read_info->vec_nalloc) {
                            if (NULL == (tmp_list = H5MM_realloc(threaded_read_info->addrs,
                                                                 2 * threaded_read_info->nalloc *
                                                                     sizeof(*threaded_read_info->addrs))))
                                HGOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, NULL,
                                            "memory reallocation failed for address list");
                            threaded_read_info->addrs = (haddr_t *)tmp_list;
                            if (NULL == (tmp_list = H5MM_realloc(threaded_read_info->sizes,
                                                                 2 * threaded_read_info->nalloc *
                                                                     sizeof(*threaded_read_info->sizes))))
                                HGOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, NULL,
                                            "memory reallocation failed for size list");
                            threaded_read_info->sizes = (size_t *)tmp_list;
                            if (NULL == (tmp_list = H5MM_realloc(threaded_read_info->bufs,
                                                                 2 * threaded_read_info->nalloc *
                                                                     sizeof(*threaded_read_info->bufs))))
                                HGOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, NULL,
                                            "memory reallocation failed for buffer list");
                            threaded_read_info->bufs = (void **)tmp_list;

                            threaded_read_info->vec_nalloc *= 2;
                        }

                        /* Add to list */
                        threaded_read_info->addrs[threaded_io_info->vec_nused] = chunk_addr;
                        threaded_read_info->sizes[threaded_io_info->vec_nused] = chunk_nbytes;
                        threaded_read_info->bufs[threaded_io_info->vec_nused]  = chunk;
                        (threaded_io_info->vec_nused)++;

                        /* Cache pipeline to use for decompression */
                    ...
                    if (fill->fill_time == H5D_FILL_TIME_ALLOC ||
                        ...
                        if (fb_info.has_vlen_fill_type)
                            ...
                            if (H5D__fill_refill_vl(&fb_info, fb_info.elmts_per_buf) < 0)
                                HGOTO_ERROR(H5E_DATASET, H5E_CANTCONVERT, NULL,
                                            "can't refill fill value buffer");
                    } /* end if */
                    else
                        memset(chunk, 0, chunk_size);
        ...
        /* Add chunk to list of chunks in threaded I/O */
    ...
    H5D__chunk_allocate(const H5D_t *dset, bool full_overwrite, const hsize_t old_dim[])
    {
        ...
        for (op_dim = 0; op_dim < space_ndims; op_dim++) {
            ...
            if ((layout->u.chunk.flags & H5O_LAYOUT_CHUNK_DONT_FILTER_PARTIAL_BOUND_CHUNKS) &&
                pline->nused > 0 && space_dim[op_dim] % chunk_dim[op_dim] != 0) {
            ...
            while (!carry) {
                ...
                if (fb_info_init && fb_info.has_vlen_fill_type) {
                    ...
                    if (fb_info.fill_buf_size < orig_chunk_size) {
                        if (NULL == (fb_info.fill_buf =
                                         H5D__chunk_mem_realloc(fb_info.fill_buf, orig_chunk_size, pline)))
                            HGOTO_ERROR(H5E_RESOURCE, H5E_NOSPACE, FAIL,
                ...
                if (should_fill) {
                    ...
                    if (using_mpi) {
                        ...
                        chunk_fill_info.chunk_info[chunk_fill_info.num_chunks].addr =
...
#endif /* H5_HAVE_PARALLEL */
                    {
                        ...
                        if (H5F_shared_block_write(H5F_SHARED(dset->oloc.file), H5FD_MEM_DRAW,
                                                   udata.chunk_block.offset, (size_t)chunk_size,
                                                   *fill_buf) < 0)
                    ...
                    if (scaled[i] > max_unalloc[i]) {
                        ...
                        if (unfilt_edge_chunk_dim[i] && edge_chunk_scaled[i] == max_unalloc[i] &&
                            ...
                            if (should_fill && nunfilt_edge_chunk_dims == 0 && !fb_info.has_vlen_fill_type) {
                                assert(!H5D__chunk_is_partial_edge_chunk(space_ndims, chunk_dim, scaled,
                                                                         space_dim));
                                assert(orig_chunk_size);
                                fill_buf = &fb_info.fill_buf;
                                H5_CHECKED_ASSIGN(chunk_size, hsize_t, orig_chunk_size, size_t);
        ...
        for (op_dim = 0; op_dim < space_ndims; op_dim++) {
            ...
            while (!carry) {
                ...
                if (H5_addr_defined(chk_udata.chunk_block.offset) || (UINT_MAX != chk_udata.idx_hint)) {
                    ...
                    if (NULL == (chunk = (void *)H5D__chunk_lock(&chk_io_info, &chk_dset_info, &chk_udata,
                                                                 false, true)))
                        HGOTO_ERROR(H5E_DATASET, H5E_READERROR, FAIL, "unable to lock raw data chunk");

                    /* Unlock the chunk */
                    if (H5D__chunk_unlock(&chk_io_info, &chk_dset_info, &chk_udata, true, chunk, (hsize_t)0) <
                        0)
                               ...
                               const void *fill_buf, const void *partial_chunk_fill_buf)
    {
        ...
        num_blocks = (size_t)(chunk_fill_info->num_chunks /
                              (size_t)mpi_size); /* value should be the same on all procs */
    ...
    H5D__chunk_prune_fill(H5D_chunk_it_ud1_t *udata, bool new_unfilt_chunk)
    {
        ...
        for (u = 0; u < rank; u++) {
            count[u] =
                MIN(layout->u.chunk.dim[u], (udata->space_dim[u] - (scaled[u] * layout->u.chunk.dim[u])));
            assert(count[u] > 0);
        } /* end for */
        ...
        if (H5S_select_hyperslab(udata->chunk_space, H5S_SELECT_NOTB, udata->hyper_start, NULL, count, NULL) <
            0)
    ...
    {
        hsize_t
            min_mod_chunk_sc[H5O_LAYOUT_NDIMS]; /* Scaled offset of first chunk to modify in each dimension */
        hsize_t
            max_mod_chunk_sc[H5O_LAYOUT_NDIMS]; /* Scaled offset of last chunk to modify in each dimension */
        ...
        for (op_dim = 0; op_dim < (unsigned)space_ndims; op_dim++) {
            ...
            while (!carry) {
                ...
                if (0 == ndims_outside_fill) {
                    ...
                    assert(H5D__chunk_is_partial_edge_chunk(space_ndims, layout->u.chunk.dim, scaled,
                                                            space_dim));
    ...
    H5D__chunk_copy_cb(const H5D_chunk_rec_t *chunk_rec, void *_udata)
    {
        ...
        if (is_vlen) {
            ...
            if (H5T_convert(tpath_src_mem, dt_src, dt_mem, udata->nelmts, (size_t)0, (size_t)0, udata->buf,
                            bkg) < 0)
            ...
            if (H5T_convert(tpath_mem_dst, dt_mem, dt_dst, udata->nelmts, (size_t)0, (size_t)0, udata->buf,
                            bkg) < 0)
                HGOTO_ERROR(H5E_DATATYPE, H5E_CANTCONVERT, H5_ITER_ERROR, "datatype conversion failed");

            /* Reclaim space from variable length data */
            if (H5T_reclaim(dt_mem, buf_space, reclaim_buf) < 0)
                HGOTO_ERROR(H5E_DATASET, H5E_CANTFREE, H5_ITER_ERROR,
                            "unable to reclaim variable-length data");
        } /* end if */
        else if (fix_ref) {
            ...
            if (udata->cpy_info->expand_ref) {
                ...
                if (H5O_copy_expand_ref(udata->file_src, dt_src, udata->buf, nbytes, udata->idx_info_dst->f,
                                        bkg, udata->cpy_info) < 0)
                    HGOTO_ERROR(H5E_DATASET, H5E_CANTCOPY, H5_ITER_ERROR,
                                "unable to copy reference attribute");
            } /* end if */
        ...
        udata_dst.chunk_idx =
            H5VM_array_offset_pre(udata_dst.common.layout->ndims - 1,
                    ...
                    H5O_copy_t *cpy_info)
    {
        ...
        if (H5T_detect_class(dt_src, H5T_VLEN, false) > 0) {
            ...
            if ((uint64_t)nelmts != nelmts_64)
                HGOTO_ERROR(H5E_DATASET, H5E_BADRANGE, FAIL,
                            "number of chunk elements too big to fit in size_t");
    ...
    H5D__chunk_dump_index_cb(const H5D_chunk_rec_t *chunk_rec, void *_udata)
    {
        ...
        if (udata->stream) {
            ...
            fprintf(udata->stream, "        0x%08x %8" PRIuHSIZE " %10" PRIuHADDR " [",
                    chunk_rec->filter_mask, chunk_rec->nbytes, chunk_rec->chunk_addr);
    ...
    H5D__nonexistent_readvv_cb(hsize_t H5_ATTR_UNUSED dst_off, hsize_t src_off, size_t len, void *_udata)
    {
        H5D_chunk_readvv_ud_t *udata =
            (H5D_chunk_readvv_ud_t *)_udata;        /* User data for H5VM_opvv() operator */
        H5D_fill_buf_info_t fb_info;                /* Dataset's fill buffer info */
        bool                fb_info_init = false;   /* Whether the fill value buffer has been initialized */
        herr_t              ret_value    = SUCCEED; /* Return value */
        ...
        if (H5D__fill_init(&fb_info, (udata->rbuf + src_off), NULL, NULL, NULL, NULL,
                           &udata->dset->shared->dcpl_cache.fill, udata->dset->shared->type, (size_t)0,
                           len) < 0)
            HGOTO_ERROR(H5E_DATASET, H5E_CANTINIT, FAIL, "can't initialize fill buffer info");
        fb_info_init = true;
    ...
    H5D__nonexistent_readvv(const H5D_io_info_t H5_ATTR_NDEBUG_UNUSED *io_info,
                            const H5D_dset_io_info_t *dset_info, size_t chunk_max_nseq,
                            size_t *chunk_curr_seq, size_t chunk_len_arr[], hsize_t chunk_off_arr[],
                            size_t mem_max_nseq, size_t *mem_curr_seq, size_t mem_len_arr[],
                            hsize_t mem_off_arr[])
    {
        ...
        if ((ret_value =
                 H5VM_opvv(chunk_max_nseq, chunk_curr_seq, chunk_len_arr, chunk_off_arr, mem_max_nseq,
                           mem_curr_seq, mem_len_arr, mem_off_arr, H5D__nonexistent_readvv_cb, &udata)) < 0)
        ...
        if (idx_info->pline->nused > 0) {
            ...
            if (old_chunk && H5_addr_defined(old_chunk->offset)) {
                ...
                assert(!H5_addr_defined(new_chunk->offset) ||
                       H5_addr_eq(new_chunk->offset, old_chunk->offset));
    ...
    H5D__chunk_format_convert_cb(const H5D_chunk_rec_t *chunk_rec, void *_udata)
    {
        ...
        if (new_idx_info->pline->nused &&
            ...
            if ((chunk_addr = H5MF_alloc(new_idx_info->f, H5FD_MEM_DRAW, (hsize_t)nbytes)) == HADDR_UNDEF)
                HGOTO_ERROR(H5E_DATASET, H5E_NOSPACE, H5_ITER_ERROR,
                            "file allocation failed for filtered chunk");
    ...
    H5D__chunk_format_convert(H5D_t *dset, H5D_chk_idx_info_t *idx_info, H5D_chk_idx_info_t *new_idx_info)
    {
         ...
         * index
         */
        if ((idx_info->layout->storage.u.chunk.ops->iterate)(idx_info, H5D__chunk_format_convert_cb, &udata) <
            0)
    ...
    H5D__chunk_index_empty(const H5D_t *dset, bool *empty)
    {
        ...
        *empty = true;
        ...
        if (H5_addr_defined(idx_info.layout->storage.u.chunk.idx_addr)) {
            ...
            if ((dset->shared->layout.storage.u.chunk.ops->iterate)(&idx_info, H5D__chunk_index_empty_cb,
                                                                    empty) < 0)
                HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, FAIL,
                            "unable to retrieve allocated chunk information from index");
        }
     ...
     */
    herr_t
    H5D__get_chunk_info(const H5D_t *dset, const H5S_t H5_ATTR_UNUSED *space, hsize_t chk_index,
                        hsize_t *offset, unsigned *filter_mask, haddr_t *addr, hsize_t *size)
    {
        H5D_chk_idx_info_t idx_info;            /* Chunked index info */
        const H5D_rdcc_t  *rdcc = NULL;         /* Raw data chunk cache */
        ...
        if (H5_addr_defined(idx_info.layout->storage.u.chunk.idx_addr)) {
            ...
            /* Iterate over the allocated chunks */
            if ((dset->shared->layout.storage.u.chunk.ops->iterate)(&idx_info, H5D__get_chunk_info_cb,
                                                                    &udata) < 0)
                HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, FAIL,
                            "unable to retrieve allocated chunk information from index");
     ...
     */
    herr_t
    H5D__get_chunk_info_by_coord(const H5D_t *dset, const hsize_t *offset, unsigned *filter_mask,
                                 haddr_t *addr, hsize_t *size)
    {
        ...
        if (H5_addr_defined(idx_info.layout->storage.u.chunk.idx_addr)) {
            ...
            if ((dset->shared->layout.storage.u.chunk.ops->iterate)(
                    &idx_info, H5D__get_chunk_info_by_coord_cb, &udata) < 0)
                HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, FAIL,
    ...
    H5D__chunk_iter_cb(const H5D_chunk_rec_t *chunk_rec, void *udata)
    {
        ...
        H5_BEFORE_USER_CB_NOERR(FAIL)
            {
                ret_value =
                    (data->op)(offset, (unsigned)chunk_rec->filter_mask,
