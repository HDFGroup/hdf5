...
H5D__create_piece_file_map_hyper(H5D_dset_info_t *dinfo, const H5D_io_info_t *io_info)
{
    ...
    while(sel_points) {
        ...
        if(TRUE == H5S_hyper_intersect_block((H5S_t *)dinfo->file_space, coords, end)) {
            ...
            HDmemcpy(new_piece_info->coords, coords, sizeof(hsize_t) * dinfo->f_ndims);
...
#ifndef NDEBUG
{
    ...
    if(ent) {
        ...
        if(!ent || !ent->locked) {
            ...
            HDmemcpy(ent->offset, io_info->dsets_info[0].store->chunk.offset, sizeof(hsize_t) * layout->u.chunk.ndims);
