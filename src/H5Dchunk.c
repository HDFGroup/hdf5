...
H5D__sel_io_chunk_cache(H5D_io_info_t *io_info, H5D_dset_io_info_t *dset_info)
{
    ...
    if (dset_info->num_ents_to_evict) {
        ...
        H5SL_close(dset_info->entries_to_evict);
    }

    /* Process #5 if there is still not enough space in cache: IN PROGRESS */
    /* Process #6 if there is still not enough space in cache */
    ...
    if (dset_info->num_chks_to_load) {
        ...
        /* Close the skip list for chunks_to_load */
        H5SL_close(dset_info->chunks_to_load);
