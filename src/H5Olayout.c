            ...
            case H5D_STRUCT_CHUNK:
                ...
                if (*p++ != H5O_STRUCT_CHUNK_STORAGE_PROPERTY_VERSION)
                    HGOTO_ERROR(H5E_OHDR, H5E_VERSION, NULL,
                                "wrong structured chunk storage property version");
                ...
                if (H5_IS_BUFFER_OVERFLOW(
                        p, (mesg->u.struct_chunk.ndims * mesg->u.struct_chunk.enc_bytes_per_dim), p_end))
                    HGOTO_ERROR(H5E_OHDR, H5E_OVERFLOW, NULL, "ran off end of input buffer while decoding");
...
/* NOTE HERE */
/* TBD: probably put structured chunk composition here after idx type */
...
/* NOTE HERE */
                         ...
                         */
                        chunk_size_len = 1 + ((H5VM_log2_gen((uint64_t)mesg->u.struct_chunk.size) + H5O_STRUCT_CHUNK_OFFSET_SIZE) /
                        ...
                        /* chunk size */
                        UINT64DECODE_VAR(p, mesg->storage.u.struct_chunk.u.single.chunk_size, chunk_size_len);

/* NOTE HERE */
/* TBD: composition information (nsects, offset_size) is not available yet at this point */

                        if (H5_IS_BUFFER_OVERFLOW(p, (H5O_SPARSE_NSECTS - 1) * 8, p_end))
                            HGOTO_ERROR(H5E_OHDR, H5E_OVERFLOW, NULL,
                                        "ran off end of input buffer while decoding");

                        for (unsigned i = 1; i < H5O_SPARSE_NSECTS; i++)
                        ...
                        /* Set the chunk operations */
                        mesg->storage.u.struct_chunk.ops = H5D_COPS_STRUCT_CHUNK_SINGLE;
                        break;
                    ...
                    case H5D_CHUNK_IDX_EARRAY:
                        ...
                        /* Set the chunk operations */
                        mesg->storage.u.struct_chunk.ops = H5D_COPS_STRUCT_CHUNK_EARRAY;
                        break;
                    ...
                    case H5D_CHUNK_IDX_BT2: /* v2 B-tree index */
                        ...
                        /* Set the chunk operations */
                        mesg->storage.u.struct_chunk.ops = H5D_COPS_STRUCT_CHUNK_BT2;
                        break;
