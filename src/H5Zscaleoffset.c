...
#define H5Z_scaleoffset_check_3(i, type, max, min, minbits, D_val)                                           \
    {                                                                                                        \
        if (sizeof(type) == sizeof(int)) {                                                                   \
            if (H5Z_scaleoffset_rnd((double)max * HDpow(10.0, D_val) - (double) min * HDpow(10.0, D_val)) > \
        ...
        if (sizeof(type) == sizeof(int))                                                                     \
            for (i = 0; i < d_nelmts; i++) {                                                                 \
                if (HDfabs((double)(buf[i] - filval)) < HDpow(10.0, -D_val)) \
                    ...
                    *(int *)&buf[i] =                                                                        \
                        (int)H5Z_scaleoffset_rnd((double)buf[i] * HDpow(10.0, (double)D_val) - (double)min * HDpow(10.0, (double)D_val)); \
            }                                                                                                \
        else if (sizeof(type) == sizeof(long))                                                               \
            for (i = 0; i < d_nelmts; i++) {                                                                 \
                if (HDfabs((double)(buf[i] - filval)) < HDpow(10.0, -D_val)) \
                    ...
                    *(long *)&buf[i] =                                                                       \
                        (long)H5Z_scaleoffset_rnd((double)buf[i] * HDpow(10.0, (double)D_val) - (double)min * HDpow(10.0, (double)D_val)); \
            }                                                                                                \
        else if (sizeof(type) == sizeof(long long))                                                          \
            for (i = 0; i < d_nelmts; i++) {                                                                 \
                if (HDfabs((double)(buf[i] - filval)) < HDpow(10.0, -D_val)) \
                    ...
                    *(long long *)&buf[i] =                                                                  \
                        (long long)H5Z_scaleoffset_rnd((double)buf[i] * HDpow(10.0, (double)D_val)- (double)min * HDpow(10.0, (double)D_val)); \
            }                                                                                                \
        else                                                                                                 \
            HGOTO_ERROR(H5E_PLINE, H5E_BADTYPE, FAIL, "cannot find matched integer dataype")                 \
    }
...
#define H5Z_scaleoffset_modify_2(i, type, buf, d_nelmts, min, D_val)                                         \
    {                                                                                                        \
        if (sizeof(type) == sizeof(int))                                                                     \
            for (i = 0; i < d_nelmts; i++)                                                                   \
                *(int *)&buf[i] =                                                                            \
                    (int)H5Z_scaleoffset_rnd((double)buf[i] * HDpow(10.0, D_val)- (double)min * HDpow(10.0, D_val)); \
        else if (sizeof(type) == sizeof(long))                                                               \
            for (i = 0; i < d_nelmts; i++)                                                                   \
                *(long *)&buf[i] =                                                                           \
                    (long)H5Z_scaleoffset_rnd((double)buf[i] * HDpow(10.0, D_val)- (double)min * HDpow(10.0, D_val)); \
        else if (sizeof(type) == sizeof(long long))                                                          \
            for (i = 0; i < d_nelmts; i++)                                                                   \
                *(long long *)&buf[i] =                                                                      \
                    (long long)H5Z_scaleoffset_rnd((double)buf[i] * HDpow(10.0, D_val)- (double)min * HDpow(10.0, D_val)); \
        else                                                                                                 \
            HGOTO_ERROR(H5E_PLINE, H5E_BADTYPE, FAIL, "cannot find matched integer dataype")                 \
    }
        ...
        *minval = 0;                                                                                         \
        if (filavail == H5Z_SCALEOFFSET_FILL_DEFINED) { /* fill value defined */                             \
            H5Z_scaleoffset_get_filval_2(type, cd_values, filval)                                            \
                ...
                (unsigned long long) H5Z_scaleoffset_rnd((double)max * HDpow(10.0, D_val) - (double)min * HDpow(10.0, D_val)) + 1; \
            *minbits = H5Z_scaleoffset_log2((unsigned long long)(span + 1));                                 \
        ...
        if (sizeof(type) == sizeof(int))                                                                     \
            for (i = 0; i < d_nelmts; i++)                                                                   \
                buf[i] = (type)((*(int *)&buf[i] == (int)(((unsigned int)1 << minbits) - 1))                 \
                                ? (double) filval                        \
                                : (double)(*(int *)&buf[i]) / HDpow(10.0, D_val) + (double)min); \
        else if (sizeof(type) == sizeof(long))                                                               \
            for (i = 0; i < d_nelmts; i++)                                                                   \
                buf[i] = (type)((*(long *)&buf[i] == (long)(((unsigned long)1 << minbits) - 1))              \
                                ? (double)filval                        \
                                : (double)(*(long *)&buf[i]) / HDpow(10.0, D_val) + (double)min); \
        else if (sizeof(type) == sizeof(long long))                                                          \
            for (i = 0; i < d_nelmts; i++)                                                                   \
                buf[i] =                                                                                     \
                    (type)((*(long long *)&buf[i] == (long long)(((unsigned long long)1 << minbits) - 1))    \
                           ? (double)filval                             \
                           : (double)(*(long long *)&buf[i]) / HDpow(10.0, D_val) + (double)min); \
        else                                                                                                 \
            HGOTO_ERROR(H5E_PLINE, H5E_BADTYPE, FAIL, "cannot find matched integer dataype")                 \
    }
...
#define H5Z_scaleoffset_modify_4(i, type, buf, d_nelmts, min, D_val)                                         \
    {                                                                                                        \
        if (sizeof(type) == sizeof(int))                                                                     \
            for (i = 0; i < d_nelmts; i++)                                                                   \
                buf[i] = (type)((double)(*(int *)&buf[i]) / HDpow(10.0, D_val) + (double)min); \
        else if (sizeof(type) == sizeof(long))                                                               \
            for (i = 0; i < d_nelmts; i++)                                                                   \
                buf[i] = (type)((double)(*(long *)&buf[i]) / HDpow(10.0, D_val) + (double)min); \
        else if (sizeof(type) == sizeof(long long))                                                          \
            for (i = 0; i < d_nelmts; i++)                                                                   \
                buf[i] = (type)((double)(*(long long *)&buf[i]) / HDpow(10.0, D_val) + (double)min); \
        else                                                                                                 \
            HGOTO_ERROR(H5E_PLINE, H5E_BADTYPE, FAIL, "cannot find matched integer dataype")                 \
    }
...
H5Z_set_local_scaleoffset(hid_t dcpl_id, hid_t type_id, hid_t space_id)
{
    ...
    /* Set "local" parameter for datatype size */
            cd_values[H5Z_SCALEOFFSET_PARM_SIZE] = (unsigned int) dtype_size;
                       ...
                       size_t *buf_size, void **buf)
{
    ...
    p.mem_order = (unsigned int)H5T_native_order_g;
                                      ...
                                      size_t *j, unsigned *buf_len, parms_atomic p)
{
    ...
    dtype_len = (unsigned int) p.size * 8;

    if (p.mem_order == H5Z_SCALEOFFSET_ORDER_LE) { /* little endian */
        begin_i = (unsigned int)p.size - 1 - (dtype_len - p.minbits) / 8;
