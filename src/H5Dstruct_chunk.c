...
static herr_t H5D__struct_chunk_vector_read(H5D_t *dset, haddr_t addr, H5S_t *file_space_in,
                                            void *chunk /*in*/, size_t *vec_count /*out*/,
                                            haddr_t **offsets /*out*/, size_t **sizes /*out*/,
                                            bool *vector_possible /*out*/, void *udata);

static herr_t H5D__struct_chunk_vector_write(H5D_t *dset, haddr_t addr, H5S_t *file_space_in,
                                             void *chunk /*in*/, size_t *vec_count /*out*/,
                                             haddr_t **offsets /*out*/, size_t **sizes /*out*/,
                                             bool *vector_possible /*out*/, void *udata);
 ...
 */
static herr_t
H5D__struct_chunk_lookup(H5D_t *dset, size_t count, hsize_t *scaled[] /*in*/, haddr_t *addr[] /*out*/,
                         ...
                         void **chunk /*in,out*/, void *_udata)
{
    ...
    if (filtered) {
        ...
        /* Decompress the encoded selection */
        if (H5Z_pipeline(pline, H5Z_FLAG_REVERSE, &(udata->filt_mask[0]), err_detect, filter_cb, &sel_nbytes,
                         &sel_alloc_size, &sel_buf) < 0)
            HGOTO_ERROR(H5E_DATASET, H5E_CANTFILTER, FAIL, "data pipeline read failed");

        /* Decompress the data values */
        if (H5Z_pipeline(pline, H5Z_FLAG_REVERSE, &(udata->filt_mask[1]), err_detect, filter_cb, &data_nbytes,
                                        ...
                                        void **chunk /*in,out*/, void *_udata)
{
    ...
    if (NULL == (chk = H5MM_malloc(sizeof(H5D_chunk_mem_cache_t))))
        HGOTO_ERROR(H5E_RESOURCE, H5E_CANTALLOC, FAIL,
                    "memory allocation failed for chunk's memory cache info");
