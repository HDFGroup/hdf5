 ...
 *
 /* clang-format off */
* \subsubsection subsubsec_filter_plugins_model_apply Applying a Third -
    party Filter When Creating and Writing a Dataset
        /* clang-format on */
        *A                                                                            third -
    party filter can be added to the HDF5 filter pipeline by using the H5Pset_filter *function,
    as a user would do in
        the                  past.The identification number                  andthe filter parameters should be *
            available to the application.For example,
    if the application intends to apply the HDF5 bzip2 compression *
    ...
    : * \code *dcpl = H5Pcreate(H5P_DATASET_CREATE);
...
* \endcode ** \subsubsection subsubsec_filter_plugins_model_read Reading Data with an Applied Third -
    party Filter *An application does not need to do anything special to read the data with a third -
    party filter                                      applied.For * example,
    if one wants to read data written in the previous example,
    the following regular steps should be *taken : * \code *file = H5Fopen(FILE, H5F_ACC_RDONLY, H5P_DEFAULT);
 ...
 *            USER_DEFINED_FILTER {
 *               FILTER_ID 307
 *               COMMENT HDF5 bzip2 filter;
 see *https : //\PLURL/docs/RegisteredFilterPlugins.md
              *PARAMS{2} *
 }
 *
 }
 *FILLVALUE{*FILL_TIME H5D_FILL_TIME_IFSET * VALUE H5D_FILL_VALUE_DEFAULT * } *
     ALLOCATION_TIME{*H5D_ALLOC_TIME_INCR * } * DATA{*... * } *
 }
 *
 }
 *
 }
 * \endcode **If the filter can not be loaded then \ref sec_cltools_h5dump will show the following : * \code *
     ... *
 }
 *DATA{h5dump error : unable to print data * } *
         ... * \endcode ** \subsubsection subsubsec_filter_plugins_model_custom A Word of Caution When Using
             Custom Filters *Data goes through the HDF5 filter pipeline only when it is written to the file or
     read into application *memory space from the file.For example,
     the I / O operation is triggered with a call to #H5Fflush,
     or when *a data item(HDF5 metadata or a raw data chunk) is evicted from the cache or
         brought into the cache.*Please notice that #H5Dread /
                 #H5Dwrite calls on the chunked datasets do not necessarily trigger I / O since
                 * the HDF5 Library uses a separate chunk cache.*
                 *A data item may remain in the cache until the HDF5 Library is closed
                      .If the HDF5 plugin that has to be
                 * applied to the data item becomes unavailable before the file
             and all objects in the file are closed,
     an *error will occur.The following example
     demonstrates the issue.Please notice the position of the *#H5Zunregister call
     : ** \code * // Create a new group using compression.
 ...
 * buffer. If the two differ then zero (failure)
 is returned, otherwise the buffer size is reduced to exclude *the checksum.* \code *size_t
              md5_filter(unsigned int flags, size_t cd_nelmts, const unsigned int cd_values[], *size_t nbytes,
                         size_t *buf_size, void **buf) *
 {
     *#ifdef HAVE_MD5 *unsigned char cksum[16];
 ...
 * \endcode **Once the filter function is defined it must be registered so *the HDF5 library knows about
         it.Since we're testing this *filter we choose one of the #H5Z_filter_t numbers *from the reserved
             range.We'll randomly choose 305. ** \code *#define FILTER_MD5 305 *
 ...
 * \code *const H5Z_class2_t H5Z_BZIP2[1] = {{
 ...
 *     <table>
 *     <tr><td><code>H5PL_type_t H5PLget_plugin_type(void);
 </ code></ td></ tr> *<tr><td><code> const void *H5PLget_plugin_info(void);</code></td></tr>
 *     </table><br />
 ...
 *     <tr><td><code>H5PL_type_t H5PLget_plugin_type(void)
 ...
 </ code></ td></ tr> *<tr><td><code> const void *
