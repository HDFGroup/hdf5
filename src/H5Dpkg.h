...
/* Macro to check if in-place type conversion will be used for a piece and add it to the global type conversion size if it won't be used */
#define H5D_INIT_PIECE_TCONV(IO_INFO, DINFO, PIECE_INFO) \
 ...
 \
            /* Make sure the memory type is not smaller than the file type, otherwise the memory buffer won't be big enough tp serve as the type conversion buffer */ \
 ...
 \
        /* If we're not using in-place type conversion, add this piece to global type conversion buffer size.  This will only be used if we must allocate a type conversion buffer for the entire I/O. */ \
        if (!(PIECE_INFO)->in_place_tconv) \
            (IO_INFO)->tconv_buf_size += (PIECE_INFO)->piece_points * MAX((DINFO)->type_info.src_type_size, (DINFO)->type_info.dst_type_size); \
    }
...
typedef struct H5D_io_info_t {
    ...
    hbool_t may_use_in_place_tconv; /* Whether datasets in this I/O could potentially use in-place type conversion if the type sizes are compatible with it */
#ifdef H5_HAVE_PARALLEL
