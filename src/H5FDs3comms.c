                                   ...
                                   int _sh_size, hrb_t *http_request)
{
    ...
    while (node != NULL) {
        ...
        ret = snprintf(tmpstr, 256, "%s;", node->lowername);
        if (ret < 0 || ret >= 256)
    ...
    do {
        ...
        for (setting_i = 0; setting_i < setting_count; setting_i++) {
            ...
            if (!strncmp(line_buffer, line_prefix, setting_name_len + 1)) {
                ...
                buffer_i = 0;
                while (!isspace((int)(unsigned char)setting_pointers[setting_i][buffer_i]))
                    buffer_i++;
...
H5FD_s3comms_parse_url(const char *str, parsed_url_t **_purl)
{
    ...
    if (':' == *tmpstr) {
        ...
        for (i = 0; i < len; i++)
            if (!isdigit((int)(unsigned char)curstr[i]))
...
H5FD_s3comms_trim(char *dest, char *s, size_t s_len, size_t *n_written)
{
    ...
    if (s_len > 0) {
         ...
         */
        while ((s_len > 0) && isspace((int)(unsigned char)s[0]) && s_len > 0) {
            s++;
            s_len--;
        }
        ...
        if (s_len > 0) {
            do {
                s_len--;
            } while (isspace((int)(unsigned char)s[s_len]));
            s_len++;
...
H5FD_s3comms_uriencode(char *dest, const char *s, size_t s_len, hbool_t encode_slash, size_t *n_written)
{
    ...
    for (s_off = 0; s_off < s_len; s_off++) {
        c = s[s_off];
        if (isalnum((int)(unsigned char)c) || c == '.' || c == '-' || c == '_' || c == '~' ||
