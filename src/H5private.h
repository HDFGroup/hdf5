 ...
 */
#ifdef H5_HAVE_ENDIAN_H
#   include <endian.h>
/* Encode macros for std integer types */
#   define INT16ENCODE(p, i) do {                                           \
    *((uint16_t *)p) = htole16((uint16_t)i); (p) += H5_SIZEOF_INT16_T;      \
} while(0)
#   define UINT16ENCODE(p, i) do {                                          \
    *((uint16_t *)p) = htole16(i); (p) += H5_SIZEOF_UINT16_T;               \
} while(0)
#   define INT32ENCODE(p, i) do {                                           \
    *((uint32_t *)p) = htole32((uint32_t)i); (p) += H5_SIZEOF_INT32_T;      \
} while(0)
#   define UINT32ENCODE(p, i)  do {                                         \
    *((uint32_t *)p) = htole32(i); (p) += H5_SIZEOF_UINT32_T;               \
} while(0)
#   define INT64ENCODE(p, i) do {                                           \
    *((uint64_t *)p) = htole64((uint64_t)i); (p) += H5_SIZEOF_INT64_T;      \
} while(0)
#   define UINT64ENCODE(p, i) do {                                          \
    *((uint64_t *)p) = htole64(i); (p) += H5_SIZEOF_UINT64_T;               \
} while(0)
/* Decode macros for std integer types */
#   define INT16DECODE(p, i) do {                                           \
    i = (int16_t)le16toh(*(uint16_t *)p); (p) += H5_SIZEOF_INT16_T;         \
} while(0)
#   define UINT16DECODE(p, i) do {                                          \
    i = le16toh(*(uint16_t *)p); (p) += H5_SIZEOF_UINT16_T;                 \
} while(0)
#   define INT32DECODE(p, i) do {                                           \
    i = (int32_t)le32toh(*(uint32_t *)p); (p) += H5_SIZEOF_INT32_T;         \
} while(0)
#   define UINT32DECODE(p, i) do {                                          \
    i = le32toh(*(uint32_t *)p); (p) += H5_SIZEOF_UINT32_T;                 \
} while(0)
#   define INT64DECODE(p, i) do {                                           \
    i = (int64_t)le64toh(*(uint64_t *)p); (p) += H5_SIZEOF_INT64_T;         \
} while(0)
#   define UINT64DECODE(p, i) do {                                          \
    i = le64toh(*(uint64_t *)p); (p) += H5_SIZEOF_UINT64_T;                 \
} while(0)
#else /* H5_HAVE_ENDIAN_H */
#   define INT16ENCODE(p, i) do {                                           \
 ...
 * correctly even if it is actually a 64bit int like in a Cray.
 */
#   define INT16DECODE(p, i) do {                                           \
    (i)  = (int16_t)((*(p) & 0xff));      (p)++;                            \
    (i) |= (int16_t)(((*(p) & 0xff) << 8) |                                 \
