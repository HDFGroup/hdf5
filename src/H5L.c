...
static herr_t H5L__get_name_by_idx_cb(H5G_loc_t *grp_loc /*in*/, const char *name, const H5O_link_t *lnk,
                                      H5G_loc_t *obj_loc, void *_udata /*in,out*/,
                                      H5G_own_loc_t *own_loc /*out*/);
static herr_t H5L__create_soft_api_common(const char *link_target, hid_t link_loc_id, const char *link_name, hid_t lcpl_id,
...
static herr_t H5L__delete_by_idx_api_common(hid_t loc_id, const char *group_name, H5_index_t idx_type,
    H5_iter_order_t order, hsize_t n, hid_t lapl_id, void **token_ptr, H5VL_object_t ** _vol_obj_ptr);
static htri_t H5L__exists_api_common(hid_t loc_id, const char *name, hid_t lapl_id,
    void **token_ptr, H5VL_object_t ** _vol_obj_ptr);
static herr_t H5L__iterate_api_common(hid_t group_id, H5_index_t idx_type, H5_iter_order_t order, hsize_t *idx_p, 
    ...
    hid_t lapl_id, void **token_ptr, H5VL_object_t ** _vol_obj_ptr)
{
    ...
    if (H5VL_setup_name_args(link_loc_id, link_name, H5P_CLS_LACC, TRUE, lapl_id, vol_obj_ptr, &loc_params) < 0)
 ...
 */
herr_t
H5Lcreate_soft_async(const char *app_file, const char *app_func, unsigned app_line,
    const char *link_target, hid_t link_loc_id, const char *link_name, hid_t lcpl_id,
...
H5L__create_hard_api_common(hid_t cur_loc_id, const char *cur_name, hid_t new_loc_id, 
    const char *new_name, hid_t lcpl_id, hid_t lapl_id, void **token_ptr, H5VL_object_t ** _vol_obj_ptr)
{
    ...
    H5VL_object_t **  tmp_vol_obj_ptr_ptr = (_vol_obj_ptr ? _vol_obj_ptr : &tmp_vol_obj_ptr);   /* Ptr to ptr to temporary object */
 ...
 */
herr_t
H5Lcreate_hard_async(const char *app_file, const char *app_func, unsigned app_line,
    hid_t cur_loc_id, const char *cur_name, hid_t new_loc_id, const char *new_name,
    hid_t lcpl_id, hid_t lapl_id, hid_t es_id)
{
    ...
    if(H5L__create_hard_api_common (cur_loc_id, cur_name, new_loc_id, new_name, 
            lcpl_id, lapl_id, token_ptr, &vol_obj) < 0)
    ...
    void **token_ptr, H5VL_object_t ** _vol_obj_ptr)
{
    ...
    /* Unlink */
    if (H5VL_link_specific(*vol_obj_ptr, &loc_params, H5VL_LINK_DELETE, H5P_DATASET_XFER_DEFAULT,
                           token_ptr) < 0)
 ...
 */
herr_t
H5Ldelete_async(const char *app_file, const char *app_func, unsigned app_line,
    ...
    H5_iter_order_t order, hsize_t n, hid_t lapl_id, void **token_ptr, H5VL_object_t ** _vol_obj_ptr)
{
    ...
    /* Set up object access arguments */
    if (H5VL_setup_idx_args(loc_id, group_name, idx_type, order, n, H5P_CLS_LACC, TRUE, lapl_id, vol_obj_ptr, &loc_params) < 0)
 ...
 */
herr_t
H5Ldelete_by_idx_async(const char *app_file, const char *app_func, unsigned app_line,
