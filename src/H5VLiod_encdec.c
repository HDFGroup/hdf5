...
int hg_proc_link_iterate_t(hg_proc_t proc, void *data)
{
    int ret = HG_SUCCESS;
    unsigned i;
    hg_proc_op_t op;
    link_iterate_t *iterate_str = (link_iterate_t *) data;
    ...
    case HG_ENCODE:
        for(i=0 ; i<iterate_str->num_objs ; i++) {
            ...
            ret = hg_proc_linfo_t(proc, &iterate_str->linfos[i]);
            if (ret != HG_SUCCESS) {
                HG_ERROR_DEFAULT("Proc error");
                ret = HG_FAIL;
                return ret;
    ...
    case HG_DECODE:
        if(iterate_str->num_objs) {
            iterate_str->linfos = (H5O_ff_info_t *)calloc(iterate_str->num_objs, sizeof(H5O_ff_info_t));
            iterate_str->paths = (const char **)calloc(iterate_str->num_objs, sizeof(char *));
        }
        for(i=0 ; i<iterate_str->num_objs ; i++) {
            ret = hg_proc_hg_const_string_t(proc, &(iterate_str->paths[i]));
