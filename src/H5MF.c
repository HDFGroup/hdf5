 ...
 *              available for reuse. Renamed from previous H5MF_xfree
 *
...
H5MF__xfree_freedspace(H5MF_freedspace_t *freedspace)
{
    ...
    if(H5MF__freedspace_push(&freedspace->f->shared->freedspace_head, &freedspace->f->shared->freedspace_tail, freedspace) < 0)
 ...
 * Purpose:     Frees part of a file, making that part of the file
 *              available for reuse.
 *              For full SWMR implementation, this function replaced the
 *              previous one, which is renamed to H5MF__xfree_real.
 *
 * Return:      Non-negative on success/Negative on failure
 *
...
HDfprintf(stderr, "%s: Entering - alloc_type = %u, addr = %a, size = %Hu\n", FUNC, (unsigned)alloc_type, addr, size);
    ...
    if((H5F_INTENT(f) & H5F_ACC_SWMR_WRITE) ) {
        ...
        if(H5MF__freedspace_create(f, alloc_type, addr, size, &fs) < 0)
...
H5MF_get_freespace(H5F_t *f, hsize_t *tot_space, hsize_t *meta_size)
{
    ...
    /* (Include the space deferred for freeing also) */
    if(tot_space)
        *tot_space = tot_fs_size + ma_size + sda_size + f->shared->deferred_free_space;
    if(meta_size)
        *meta_size = tot_meta_size + f->shared->deferred_free_space;
