...
#define H5D_EARRAY_STC_IDX_IS_OPEN(idx_info) (NULL != (idx_info)->stc_storage->u.earray.ea)
...
#define H5D_EARRAY_STC_FILT_FILL                                                                             \
    {                                                                                                        \
...
typedef struct H5D_earray_stc_ctx_ud_t {
    const H5F_t *f;           /* Pointer to file info */
...
typedef struct H5D_earray_stc_ctx_t {
    size_t   file_addr_len;  /* Size of addresses in the file (bytes) */
    size_t   chunk_size_len; /* Size of chunk sizes in the file (bytes) */
    unsigned nsects;         /* # of sections */
    unsigned offset_size;    /* TBD: Offset size to encode/decode chunk size */
} H5D_earray_stc_ctx_t;
...
/* Native extensible array element for structured chunks w/o filters */
typedef struct H5D_earray_stc_elmt_t {
    haddr_t  addr;                       /* Address of chunk */
...
/* Extensible array class callbacks for structured chunks w/o filters */
static void  *H5D__earray_stc_crt_context(void *udata);
static herr_t H5D__earray_stc_dst_context(void *ctx);
static herr_t H5D__earray_stc_fill(void *nat_blk, size_t nelmts);
static herr_t H5D__earray_stc_encode(void *raw, const void *elmt, size_t nelmts, void *ctx);
static herr_t H5D__earray_stc_decode(const void *raw, void *elmt, size_t nelmts, void *ctx);
static herr_t H5D__earray_stc_debug(FILE *stream, int indent, int fwidth, hsize_t idx, const void *elmt,
                                    void *dbg_ctx);
static void  *H5D__earray_stc_crt_dbg_context(H5F_t *f, haddr_t obj_addr);
static herr_t H5D__earray_stc_dst_dbg_context(void *dbg_ctx);

/* Extensible array class callbacks for structured chunks w/filters */
static herr_t H5D__earray_stc_filt_fill(void *nat_blk, size_t nelmts);
static herr_t H5D__earray_stc_filt_encode(void *raw, const void *elmt, size_t nelmts, void *ctx);
static herr_t H5D__earray_stc_filt_decode(const void *raw, void *elmt, size_t nelmts, void *ctx);
static herr_t H5D__earray_stc_filt_debug(FILE *stream, int indent, int fwidth, hsize_t idx, const void *elmt,
                                         void *dbg_ctx);

/*
 * Extensible Fixed array indexing operations for structured chunk
 */
static herr_t H5D__earray_stc_idx_init(const H5D_chk_idx_info_t *idx_info, const H5S_t *space,
                                       haddr_t dset_ohdr_addr);
static herr_t H5D__earray_stc_idx_create(const H5D_chk_idx_info_t *idx_info);
static herr_t H5D__earray_stc_idx_open(const H5D_chk_idx_info_t *idx_info);
static herr_t H5D__earray_stc_idx_close(const H5D_chk_idx_info_t *idx_info);
static herr_t H5D__earray_stc_idx_is_open(const H5D_chk_idx_info_t *idx_info, bool *is_open);
static bool   H5D__earray_stc_idx_is_space_alloc(const void *storage);
static herr_t H5D__earray_stc_idx_insert(const H5D_chk_idx_info_t *idx_info, H5D_chunk_ud_t *udata,
                                         const H5D_t *dset);
static herr_t H5D__earray_stc_idx_get_addr(const H5D_chk_idx_info_t *idx_info, H5D_chunk_ud_t *udata);
static herr_t H5D__earray_stc_idx_load_metadata(const H5D_chk_idx_info_t *idx_info);
static herr_t H5D__earray_stc_idx_resize(void *layout);

static int H5D__earray_stc_idx_iterate(const H5D_chk_idx_info_t *idx_info, H5D_chunk_cb_func_t chunk_cb,
                                       void *chunk_udata);

static herr_t H5D__earray_stc_idx_remove(const H5D_chk_idx_info_t *idx_info, H5D_chunk_common_ud_t *udata);

static herr_t H5D__earray_stc_idx_delete(const H5D_chk_idx_info_t *idx_info);

static herr_t H5D__earray_stc_idx_copy_setup(const H5D_chk_idx_info_t *idx_info_src,
                                             const H5D_chk_idx_info_t *idx_info_dst);
static herr_t H5D__earray_stc_idx_copy_shutdown(void *storage_src, void *storage_dst);

static herr_t H5D__earray_stc_idx_size(const H5D_chk_idx_info_t *idx_info, hsize_t *size);
static herr_t H5D__earray_stc_idx_reset(void *storage, bool reset_addr);
static herr_t H5D__earray_stc_idx_dump(const void *storage, FILE *stream);
static herr_t H5D__earray_stc_idx_dest(const H5D_chk_idx_info_t *idx_info);

/* TBD: this is for legacy SWMR and will decide later whether this is needed for structured chunk */
/* Generic extensible array routines */
static herr_t H5D__earray_stc_idx_depend(const H5D_chk_idx_info_t *idx_info);
...
/* Extensible array indexing operations for structured chunk */
const H5D_chunk_ops_t H5D_COPS_STRUCT_CHUNK_EARRAY[1] = {{
    true,                               /* Extensible array indices support SWMR access */
    H5D__earray_stc_idx_init,           /* init */
    H5D__earray_stc_idx_create,         /* create */
    H5D__earray_stc_idx_open,           /* open */
    H5D__earray_stc_idx_close,          /* close */
    H5D__earray_stc_idx_is_open,        /* is_open */
    H5D__earray_stc_idx_is_space_alloc, /* is_space_alloc */
    H5D__earray_stc_idx_insert,         /* insert */
    H5D__earray_stc_idx_get_addr,       /* get_addr */
    H5D__earray_stc_idx_load_metadata,  /* load_metadata */
    H5D__earray_stc_idx_resize,         /* resize */
    H5D__earray_stc_idx_iterate,        /* iterate */
    H5D__earray_stc_idx_remove,         /* remove */
    H5D__earray_stc_idx_delete,         /* delete */
    H5D__earray_stc_idx_copy_setup,     /* copy_setup */
    H5D__earray_stc_idx_copy_shutdown,  /* copy_shutdown */
    H5D__earray_stc_idx_size,           /* size */
    H5D__earray_stc_idx_reset,          /* reset */
    H5D__earray_stc_idx_dump,           /* dump */
    H5D__earray_stc_idx_dest            /* destroy */
}};
...
/* Extensible array class callbacks for dataset structured chunks w/o filters */
const H5EA_class_t H5EA_CLS_STRUCT_CHUNK[1] = {{
    H5EA_CLS_STRUCT_CHUNK_ID,      /* Type of extensible array */
    "Struct chunk w/o filters",    /* Name of extensible array class */
    sizeof(H5D_earray_stc_elmt_t), /* Size of native element */
    H5D__earray_stc_crt_context,   /* Create context */
    H5D__earray_stc_dst_context,   /* Destroy context */
    H5D__earray_stc_fill,          /* Fill block of missing elements callback */
    H5D__earray_stc_encode,        /* Element encoding callback */
    H5D__earray_stc_decode,        /* Element decoding callback */
    H5D__earray_stc_debug,         /* Element debugging callback */
    H5D__earray_stc_crt_dbg_context, /* Create debugging context */
    H5D__earray_stc_dst_dbg_context /* Destroy debugging context */
}};

/* Extensible array class callbacks for dataset structured chunks w/filters */
const H5EA_class_t H5EA_CLS_FILT_STRUCT_CHUNK[1] = {{
    H5EA_CLS_FILT_STRUCT_CHUNK_ID,      /* Type of extensible array */
    "Struct chunk w/filters",           /* Name of extensible array class */
    sizeof(H5D_earray_stc_filt_elmt_t), /* Size of native element */
    H5D__earray_stc_crt_context,        /* Create context */
    H5D__earray_stc_dst_context,        /* Destroy context */
    H5D__earray_stc_filt_fill,          /* Fill block of missing elements callback */
    H5D__earray_stc_filt_encode,        /* Element encoding callback */
    H5D__earray_stc_filt_decode,        /* Element decoding callback */
    H5D__earray_stc_filt_debug,         /* Element debugging callback */
    H5D__earray_stc_crt_dbg_context,    /* Create debugging context */
    H5D__earray_stc_dst_dbg_context     /* Destroy debugging context */
}};
...
/* Declare a free list to manage the H5D_earray_stc_ctx_t struct */
H5FL_DEFINE_STATIC(H5D_earray_ctx_t);
H5FL_DEFINE_STATIC(H5D_earray_stc_ctx_t);

/* Declare a free list to manage the H5D_earray_ctx_ud_t struct */
/* Declare a free list to manage the H5D_earray_stc_ctx_ud_t struct */
H5FL_DEFINE_STATIC(H5D_earray_ctx_ud_t);
H5FL_DEFINE_STATIC(H5D_earray_stc_ctx_ud_t);
 ...
 */
static herr_t
H5D__earray_debug(FILE *stream, int indent, int fwidth, hsize_t idx, const void *elmt, void H5_ATTR_UNUSED *dbg_ctx)
{
 ...
 */
static herr_t
H5D__earray_filt_debug(FILE *stream, int indent, int fwidth, hsize_t idx, const void *_elmt, void H5_ATTR_UNUSED *dbg_ctx)
{
    const H5D_earray_filt_elmt_t *elmt =
        (const H5D_earray_filt_elmt_t *)_elmt; /* Convenience pointer to native elements */
    char temp_str[128];                        /* Temporary string, for formatting */
...
{
    H5D_earray_ctx_ud_t *dbg_ctx = NULL;     /* Context for extensible array callback */
...
H5D__earray_idx_create(const H5D_chk_idx_info_t *idx_info)
{
    ...
    cparam.version = H5EA_HDR_VERSION_0;
...
H5D__earray_idx_iterate(const H5D_chk_idx_info_t *idx_info, H5D_chunk_cb_func_t chunk_cb, void *chunk_udata)
{
    ...
    if (ea_stat.stored.max_idx_set > 0) {
        ...
        if ((ret_value = H5EA_iterate(ea, H5D__earray_idx_iterate_cb, &udata)) < 0)
            HERROR(H5E_DATASET, H5E_BADITER, "unable to iterate over extensible array chunk index");
    } /* end if */
...
/*-------------------------------------------------------------------------
 * Function:    H5D__earray_stc_idx_depend
 *
 * Purpose:     Create flush dependency between extensible array and dataset's
 *              object header.
 *
 ...
 */
static herr_t
H5D__earray_stc_idx_depend(const H5D_chk_idx_info_t *idx_info)
{
    H5O_t              *oh = NULL;           /* Object header */
    H5O_loc_t           oloc;                /* Temporary object header location for dataset */
    H5AC_proxy_entry_t *oh_proxy;            /* Dataset's object header proxy */
    ...
    assert(H5D_CHUNK_IDX_EARRAY == idx_info->stc_storage->idx_type);
    assert(H5_addr_defined(idx_info->stc_storage->idx_addr));
    assert(idx_info->stc_storage->u.earray.ea);

    /* Set up object header location for dataset */
    H5O_loc_reset(&oloc);
    oloc.file = idx_info->f;
    oloc.addr = idx_info->stc_storage->u.earray.dset_ohdr_addr;
    ...
    /* Make the extensible array a child flush dependency of the dataset's object header proxy */
    if (H5EA_depend(idx_info->stc_storage->u.earray.ea, oh_proxy) < 0)
        HGOTO_ERROR(H5E_DATASET, H5E_CANTDEPEND, FAIL,
                    "unable to create flush dependency on object header proxy");
    ...
    FUNC_LEAVE_NOAPI(ret_value)
} /* end H5D__earray_stc_idx_depend() */

/*-------------------------------------------------------------------------
 * Function:    H5D__earray_stc_idx_init
 *
 * Purpose:     Initialize the indexing information for a dataset.
 *
 * Return:      Non-negative on success/Negative on failure
 ...
 */
static herr_t
H5D__earray_stc_idx_init(const H5D_chk_idx_info_t *idx_info, const H5S_t *space, haddr_t dset_ohdr_addr)
{
    hsize_t  max_dims[H5O_LAYOUT_NDIMS]; /* Max. size of dataset dimensions */
    int      unlim_dim;                  /* Rank of the dataset's unlimited dimension */
    int      sndims;                     /* Rank of dataspace */
    ...
    /* Set the unlimited dimension for the layout's future use */
    idx_info->stc_layout->u.earray.unlim_dim = (unsigned)unlim_dim;

    /* Store the dataset's object header address for later */
    idx_info->stc_storage->u.earray.dset_ohdr_addr = dset_ohdr_addr;

done:
    FUNC_LEAVE_NOAPI(ret_value)
} /* end H5D__earray_stc_idx_init() */

/*-------------------------------------------------------------------------
 * Function:    H5D__earray_stc_idx_create
 *
 * Purpose:     Creates a new indexed-storage extensbile array and initializes
 *              the layout struct with information about the storage.  The
 *              struct should be immediately written to the object header.
 *
 *              This function must be called before passing LAYOUT to any of
 ...
 */
static herr_t
H5D__earray_stc_idx_create(const H5D_chk_idx_info_t *idx_info)
{
    H5EA_create_t               cparam;         /* Fixed array creation parameters */
    H5D_earray_stc_ctx_ud_t     udata;          /* User data for extensible array create call */
    ...
    assert(NULL == storage->u.earray.ea);
    ...
    if (idx_info->pline->tot_filt_nsects > 0) {
        ...
        cparam.cls = H5EA_CLS_STRUCT_CHUNK;
    ...
    cparam.max_nelmts_bits = layout->u.earray.cparam.max_nelmts_bits;
    assert(cparam.max_nelmts_bits > 0);
    cparam.idx_blk_elmts = layout->u.earray.cparam.idx_blk_elmts;
    assert(cparam.idx_blk_elmts > 0);
    cparam.sup_blk_min_data_ptrs = layout->u.earray.cparam.sup_blk_min_data_ptrs;
    assert(cparam.sup_blk_min_data_ptrs > 0);
    cparam.data_blk_min_elmts = layout->u.earray.cparam.data_blk_min_elmts;
    assert(cparam.data_blk_min_elmts > 0);
    cparam.max_dblk_page_nelmts_bits = layout->u.earray.cparam.max_dblk_page_nelmts_bits;
    assert(cparam.max_dblk_page_nelmts_bits > 0);

    cparam.version = H5EA_HDR_VERSION_1;
    ...
    /* Create the extensible array for the chunk index */
    if (NULL == (storage->u.earray.ea = H5EA_create(idx_info->f, &cparam, &udata)))
        HGOTO_ERROR(H5E_DATASET, H5E_CANTINIT, FAIL, "can't create extensible array");

    /* Get the address of the extensible array in file */
    if (H5EA_get_addr(storage->u.earray.ea, &(storage->idx_addr)) < 0)
        HGOTO_ERROR(H5E_DATASET, H5E_CANTGET, FAIL, "can't query extensible array address");

    /* TBD: this is for legacy SWMR and will decide later whether this is needed for structured chunk */
    /* Check for SWMR writes to the file */
    if (H5F_INTENT(idx_info->f) & H5F_ACC_SWMR_WRITE)
        if (H5D__earray_stc_idx_depend(idx_info) < 0)
            HGOTO_ERROR(H5E_DATASET, H5E_CANTDEPEND, FAIL,
                        "unable to create flush dependency on object header");

done:
    FUNC_LEAVE_NOAPI(ret_value)
} /* end H5D__earray_stc_idx_create() */

/*-------------------------------------------------------------------------
 * Function:    H5D__earray_stc_idx_open
 *
 ...
 */
static herr_t
H5D__earray_stc_idx_open(const H5D_chk_idx_info_t *idx_info)
{
    size_t                      chunk_size_len; /* Size of encoded chunk size */
    H5D_earray_stc_ctx_ud_t udata;               /* User data for extensible array open call */
    herr_t                  ret_value = SUCCEED; /* Return value */
    ...
    assert(H5D_CHUNK_IDX_EARRAY == idx_info->stc_storage->idx_type);
    assert(H5_addr_defined(idx_info->stc_storage->idx_addr));
    assert(NULL == idx_info->stc_storage->u.earray.ea);
    ...
    /* Open the extensible array for the chunk index */
    if (NULL == (idx_info->stc_storage->u.earray.ea =
                     H5EA_open(idx_info->f, idx_info->stc_storage->idx_addr, &udata)))
        HGOTO_ERROR(H5E_DATASET, H5E_CANTINIT, FAIL, "can't open extensible array");
    ...
    FUNC_LEAVE_NOAPI(ret_value)
} /* end H5D__earray_stc_idx_open() */

/*-------------------------------------------------------------------------
 * Function:    H5D__earray_stc_idx_close
 *
 * Purpose:     Closes an existing extensible array.
 *
 * Return:      Success:    non-negative
 ...
 */
static herr_t
H5D__earray_stc_idx_close(const H5D_chk_idx_info_t *idx_info)
{
    herr_t ret_value = SUCCEED; /* Return value */
    ...
    if (H5EA_close(idx_info->stc_storage->u.earray.ea) < 0)
        HGOTO_ERROR(H5E_DATASET, H5E_CANTCLOSEOBJ, FAIL, "unable to close extensible array");
    idx_info->stc_storage->u.earray.ea = NULL;

done:
    FUNC_LEAVE_NOAPI(ret_value)
} /* end H5D__earray_stc_idx_close() */

/*-------------------------------------------------------------------------
 * Function:    H5D__earray_stc_idx_is_open
 *
 * Purpose:     Query if the index is opened or not
 *
 * Return:      SUCCEED (can't fail)
 ...
 */
static herr_t
H5D__earray_stc_idx_is_open(const H5D_chk_idx_info_t *idx_info, bool *is_open)
{
    FUNC_ENTER_PACKAGE_NOERR
    ...
    *is_open = H5D_EARRAY_STC_IDX_IS_OPEN(idx_info);

    FUNC_LEAVE_NOAPI(SUCCEED)
} /* end H5D__earray_stc_idx_is_open() */

/*-------------------------------------------------------------------------
 * Function:    H5D__earray_stc_idx_is_space_alloc
 *
 * Purpose:     Query if space is allocated for index method
 *
 * Return:      true/false
 ...
 */
static bool
H5D__earray_stc_idx_is_space_alloc(const void *store)
{
    ...
    FUNC_LEAVE_NOAPI((bool)H5_addr_defined(storage->idx_addr))
} /* end H5D__earray_stc_idx_is_space_alloc() */

/*-------------------------------------------------------------------------
 * Function:    H5D__earray_stc_idx_insert
 *
 * Purpose:     Insert chunk address into the indexing structure.
 *
 * Return:      Non-negative on success/Negative on failure
 ...
 */
static herr_t
H5D__earray_stc_idx_insert(const H5D_chk_idx_info_t *idx_info, H5D_chunk_ud_t *udata,
                           const H5D_t H5_ATTR_UNUSED *dset)
{
    H5EA_t *ea;                  /* Pointer to extensible array structure */
    herr_t  ret_value = SUCCEED; /* Return value */
    ...
    /* Check if the extensible array is open yet */
    if (!H5D_EARRAY_STC_IDX_IS_OPEN(idx_info)) {
        /* Open the extensible array in file */
        if (H5D__earray_stc_idx_open(idx_info) < 0)
            HGOTO_ERROR(H5E_DATASET, H5E_CANTOPENOBJ, FAIL, "can't open extensible array");
    }
    else /* Patch the top level file pointer contained in fa if needed */
        H5EA_patch_file(idx_info->stc_storage->u.earray.ea, idx_info->f);

    /* Set convenience pointer to extensible array structure */
    ea = idx_info->stc_storage->u.earray.ea;
    ...
    if (idx_info->pline->tot_filt_nsects > 0) {
        ...
        H5D_earray_stc_elmt_t elmt; /* Fixed array element */
    ...
    FUNC_LEAVE_NOAPI(ret_value)
} /* H5D__earray_stc_idx_insert() */

/*-------------------------------------------------------------------------
 * Function:    H5D__earray_stc_idx_get_addr
 *
 * Purpose:     Get the file address of a chunk if file space has been
 *              assigned.  Save the retrieved information in the udata
 *              supplied.
 ...
 */
static herr_t
H5D__earray_stc_idx_get_addr(const H5D_chk_idx_info_t *idx_info, H5D_chunk_ud_t *udata)
{
    H5EA_t *ea;                  /* Pointer to extensible array structure */
    hsize_t idx;                 /* Array index of chunk */
    herr_t  ret_value = SUCCEED; /* Return value */
    ...
    /* Check for unlimited dim. not being the slowest-changing dim. */
    if (idx_info->stc_layout->u.earray.unlim_dim > 0) {
        hsize_t  swizzled_coords[H5O_LAYOUT_NDIMS];     /* swizzled chunk coordinates */
        unsigned ndims = (idx_info->stc_layout->ndims - 1); /* Number of dimensions */
        unsigned u;

        /* Compute coordinate offset from scaled offset */
        for (u = 0; u < ndims; u++)
            swizzled_coords[u] = udata->common.scaled[u] * idx_info->stc_layout->dim[u];

        H5VM_swizzle_coords(hsize_t, swizzled_coords, idx_info->stc_layout->u.earray.unlim_dim);

        /* Calculate the index of this chunk */
        idx = H5VM_chunk_index(ndims, swizzled_coords, idx_info->stc_layout->u.earray.swizzled_dim,
                               idx_info->stc_layout->u.earray.swizzled_max_down_chunks);
    } /* end if */
    else {
        /* Calculate the index of this chunk */
    ...
    if (idx_info->pline->tot_filt_nsects > 0) {
        ...
        H5D_earray_stc_elmt_t elmt; /* Extensible array element */
    ...
    FUNC_LEAVE_NOAPI(ret_value)
} /* H5D__earray_stc_idx_get_addr() */

/*-------------------------------------------------------------------------
 * Function:    H5D__earray_stc_idx_load_metadata
 *
 * Purpose:     Load additional chunk index metadata beyond the chunk index
 *              itself.
 *
 ...
 */
static herr_t
H5D__earray_stc_idx_load_metadata(const H5D_chk_idx_info_t *idx_info)
{
    H5D_chunk_ud_t chunk_ud;
    hsize_t        scaled[H5O_LAYOUT_NDIMS] = {0};
    herr_t         ret_value                = SUCCEED;
    ...
    /*
     * After opening a dataset that uses an extensible array, the
     * extensible array data block will generally not be read in
    ...
    FUNC_LEAVE_NOAPI(ret_value)
} /* H5D__earray_stc_idx_load_metadata() */

/*-------------------------------------------------------------------------
 * Function:    H5D__earray_stc_idx_resize
 *
 * Purpose:     Calculate/setup the swizzled down chunk array, used for chunk
 *              index calculations.
 *
 ...
 */
static herr_t
H5D__earray_stc_idx_resize(void *ll)
{
    H5O_layout_struct_chunk_t *layout = (H5O_layout_struct_chunk_t *)ll;
    ...
    FUNC_LEAVE_NOAPI(SUCCEED)
} /* end H5D__earray_stc_idx_resize() */

/*-------------------------------------------------------------------------
 * Function:    H5D__earray_stc_idx_iterate_cb
 *
 * Purpose:     Callback routine for extensible array element iteration.
 *
 * Return:      Non-negative on success/Negative on failure
 ...
 */
static int
H5D__earray_stc_idx_iterate_cb(hsize_t H5_ATTR_UNUSED idx, const void *_elmt, void *_udata)
{
    H5D_earray_it_ud_t *udata = (H5D_earray_it_ud_t *)_udata; /* User data */
    unsigned            ndims;                                /* Rank of chunk */
    int                 curr_dim;                             /* Current dimension */
    ...
    /* Compose generic chunk record for callback */
    if (udata->filtered) {
        const H5D_earray_stc_filt_elmt_t *filt_elmt = (const H5D_earray_stc_filt_elmt_t *)_elmt;
        ...
        const H5D_earray_stc_elmt_t *elmt = (const H5D_earray_stc_elmt_t *)_elmt;
    ...
    FUNC_LEAVE_NOAPI(ret_value)
} /* H5D__earray_stc_idx_iterate_cb() */

/*-------------------------------------------------------------------------
 * Function:    H5D__earray_stc_idx_iterate
 *
 * Purpose:     Iterate over the chunks in an index, making a callback
 *              for each one.
 *
 ...
 */
static int
H5D__earray_stc_idx_iterate(const H5D_chk_idx_info_t *idx_info, H5D_chunk_cb_func_t chunk_cb,
                            void *chunk_udata)
{
    H5EA_t     *ea;               /* Pointer to extensible array structure */
    H5EA_stat_t ea_stat;          /* Fixed array statistics */
    ...
    if (ea_stat.stored.max_idx_set > 0) {
        ...
        /* Iterate over the extensible array elements */
        if ((ret_value = H5EA_iterate(ea, H5D__earray_stc_idx_iterate_cb, &udata)) < 0)
            HERROR(H5E_DATASET, H5E_BADITER, "unable to iterate over extensible array chunk index");
    } /* end if */
done:
    FUNC_LEAVE_NOAPI(ret_value)
} /* end H5D__earray_stc_idx_iterate() */

/*-------------------------------------------------------------------------
 * Function:    H5D__earray_stc_idx_remove
 *
 * Purpose:     Remove chunk from index.
 *
 * Return:      Non-negative on success/Negative on failure
 ...
 */
static herr_t
H5D__earray_stc_idx_remove(const H5D_chk_idx_info_t *idx_info, H5D_chunk_common_ud_t *udata)
{
    H5EA_t *ea;                  /* Pointer to extensible array structure */
    hsize_t idx;                 /* Array index of chunk */
    herr_t  ret_value = SUCCEED; /* Return value */
    ...
    if (!H5D_EARRAY_STC_IDX_IS_OPEN(idx_info)) {
        ...
        if (H5EA_patch_file(idx_info->stc_storage->u.earray.ea, idx_info->f) < 0)
            HGOTO_ERROR(H5E_DATASET, H5E_CANTOPENOBJ, FAIL, "can't patch extensible array file pointer");
    ...
    if (idx_info->stc_layout->u.earray.unlim_dim > 0) {
        ...
        /* Compute coordinate offset from scaled offset */
        for (u = 0; u < ndims; u++)
            swizzled_coords[u] = udata->scaled[u] * idx_info->stc_layout->dim[u];
    ...
    FUNC_LEAVE_NOAPI(ret_value)
} /* H5D__earray_stc_idx_remove() */

/*-------------------------------------------------------------------------
 * Function:    H5D__earray_stc_idx_delete_cb
 *
 * Purpose:     Delete space for chunk in file
 *
 * Return:      Success:    Non-negative
 ...
 */
static int
H5D__earray_stc_idx_delete_cb(const void *rec, void *_udata)
{
    H5F_t                        *f         = (H5F_t *)_udata; /* User data for callback */
    ...
    FUNC_LEAVE_NOAPI(ret_value)
} /* end H5D__earray_stc_idx_delete_cb() */

/*-------------------------------------------------------------------------
 * Function:    H5D__earray_stc_idx_delete
 *
 * Purpose:     Delete index and raw data storage for entire dataset
 *              (i.e. all chunks)
 *
 ...
 */
static herr_t
H5D__earray_stc_idx_delete(const H5D_chk_idx_info_t *idx_info)
{
    herr_t ret_value = SUCCEED; /* Return value */
    ...
    if (H5_addr_defined(idx_info->stc_storage->idx_addr)) {
        ...
        /* Close extensible array */
        if (H5D__earray_stc_idx_close(idx_info) < 0)
            HGOTO_ERROR(H5E_DATASET, H5E_CANTCLOSEOBJ, FAIL, "unable to close extensible array");
        ...
        /* Delete extensible array */
        if (H5EA_delete(idx_info->f, idx_info->stc_storage->idx_addr, &ctx_udata) < 0)
            HGOTO_ERROR(H5E_DATASET, H5E_CANTDELETE, FAIL, "unable to delete chunk extensible array");
    ...
    FUNC_LEAVE_NOAPI(ret_value)
} /* end H5D__earray_stc_idx_delete() */

/*-------------------------------------------------------------------------
 * Function:    H5D__earray_stc_idx_copy_setup
 *
 * Purpose:     Set up any necessary information for copying chunks
 *
 * Return:      Non-negative on success/Negative on failure
 ...
 */
static herr_t
H5D__earray_stc_idx_copy_setup(const H5D_chk_idx_info_t *idx_info_src, const H5D_chk_idx_info_t *idx_info_dst)
{
    herr_t ret_value = SUCCEED; /* Return value */
    ...
    /* Check if the source extensible array is open yet */
    if (!H5D_EARRAY_STC_IDX_IS_OPEN(idx_info_src)) {
        /* Open the extensible array in file */
        if (H5D__earray_stc_idx_open(idx_info_src) < 0)
            HGOTO_ERROR(H5E_DATASET, H5E_CANTOPENOBJ, FAIL, "can't open extensible array");
    }
    ...
    /* Create the extensible array that describes chunked storage in the dest. file */
    if (H5D__earray_stc_idx_create(idx_info_dst) < 0)
        HGOTO_ERROR(H5E_DATASET, H5E_CANTINIT, FAIL, "unable to initialize chunked storage");
    assert(H5_addr_defined(idx_info_dst->stc_storage->idx_addr));
    ...
    FUNC_LEAVE_NOAPI(ret_value)
} /* end H5D__earray_stc_idx_copy_setup() */

/*-------------------------------------------------------------------------
 * Function:    H5D__earray_stc_idx_copy_shutdown
 *
 * Purpose:     Shutdown any information from copying chunks
 *
 * Return:      Non-negative on success/Negative on failure
 ...
 */
static herr_t
H5D__earray_stc_idx_copy_shutdown(void *sc_src, void *sc_dst)
{
    ...
    /* Close extensible arrays */
    if (H5EA_close(storage_src->u.earray.ea) < 0)
        HGOTO_ERROR(H5E_DATASET, H5E_CANTCLOSEOBJ, FAIL, "unable to close exntensible array");
    storage_src->u.earray.ea = NULL;
    if (H5EA_close(storage_dst->u.earray.ea) < 0)
        HGOTO_ERROR(H5E_DATASET, H5E_CANTCLOSEOBJ, FAIL, "unable to close exntensible array");
    storage_dst->u.earray.ea = NULL;
    ...
    FUNC_LEAVE_NOAPI(ret_value)
} /* end H5D__earray_stc_idx_copy_shutdown() */

/*-------------------------------------------------------------------------
 * Function:    H5D__earray_stc_idx_size
 *
 * Purpose:     Retrieve the amount of index storage for chunked dataset
 *
 * Return:      Success:        Non-negative
 ...
 */
static herr_t
H5D__earray_stc_idx_size(const H5D_chk_idx_info_t *idx_info, hsize_t *index_size)
{
    H5EA_t     *ea;                  /* Pointer to extensible array structure */
    H5EA_stat_t ea_stat;             /* Extensible array statistics */
    herr_t      ret_value = SUCCEED; /* Return value */
    ...
    /* Open the extensiblee array in file */
    if (H5D__earray_stc_idx_open(idx_info) < 0)
        HGOTO_ERROR(H5E_DATASET, H5E_CANTOPENOBJ, FAIL, "can't open extensible array");
    ...
    *index_size = ea_stat.computed.hdr_size + ea_stat.computed.index_blk_size +
    ...
    if (idx_info->stc_storage->u.earray.ea) {
        if (H5D__earray_stc_idx_close(idx_info) < 0)
            HDONE_ERROR(H5E_DATASET, H5E_CANTCLOSEOBJ, FAIL, "unable to close extensible array");
    } /* end if */

    FUNC_LEAVE_NOAPI(ret_value)
} /* end H5D__earray_stc_idx_size() */

/*-------------------------------------------------------------------------
 * Function:    H5D__earray_stc_idx_reset
 *
 * Purpose:     Reset indexing information.
 *
 * Return:      Non-negative on success/Negative on failure
 ...
 */
static herr_t
H5D__earray_stc_idx_reset(void *store, bool reset_addr)
{
    ...
    FUNC_LEAVE_NOAPI(SUCCEED)
} /* end H5D__earray_stc_idx_reset() */

/*-------------------------------------------------------------------------
 * Function:    H5D__earray_stc_idx_dump
 *
 * Purpose:     Dump indexing information to a stream.
 *
 * Return:      Non-negative on success/Negative on failure
 ...
 */
static herr_t
H5D__earray_stc_idx_dump(const void *sc, FILE *stream)
{
    ...
    FUNC_LEAVE_NOAPI(SUCCEED)
} /* end H5D__earray_stc_idx_dump() */

/*-------------------------------------------------------------------------
 * Function:    H5D__earray_stc_idx_dest
 *
 * Purpose:     Release indexing information in memory.
 *
 * Return:      Non-negative on success/Negative on failure
 ...
 */
static herr_t
H5D__earray_stc_idx_dest(const H5D_chk_idx_info_t *idx_info)
{
    herr_t ret_value = SUCCEED; /* Return value */
    ...
    FUNC_LEAVE_NOAPI(ret_value)
} /* end H5D__earray_stc_idx_dest() */

/*
 * Extensbile array class callbacks for structured chunks
 */

/*-------------------------------------------------------------------------
 * Function:    H5D__earray_stc_crt_context
 *
 * Purpose:     Create context for callbacks
 *
 * Return:      Success:    non-NULL
 ...
 */
static void *
H5D__earray_stc_crt_context(void *_udata)
{
    H5D_earray_stc_ctx_t    *ctx; /* Fixed array callback context */
    H5D_earray_stc_ctx_ud_t *udata =
        (H5D_earray_stc_ctx_ud_t *)_udata; /* User data for extensible array context */
    void *ret_value = NULL;                /* Return value */
    ...
    /* Allocate new context structure */
    if (NULL == (ctx = H5FL_MALLOC(H5D_earray_stc_ctx_t)))
...
/*-------------------------------------------------------------------------
 * Function:    H5D__earray_stc_dst_context
 *
 * Purpose:     Destroy context for callbacks
 *
 * Return:      Success:    non-NULL
 ...
 */
static herr_t
H5D__earray_stc_dst_context(void *_ctx)
{
    H5D_earray_stc_ctx_t *ctx = (H5D_earray_stc_ctx_t *)_ctx; /* Extensible array callback context */
    ...
    /* Release context structure */
    ctx = H5FL_FREE(H5D_earray_stc_ctx_t, ctx);

    FUNC_LEAVE_NOAPI(SUCCEED)
} /* end H5D__earray_stc_dst_context() */

/*-------------------------------------------------------------------------
 * Function:    H5D__earray_stc_fill
 *
 * Purpose:     Fill "missing elements" in block of elements
 *
 * Return:      Success:    non-negative
 ...
 */
static herr_t
H5D__earray_stc_fill(void *nat_blk, size_t nelmts)
{
    H5D_earray_stc_elmt_t fill_val = H5D_EARRAY_STC_FILL; /* Value to fill elements with */
    ...
    assert(nat_blk);
    assert(nelmts);
    assert(sizeof(fill_val) == H5EA_CLS_STRUCT_CHUNK->nat_elmt_size);

    H5VM_array_fill(nat_blk, &fill_val, H5EA_CLS_STRUCT_CHUNK->nat_elmt_size, nelmts);

    FUNC_LEAVE_NOAPI(SUCCEED)
} /* end H5D__earray_stc_fill() */

/*-------------------------------------------------------------------------
 * Function:    H5D__earray_stc_filt_fill
 *
 * Purpose:     Fill "missing elements" in block of elements
 *
 * Return:      Success:    non-negative
 ...
 */
static herr_t
H5D__earray_stc_filt_fill(void *nat_blk, size_t nelmts)
{
    H5D_earray_stc_filt_elmt_t fill_val = H5D_EARRAY_STC_FILT_FILL; /* Value to fill elements with */
    ...
    assert(nat_blk);
    assert(nelmts);
    assert(sizeof(fill_val) == H5EA_CLS_FILT_STRUCT_CHUNK->nat_elmt_size);

    H5VM_array_fill(nat_blk, &fill_val, H5EA_CLS_FILT_STRUCT_CHUNK->nat_elmt_size, nelmts);

    FUNC_LEAVE_NOAPI(SUCCEED)
} /* end H5D__earray_stc_filt_fill() */

/*-------------------------------------------------------------------------
 * Function:    H5D__earray_stc_encode
 *
 * Purpose:     Encode an element from "native" to "raw" form
 *
 * Return:      Success:    non-negative
 ...
 */
static herr_t
H5D__earray_stc_encode(void *raw, const void *_elmt, size_t nelmts, void *_ctx)
{
    H5D_earray_stc_ctx_t        *ctx = (H5D_earray_stc_ctx_t *)_ctx; /* Extensbile array callback context */
    const H5D_earray_stc_elmt_t *elmt =
        (const H5D_earray_stc_elmt_t *)_elmt; /* Convenience pointer to native elements */
    ...
    FUNC_LEAVE_NOAPI(SUCCEED)
} /* end H5D__earray_stc_encode() */

/*-------------------------------------------------------------------------
 * Function:    H5D__earray_stc_filt_encode
 *
 * Purpose:     Encode an element from "native" to "raw" form
 *
 * Return:      Success:    non-negative
 ...
 */
static herr_t
H5D__earray_stc_filt_encode(void *_raw, const void *_elmt, size_t nelmts, void *_ctx)
{
    H5D_earray_stc_ctx_t             *ctx = (H5D_earray_stc_ctx_t *)_ctx; /* Extensible array callback context */
    uint8_t                          *raw = (uint8_t *)_raw; /* Convenience pointer to raw elements */
    const H5D_earray_stc_filt_elmt_t *elmt =
        (const H5D_earray_stc_filt_elmt_t *)_elmt; /* Convenience pointer to native elements */
    ...
    FUNC_LEAVE_NOAPI(SUCCEED)
} /* end H5D__earray_stc_filt_encode() */

/*-------------------------------------------------------------------------
 * Function:    H5D__earray_stc_decode
 *
 * Purpose:     Decode an element from "raw" to "native" form
 *
 * Return:      Success:    non-negative
 ...
 */
static herr_t
H5D__earray_stc_decode(const void *_raw, void *_elmt, size_t nelmts, void *_ctx)
{
    H5D_earray_stc_ctx_t  *ctx  = (H5D_earray_stc_ctx_t *)_ctx;   /* Extensible array callback context */
    H5D_earray_stc_elmt_t *elmt = (H5D_earray_stc_elmt_t *)_elmt; /* Convenience pointer to native elements */
    const uint8_t         *raw  = (const uint8_t *)_raw;          /* Convenience pointer to raw elements */
    ...
    FUNC_LEAVE_NOAPI(SUCCEED)
} /* end H5D__earray_stc_decode() */

/*-------------------------------------------------------------------------
 * Function:    H5D__earray_stc_filt_decode
 *
 * Purpose:     Decode an element from "raw" to "native" form
 *
 * Return:      Success:    non-negative
 ...
 */
static herr_t
H5D__earray_stc_filt_decode(const void *_raw, void *_elmt, size_t nelmts, void *_ctx)
{
    H5D_earray_stc_ctx_t       *ctx = (H5D_earray_stc_ctx_t *)_ctx; /* Extensible array callback context */
    H5D_earray_stc_filt_elmt_t *elmt =
        (H5D_earray_stc_filt_elmt_t *)_elmt;    /* Convenience pointer to native elements */
    const uint8_t *raw = (const uint8_t *)_raw; /* Convenience pointer to raw elements */
    ...
    FUNC_LEAVE_NOAPI(SUCCEED)
} /* end H5D__earray_stc_filt_decode() */

/*-------------------------------------------------------------------------
 * Function:    H5D__earray_stc_debug
 *
 * Purpose:     Display an element for debugging
 *
 * Return:      Success:    non-negative
 ...
 */
static herr_t
H5D__earray_stc_debug(FILE *stream, int indent, int fwidth, hsize_t idx, const void *_elmt, void *_dbg_ctx)
{
    const H5D_earray_stc_elmt_t *elmt =
        (const H5D_earray_stc_elmt_t *)_elmt; /* Convenience pointer to native elements */
    H5D_earray_stc_ctx_t *ctx = (H5D_earray_stc_ctx_t *)_dbg_ctx;
    char                  temp_str[128]; /* Temporary string, for formatting */
    ...
    FUNC_LEAVE_NOAPI(SUCCEED)
} /* end H5D__earray_stc_debug() */

/*-------------------------------------------------------------------------
 * Function:    H5D__earray_stc_filt_debug
 *
 * Purpose:     Display an element for debugging
 *
 * Return:      Success:    non-negative
 ...
 */
static herr_t
H5D__earray_stc_filt_debug(FILE *stream, int indent, int fwidth, hsize_t idx, const void *_elmt,
                           void *_dbg_ctx)
{
    const H5D_earray_stc_filt_elmt_t *elmt =
        (const H5D_earray_stc_filt_elmt_t *)_elmt; /* Convenience pointer to native elements */
    ...
    FUNC_LEAVE_NOAPI(SUCCEED)
} /* end H5D__earray_stc_filt_debug() */

/*-------------------------------------------------------------------------
 * Function:    H5D__earray_stc_crt_dbg_context
 *
 * Purpose:     Create context for debugging callback
 *              (get the layout message in the specified object header)
 *
 ...
 */
static void *
H5D__earray_stc_crt_dbg_context(H5F_t *f, haddr_t obj_addr)
{
    H5D_earray_stc_ctx_ud_t *dbg_ctx = NULL;     /* Context for extensible array callback */
    H5O_loc_t                obj_loc;            /* Pointer to an object's location */
    bool                     obj_opened = false; /* Flag to indicate that the object header was opened */
    H5O_layout_t             layout;             /* Layout message */
    void                    *ret_value = NULL;   /* Return value */
    ...
    /* Allocate context for debugging callback */
    if (NULL == (dbg_ctx = H5FL_MALLOC(H5D_earray_stc_ctx_ud_t)))
        HGOTO_ERROR(H5E_DATASET, H5E_CANTALLOC, NULL, "can't allocate extensible array client callback context");
    ...
    if (ret_value == NULL) {
        /* Release context structure */
        if (dbg_ctx)
            dbg_ctx = H5FL_FREE(H5D_earray_stc_ctx_ud_t, dbg_ctx);
    ...
    FUNC_LEAVE_NOAPI(ret_value)
} /* end H5D__earray_stc_crt_dbg_context() */

/*-------------------------------------------------------------------------
 * Function:    H5D__earray_stc_dst_dbg_context
 *
 * Purpose:     Destroy context for debugging callback
 *              (free the layout message from the specified object header)
 *
 ...
 */
static herr_t
H5D__earray_stc_dst_dbg_context(void *_dbg_ctx)
{
    H5D_earray_stc_ctx_ud_t *dbg_ctx =
        (H5D_earray_stc_ctx_ud_t *)_dbg_ctx; /* Context for extensible array callback */
    ...
    FUNC_LEAVE_NOAPI(SUCCEED)
} /* end H5D__earray_stc_dst_dbg_context() */
