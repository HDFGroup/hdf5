 ...
 * compression algorithm. The Consultative Committee on Space Data Systems (CCSDS) has adopted the extended-Rice
 * algorithm for international standards for space applications[1,6,7]. Szip is reported to provide fast and
 * effective compression, specifically for the EOS data generated by the NASA Earth Observatory System (EOS)[1].
 * It was originally developed at University of New Mexico (UNM) and integrated with HDF by UNM researchers and developers.
 *
 * The primary gain with Szip compression is in speed of processing. Szip also provides some advantage in compression
 * ratio over other compression methods.
 *
 * <h4>Szip and HDF5</h4>
 * Using Szip compression in HDF5: Szip is a stand-alone library that is configured as an optional filter in HDF5.
 * Depending on which Szip library is used (encoder enabled or decode-only), an HDF5 application can create, write,
 * and read datasets compressed with Szip compression, or can only read datasets compressed with Szip.
 * Applications use Szip by setting Szip as an optional filter when a dataset is created. If the Szip encoder is
 * enabled with the HDF5 library, data is automatically compressed and decompressed with Szip during I/O. If only the
 ...
 * The following sample program illustrates the use of Szip compression in HDF5. This sample program is also available in the subdirectory HDF5Examples.
 * #include "hdf5.h"
 * #define NX 500
 * #define NY 600
 * #define CH_NX 100
 ...
 *
 * Software distribution: Starting with Release 1.6.0, HDF5 has been distributed with Szip enabled, making it
 * easier to use Szip compression. The software is distributed as follows:
 * - Pre-compiled HDF5 binaries are provided with the Szip encoder enabled.
 * - Szip source code can be obtained at: https://github.com/MathisRosenhauer/libaec.git
 *
