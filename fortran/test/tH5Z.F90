...
!   Tests h5pappend_filter_f, h5pmodify_filter_by_idx_f and
!   h5pget_filter_params_by_idx_f
...
!
      ...
      IF (avail) THEN
         CALL h5pcreate_f(H5P_DATASET_CREATE_F, dcpl, error)
              CALL check("h5pcreate_f", error, total_error)
         CALL h5pappend_filter_f(dcpl, H5Z_FILTER_DEFLATE_F, 0, "level=6", error)
...
!
      ...
      IF (avail) THEN
         CALL h5pcreate_f(H5P_DATASET_CREATE_F, dcpl, error)
              CALL check("h5pcreate_f", error, total_error)
         CALL h5pappend_filter_f(dcpl, H5Z_FILTER_DEFLATE_F, 0, "level=1", error)
              CALL check("h5pappend_filter_f(deflate,str)", error, total_error)

         ! String form: replace the configuration in place
         CALL h5pmodify_filter_by_idx_f(dcpl, 0, 0, "level=9", error)
