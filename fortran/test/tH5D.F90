...
!
  ...
  SUBROUTINE test_direct_chunk_io(cleanup, total_error)
    ...
    CHARACTER(LEN=15), PARAMETER :: dsetname_c = "dset_c"
    ...
    INTEGER(HSIZE_T), DIMENSION(2) :: dims_c = (/DIM1,DIM0/)
    INTEGER(C_INT), DIMENSION(CHUNK0,CHUNK1), TARGET :: wdata1, rdata1, wdata2, rdata2
    INTEGER(C_INT), DIMENSION(CHUNK1,CHUNK0), TARGET :: wdata1_c, rdata1_c, wdata2_c, rdata2_c
    INTEGER(HSIZE_T), DIMENSION(2) :: chunk = (/CHUNK0, CHUNK1/)
    INTEGER(HSIZE_T), DIMENSION(2) :: chunk_c = (/CHUNK1, CHUNK0/)
    INTEGER :: i, j, n
    INTEGER :: error
    TYPE(C_PTR) :: f_ptr
    ...
    CALL h5screate_simple_f(RANK, dims_c, dataspace, error)
    CALL check("h5screate_simple_f",error,total_error)

    CALL h5pcreate_f(H5P_DATASET_CREATE_F, dcpl, error)
    CALL check("h5pcreate_f",error,total_error)

    CALL h5pset_chunk_f(dcpl, RANK, chunk_c, error)
    CALL check("h5pset_chunk_f",error,total_error)

    CALL h5dcreate_f(file_id, dsetname_c, H5T_NATIVE_INTEGER, dataspace, dset_id, error, dcpl )
    CALL check("h5dcreate_f",error,total_error)
    ...
    f_ptr = C_LOC(wdata1_c)
    offset(1:2) = (/0,0/)
    CALL H5Dwrite_chunk_f(dset_id, 0_C_INT32_T, offset, CHUNK0 * CHUNK1 * sizeINT, f_ptr, error, use_c_indx=.TRUE.)
    CALL check("h5dwrite_f",error,total_error)

#if 1
    f_ptr = C_LOC(wdata2_c)
    offset(1:2) = (/0, 16/)
    CALL h5dwrite_chunk_f(dset_id, 0_C_INT32_T, offset, CHUNK0 * CHUNK1 * sizeINT, f_ptr, error, dxpl, use_c_indx=.TRUE.)
    CALL check("h5dwrite_chunk_f",error,total_error)
#endif
    CALL h5dclose_f(dset_id, error)
    CALL check("h5dclose_f",error,total_error)
