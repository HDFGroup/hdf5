...
!!
  SUBROUTINE h5dread_chunk_f(dset_id, offset, filters, buf, hdferr, dxpl_id, buf_size)
    IMPLICIT NONE
    ...
    INTEGER           , INTENT(OUT) :: hdferr
    INTEGER(HID_T)    , INTENT(IN), OPTIONAL :: dxpl_id
    INTEGER(HID_T)    , INTENT(INOUT), OPTIONAL :: buf_size
    ...
    IF(.NOT. C_ASSOCIATED(buf) )THEN
       ...
       ENDIF
       RETURN
    ELSE IF(.NOT.PRESENT(buf_size))THEN ! Get the buf size parameter
       cbuf_size = 0_C_SIZE_T
       hdferr = INT(H5Dread_chunk2(dset_id, dxpl_id_default, c_offset, c_filters, C_NULL_PTR, cbuf_size))
    ELSE
       cbuf_size = INT(buf_size, C_SIZE_T)
    ...
    filters = INT(c_filters)
    IF(PRESENT(buf_size)) buf_size = INT(cbuf_size, SIZE_T)
