...
hsize_t h5tools_dump_region_data_points(hid_t region_space,
        hid_t region_id, FILE *stream, const h5tool_format_t *info, 
        h5tools_context_t *ctx/*in,out*/,
        h5tools_str_t *buffer/*string into which to render */,
        hsize_t curr_pos/*total data element position*/, unsigned flags,
        size_t ncols, hsize_t *elmt_counter, hsize_t i_count/*element counter*/);
hsize_t h5tools_dump_region_data_blocks(hid_t region_space,
        hid_t region_id, FILE *stream, const h5tool_format_t *info, 
        h5tools_context_t *ctx/*in,out*/,
        h5tools_str_t *buffer/*string into which to render */,
        hsize_t curr_pos/*total data element position*/, unsigned flags,
        ...
        hsize_t nelmts, hid_t type, void *_mem)
{
    ...
    else
    {
        ...
        for (i = 0; i < nelmts; i++, ctx->cur_elmt++, elmt_counter++) {
            ...
            curr_pos = h5tools_render_element(stream, info, ctx, &buffer, curr_pos, flags, ncols, &elmt_counter, i);
            f_type = H5Dget_type(container);
            if (region_output && H5Tequal(f_type, H5T_STD_REF_DSETREG)) {
                ...
                region_space = H5Rget_region(container, H5R_DATASET_REGION, mem + i * size);

                /* Print block information */
                curr_pos = h5tools_dump_region_data_blocks(region_space, region_id, 
                        stream, info, ctx, &buffer, curr_pos, flags, ncols, &elmt_counter, i);
                /* Print point information */
                curr_pos = h5tools_dump_region_data_points(region_space, region_id, 
                        stream, info, ctx, &buffer, curr_pos, flags, ncols, &elmt_counter, i);
        ...
        size_t ncols, hsize_t *elmt_counter, hsize_t i_count/*element counter*/)
{
     ...
     */
    {H5E_BEGIN_TRY{nblocks = H5Sget_select_hyper_nblocks(region_space);}H5E_END_TRY};
    if (nblocks <= 0)
        return curr_pos;
    ...
    H5Sget_select_hyper_blocklist(region_space, (hsize_t)0, (hsize_t)nblocks, ptdata);
    ...
    for (blkndx=0; blkndx<nblocks; blkndx++) {
        ...
        status = H5Dread(region_id, type_id, mem_space, sid1, H5P_DEFAULT, region_buf);

        /* Render the element */
        h5tools_str_reset(buffer);

        for (jndx=0; jndx<numelem; jndx++) {
            h5tools_str_sprint(buffer, info, region_id, type_id, (region_buf+jndx*type_size), ctx);

            if (jndx + 1 < numelem || (flags & END_OF_DATA) == 0)
                h5tools_str_append(buffer, "%s", OPT(info->elmt_suf1, ","));
        }
        curr_pos = h5tools_render_element(stream, info, ctx, buffer, curr_pos, flags, ncols, elmt_counter, i_count);
    }
    free(start);
    free(count);
    free(region_buf);
        ...
        size_t ncols, hsize_t *elmt_counter, hsize_t i_count/*element counter*/)
{
     ...
     * This function fails if the region does not have blocks. 
     */
    {H5E_BEGIN_TRY{npoints = H5Sget_select_elem_npoints(region_space);}H5E_END_TRY};
    if (npoints > 0) {
        dtype = H5Dget_type(region_id);
        type_id = H5Tget_native_type(dtype, H5T_DIR_DEFAULT);
        type_size = H5Tget_size(type_id);
