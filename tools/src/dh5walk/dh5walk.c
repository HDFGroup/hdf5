...
static void save_command(char *argv0)
{
    assert(argv0);
...
static size_t get_local_bufsize(uint64_t *bufsize)
{
    size_t total = 0;
    if (buft_count > 0) {
...
static void dh5tool_flist_write_text(
    const char* name,
    mfu_flist bflist)
{
...
static void filter_hdf_files(mfu_flist* pflist, char *regex_exp, int exclude, int name)
{
    mfu_flist flist = *pflist;
...
static
int count_paths(int argc, int startcnt, char **argv, int **index_out)
{
    int k;
    int path_cnt = 0;
...
#if 1
static
char ** copy_args(int argc, char** argv, int *mfu_argc, int *copy_len)
{
...
#else
static
char ** copy_args(int argc, char** argv, int *argc_out, int *copy_len)
{
    int i, bytes_copied = 0;
    int copy_arg = 1;
    int args_copied = 0;
...
check_path_count(char *h5tool)
{
    ...
    if ((strstr(thisName, "copy")   != NULL) ||
        (strstr(thisName, "dump")   != NULL) ||
        (strstr(thisName, "format") != NULL) ||
        (strstr(thisName, "ls")     != NULL) ||
        (strstr(thisName, "stat")   != NULL)) {
        return 1;
    }
    else if (
        (strstr(thisName, "copy")   != NULL) || 
        (strstr(thisName, "diff")   != NULL) ||
        (strstr(thisName, "repack") != NULL)) {
        return 2;
    }
    else if ((strstr(thisName, "import") != NULL) || 
             (strstr(thisName, "jam") != NULL)) {
        return 3;
    }
    return -1;
}
...
int MFU_PRED_EXEC (mfu_flist flist, uint64_t idx, void* arg)
{
    ...
    if (log_output_in_single_file) {
        ...
        if (txtlog == NULL) 
            ...
            else sprintf(logpath, "%s/%s_%s.log", txtlog,logbase,thisapp);
        }
        ...
        int status;
        pid = wait(&status);
        if (logbase) free(logbase);
        if (thisapp) free(thisapp);
    }
