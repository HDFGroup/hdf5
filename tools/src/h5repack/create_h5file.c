...
create_attributes(hid_t group_id)
{
    ...
    for (i = 0; i < hand.nattrs; i++) {
        ...
        if ((attr_id = H5Acreate2(group_id, attr_name, H5T_NATIVE_INT, attr_space, H5P_DEFAULT, H5P_DEFAULT)) < 0) {
...
create_dsets(hid_t file_id)
{
    ...
    for (k = 0; k < hand.ndsets; k++) {
        ...
        if ((dset_id = H5Dcreate2(file_id, dset_name, H5T_NATIVE_INT, dset_space, H5P_DEFAULT, dcpl_id, H5P_DEFAULT)) < 0) {
            fprintf(stderr, "couldn't create dataset '%s'\n", dset_name);
            goto error;
        }

        if (!hand.empty_dsets && H5Dwrite(dset_id, H5T_NATIVE_INT, dset_space, dset_space, H5P_DEFAULT, dset_data) < 0) {
