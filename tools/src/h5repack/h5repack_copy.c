...
//printf("Line %d, g_nID=%d, offset[1]=%ld, stride[1]=%ld, count[1]=%ld, block[1]=%ld\n", __LINE__, g_nID, hs_context->hs_offset[1], hs_context->hs_stride[1], hs_context->hs_count[1], hs_context->hs_block[1]);
                            ...
                            for (elmtno = 0; elmtno < p_nelmts; elmtno += hs_select_nelmts) {
                                if (rank > 0) {
                                    ...
                                    if (H5Sselect_hyperslab(f_space_id, H5S_SELECT_SET, hs_sel_offset, NULL,
                                                            hs_sel_count, NULL) < 0)
                                        H5TOOLS_GOTO_ERROR((-1), "H5Sselect_hyperslab failed");
                                    if (H5Sselect_hyperslab(hslab_space, H5S_SELECT_SET, zero, NULL,
                                ...
                                if (H5Dread(dset_in, wtype_id, hslab_space, f_space_id, H5P_DEFAULT,
                                            hslab_buf) < 0)
